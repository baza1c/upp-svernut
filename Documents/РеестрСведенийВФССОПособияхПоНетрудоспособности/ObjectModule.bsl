...
Процедура ПроверитьЗаполнениеСтрокиТЧРаботникиОрганизации(ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаИзмененияКодаПричиныНетрудоспособности) И Найти(",017,018,019,", "," + ВыборкаПоСтрокамДокумента.ДополнительныйКодПричиныНетрудоспособности + ",") > 0 Тогда
		Если Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаОкончанияПутевки) Тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(ТекстОшибкиСНомеромСтроки + "Если указана дата начала санаторной путевки, то в поле ""Дата 2"" должна быть указана дата окончания путевки!", Отказ, Заголовок);
		КонецЕсли;
		Если Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.НомерПутевки) Тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(ТекстОшибкиСНомеромСтроки + "Если указана дата начала санаторной путевки, то должен быть указан ее номер!", Отказ, Заголовок);
		КонецЕсли;
