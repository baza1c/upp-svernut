...
Процедура СформироватьСчетаФактурыВыданныеДляСтрокДокумента(ТабЧасть = "") Экспорт
	...
	Иначе
		...
		ТаблицаСФ.Колонки.Добавить("НомерСФпродавца", Новый ОписаниеТипов("Строка"));
	КонецЕсли;
	...
	Для каждого ТабЧасть Из СписокТабЧастей Цикл
		...
		ТаблицаДокумента.Колонки.Добавить("НомерСФпродавца");
		...
		Для каждого СтрокаДок Из ЭтотОбъект[ТабЧасть.Значение] Цикл
			...
			//добавлено 240626
			СтрокаТаблицыДок.НомерСФпродавца = СтрокаДок.НомерСФпродавца;
		...
		ТаблицаДокумента.Свернуть("Покупатель, ДатаСФ, СчетФактура, НомерСФпродавца", "СуммаБезНДС, НДС");
		...
		Для каждого СтрокаТаблицыДок Из ТаблицаДокумента Цикл
			...
			Если ДокСФ = Неопределено Тогда
				...
				//добавлено 240626
				ДокСФ.НомерСФпродавца = СтрокаТаблицыДок.НомерСФпродавца;
...
Функция ПолучитьТаблицуПодчиненныхСчетовФактур(ДокументОснование)
	...
	|	СчетФактураВыданный.Ссылка.НомерСФпродавца КАК НомерСФпродавца
	|ИЗ
	|	Документ.СчетФактураВыданный.ДокументыОснования КАК СчетФактураВыданный
	|ГДЕ
...
Функция НайтиСчетФактуруВСписке(ТаблицаСФ, СтрокаДокумента, УстановитьПризнакИспользования = Истина)
	...
	Если ЗначениеЗаполнено(СтрокаДокумента.СчетФактура)
		...
		Если ЗначениеЗаполнено(СтрокаДокумента.НомерСФпродавца) Тогда
	    	СтрокаСФ = ТаблицаСФ.Найти(СтрокаДокумента.НомерСФпродавца, "НомерСФпродавца");
		КонецЕсли;	 
	...
	Если НЕ ЗначениеЗаполнено(СтрокаДокумента.НомерСФпродавца) Тогда
	...
	Иначе
		Для каждого СтрокаСФ Из ТаблицаСФ Цикл
			// Поиск существующего счета-фактуры выполняется по контрагенту и договору.
			Если НачалоДня(СтрокаСФ.ДатаСФ) = НачалоДня(СтрокаДокумента.ДатаСФ) И СтрокаСФ.НомерСФпродавца = СтрокаДокумента.НомерСФпродавца
				И СтрокаСФ.Покупатель = СтрокаДокумента.Покупатель ИЛИ НЕ ЗначениеЗаполнено(СтрокаСФ.Покупатель) Тогда
				// найден подходящий счет-фактура
				ДокСФ = СтрокаСФ.Ссылка.ПолучитьОбъект();
				Если ТаблицаСФ.Колонки.Найти("Использован") <> Неопределено И УстановитьПризнакИспользования Тогда
...
Процедура ПроверитьЗаполнениеШапки(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Новый Структура("Организация, ВалютаДокумента,Контрагент, ДоговорКонтрагента, СпособРасчетаКомиссионногоВознаграждения, КратностьВзаиморасчетов, СтатьяЗатрат");
...
Процедура ПодготовитьТаблицыДокумента(СтруктураШапкиДокумента, ТаблицаПоУслугам, ТаблицаПоТоварам, ТаблицаПоДенежнымСредствам) Экспорт
    ...
    РезультатЗапросаПоТоварам = СформироватьЗапросПоТоварам(ЭтотОбъект);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//{Лео Катанович
	СуммаДокумента      = УчетНДС.ПолучитьСуммуДокументаСНДС(ЭтотОбъект, "Товары") -  УчетНДС.ПолучитьСуммуДокументаСНДС(ЭтотОбъект, "Возвраты");

	СуммаНДСВознаграждения = Товары.Итог("СуммаНДСВознаграждения") - Возвраты.Итог("СуммаНДСВознаграждения");
	СуммаВознаграждения = (Товары.Итог("СуммаВознаграждения") - Возвраты.Итог("СуммаВознаграждения"))+ 
...
Функция СформироватьЗапросПоТоварам(ДокументОбъект);
Запрос = Новый Запрос;
Запрос.Текст = "ВЫБРАТЬ
               ...
               |	СУММА(ВложенныйЗапрос.СуммаПередачи) КАК СуммаПередачи,
               |	ВложенныйЗапрос.СтавкаНДС,
               |	СУММА(ВложенныйЗапрос.НДС) КАК НДС,
               |	СУММА(ВложенныйЗапрос.НДСДок) КАК НДСДок,
               |	СУММА(ВложенныйЗапрос.СуммаНДСПередачи) КАК СуммаНДСПередачи,
               |	СУММА(ВложенныйЗапрос.СуммаВознаграждения) КАК СуммаВознаграждения,
               |	ВложенныйЗапрос.СтавкаНДСВознаграждения,
               |	СУММА(ВложенныйЗапрос.СуммаНДСВознаграждения) КАК СуммаНДСВознаграждения,
               |	ВложенныйЗапрос.ХарактеристикаНоменклатуры,
               |	ВложенныйЗапрос.СерияНоменклатуры,
               |	ВложенныйЗапрос.ПринадлежностьНоменклатуры,
               |	ВложенныйЗапрос.НоменклатурнаяГруппа,
               |	ВложенныйЗапрос.СчетУчетаБУ,
               |	ВложенныйЗапрос.СчетУчетаНУ,
               |	ВложенныйЗапрос.СчетДоходовБУ,
               |	ВложенныйЗапрос.СубконтоБУ,
               |	ВложенныйЗапрос.СчетДоходовНУ,
               |	ВложенныйЗапрос.СубконтоНУ,
               |	ВложенныйЗапрос.СчетРасходовБУ,
               |	ВложенныйЗапрос.СчетРасходовНУ,
               |	ВложенныйЗапрос.ПереданныеПринятыеБУ,
               |	ВложенныйЗапрос.ПереданныеПринятыеНУ
               |ИЗ
               |	(ВЫБРАТЬ
               ...
               |		Док.СуммаПередачи КАК СуммаПередачи,
               |		Док.СтавкаНДС КАК СтавкаНДС,
               |		Док.СуммаНДС КАК НДС,
               |		Док.СуммаНДС КАК НДСДок,
               |		Док.СуммаНДСПередачи КАК СуммаНДСПередачи,
               |		Док.СуммаВознаграждения КАК СуммаВознаграждения,
               |		Док.Ссылка.СтавкаНДСВознаграждения КАК СтавкаНДСВознаграждения,
               |		Док.СуммаНДСВознаграждения КАК СуммаНДСВознаграждения,
               |		Док.ХарактеристикаНоменклатуры КАК ХарактеристикаНоменклатуры,
               |		Док.СерияНоменклатуры КАК СерияНоменклатуры,
               |		Док.ПринадлежностьНоменклатуры КАК ПринадлежностьНоменклатуры,
               ...
               |		Док.СчетДоходовБУ КАК СчетДоходовБУ,
               |		Док.СубконтоБУ КАК СубконтоБУ,
               |		Док.СчетДоходовНУ КАК СчетДоходовНУ,
               |		Док.СубконтоНУ КАК СубконтоНУ,
               |		Док.СчетРасходовБУ КАК СчетРасходовБУ,
               |		Док.СчетРасходовНУ КАК СчетРасходовНУ,
               |		Док.ПереданныеПринятыеБУ КАК ПереданныеПринятыеБУ,
               |		Док.ПереданныеПринятыеНУ КАК ПереданныеПринятыеНУ
               |	ИЗ
               |		Документ.ОтчетКомиссионераОПродажах.Товары КАК Док
               |	ГДЕ
               ...
               |		Док1.НомерСтроки,
               |		Док1.Номенклатура,
               |		Док1.Номенклатура.Услуга,
               |		Док1.Номенклатура.Набор,
               |		-1 * Док1.Количество * Док1.Коэффициент / Док1.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент,
               |		-1 * Док1.Сумма,
               |		-1 * Док1.Сумма,
               |		-1 * Док1.СуммаПередачи,
               |		Док1.СтавкаНДС,
               |		-1 * Док1.СуммаНДС,
               |		-1 * Док1.СуммаНДС,
               |		-1 * Док1.СуммаНДСПередачи,
               |		-1 * Док1.СуммаВознаграждения,
               |		Док1.Ссылка.СтавкаНДСВознаграждения,
               |		-1 * Док1.СуммаНДСВознаграждения,
               |		Док1.ХарактеристикаНоменклатуры,
               |		Док1.СерияНоменклатуры,
               |		Док1.ПринадлежностьНоменклатуры,
               |		Док1.Номенклатура.НоменклатурнаяГруппа,
               |		Док1.СчетУчетаБУ,
               |		Док1.СчетУчетаНУ,
               |		Док1.СчетДоходовБУ,
               |		Док1.СубконтоБУ,
               ...
               |		Док1.СчетРасходовБУ,
               |		Док1.СчетРасходовНУ,
               |		Док1.ПереданныеПринятыеБУ,
               |		Док1.ПереданныеПринятыеНУ
               |	ИЗ
               |		Документ.ОтчетКомиссионераОПродажах.Возвраты КАК Док1
               |	ГДЕ
               |		Док1.Ссылка = &ДокументСсылка) КАК ВложенныйЗапрос
               |
               |СГРУППИРОВАТЬ ПО
               |	ВложенныйЗапрос.Номенклатура,
               |	ВложенныйЗапрос.Услуга,
