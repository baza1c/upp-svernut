...
Процедура Рассчитать() Экспорт
	...
	ВременнаяТаблицаНДФЛ.ЗаполнитьЗначения(0,"НалогПоСтавке09,НалогПоСтавке13,НалогПоСтавке35,ПримененныйВычетЛичный" +
	...
	Если ДанныеНДФЛ <> НеОпределено Тогда
		...
		ДанныеНДФЛ.Свернуть("ФизЛицо, ПодразделениеОрганизации", "Налог,ПримененныйВычетЛичный" +
				",ПримененныйВычетНаДетей,ПримененныйВычетНаДетейДвойной,ПримененныйВычетНаДетейДвойнойВторой" +
				",ПримененныйВычетНаТретьегоРебенка,ПримененныйВычетНаТретьегоРебенкаДвойной,ПримененныйВычетНаТретьегоРебенкаДвойнойВторой" +
				",ПримененныйВычетНаВторогоРебенка,ПримененныйВычетНаВторогоРебенкаДвойной,ПримененныйВычетНаВторогоРебенкаДвойнойВторой" +
				",ПримененныйВычетНаДетейИнвалидов,ПримененныйВычетНаДетейИнвалидовДвойной,ПримененныйВычетНаДетейИнвалидовДвойнойВторой" +
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СУММА(НДФЛОбороты.СуммаДоходаОборот) КАК СуммаДохода,
	|	СУММА(НДФЛОбороты.СуммаВычетаОборот) КАК СуммаВычета,
	...
	|			Месяц,
	|			НАЧАЛОПЕРИОДА(ПериодРегистрации, МЕСЯЦ) <= &парамПериодРегистрации
	|				И Организация = &парамГоловнаяОрганизация
	|				И КодДохода.СтавкаНалогообложенияРезидента <> ЗНАЧЕНИЕ(Перечисление.НДФЛСтавкиНалогообложенияРезидента.Ставка13)
	|				И Физлицо В
	|					(ВЫБРАТЬ
	|						СписокФизЛиц.ФизЛицо
	|					ИЗ
	...
	|	ПериодыРегистра.ПериодПолученияДохода
	|ПОМЕСТИТЬ ВТСписокФизлицРезидентов
	|ИЗ
	|	(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|		Физлица.ФизЛицо КАК ФизЛицо,
	|		Физлица.ПериодПолученияДохода КАК ПериодПолученияДохода,
	...
	|	СГРУППИРОВАТЬ ПО
	|		Физлица.ФизЛицо,
	|		Физлица.ПериодПолученияДохода) КАК ПериодыРегистра
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтатусФизЛицКакНалогоплательщиковНДФЛ КАК СтатусНалогоплательщика
	...
	|			И НДФЛОбороты.ПериодПолученияДохода = СписокФизлицРезидентов.ПериодПолученияДохода
	|
	|СГРУППИРОВАТЬ ПО
	|	НДФЛОбороты.ФизЛицо
	|;
	...
	Если Не Результат[ВсегоРезультатов - 2].Пустой() Тогда
		...
		ВременнаяТаблицаНДФЛ.Свернуть("ФизЛицо, ПодразделениеОрганизации", "НалогПоСтавке09,НалогПоСтавке13,НалогПоСтавке35,ПримененныйВычетЛичный" +
				",ПримененныйВычетНаДетей,ПримененныйВычетНаДетейДвойной,ПримененныйВычетНаДетейДвойнойВторой" +
				",ПримененныйВычетНаТретьегоРебенка,ПримененныйВычетНаТретьегоРебенкаДвойной,ПримененныйВычетНаТретьегоРебенкаДвойнойВторой" +
				",ПримененныйВычетНаВторогоРебенка,ПримененныйВычетНаВторогоРебенкаДвойной,ПримененныйВычетНаВторогоРебенкаДвойнойВторой" +
				",ПримененныйВычетНаДетейИнвалидов,ПримененныйВычетНаДетейИнвалидовДвойной,ПримененныйВычетНаДетейИнвалидовДвойнойВторой" +
