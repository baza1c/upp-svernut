...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	КонецЕсли;
	 ...
	 Если (ВидСобытия = Перечисления.ВидыСобытий.ПередачаКонтакта) Тогда
 
	            НоваяЗадача = Задачи.ЗадачаИсполнителя.СоздатьЗадачу();
				НоваяЗадача.Дата			= ТекущаяДата();
				НоваяЗадача.ВидЗадачи	    = Справочники.ВидыЗадачПользователей.Исполнение;
				НоваяЗадача.ОбъектЗадачи	= Ссылка;
		        НоваяЗадача.Наименование	= "Ознакомиться с документом: "  + Строка(ВидСобытия) + " " + Контрагент.Наименование + " от " + Ответственный.Наименование;
				НоваяЗадача.Исполнитель	    = ОтветственныйНовый;
				НоваяЗадача.Уровень         = 0;
...
Функция УстановитьНовогоМенеджераКонтрагента();
КонтрагентСсылка = Контрагент.ПолучитьОбъект(); 
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |ГДЕ
		               |	Регионы.абс_Менеджер = &абс_Менеджер
		               |	И Регионы.абс_ТипБизнесРегиона = ЗНАЧЕНИЕ(Перечисление.абс_ТипыБизнесРегионов.БизнесРегион)" ;
		
		
		Запрос.УстановитьПараметр("абс_Менеджер",ОтветственныйНовый);
		
		Результат = Запрос.Выполнить().Выбрать();
		Если Результат.Следующий() Тогда
			 Регион = Результат.Ссылка;
		 Иначе     
			 ТекстСообщения = ("Для менеджера " + ОтветственныйНовый.Наименование + " не установлен Бизнес-регион." + Символы.ПС + "Изменение не выполнено!");	 
	 ...
	 КонецЕсли;	
		...
		Если НЕ Регион	= Неопределено Тогда
			...
			КонтрагентСсылка.Регион = Регион;
			КонтрагентСсылка.абс_Город 		= ПолучитьБизнесРегионПоУровню(3,Регион);
			КонтрагентСсылка.абс_Территория 	= ПолучитьБизнесРегионПоУровню(2,Регион);
			КонтрагентСсылка.абс_БизнесРегион 	= ПолучитьБизнесРегионПоУровню(1,Регион);
			КонтрагентСсылка.абс_Отдел			= ПолучитьБизнесРегионПоУровню(0,Регион);
			КонтрагентСсылка.МенеджерыПокупателя.Добавить().МенеджерПокупателя = КонтрагентСсылка.абс_БизнесРегион.абс_Менеджер;
			КонтрагентСсылка.ОсновнойМенеджерПокупателя = КонтрагентСсылка.Регион.абс_Менеджер;
		КонецЕсли;		
...
//СформироватьЗадачуПользователю();
	
КонецФункции  



  Функция ПолучитьБизнесРегионПоУровню(Уровень, Регион) Экспорт
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если (ВидСобытия = Перечисления.ВидыСобытий.ПередачаКонтакта) Тогда
		  Если НЕ ЗначениеЗаполнено(ОтветственныйНовый) Тогда
	        	Сообщить("Не указан новый ответственный сотрудник");
		       Отказ = Истина;     
		   ИначеЕсли  НЕ ЗначениеЗаполнено(Контрагент) Тогда
	           Сообщить("Не указан Контрагент");
		       Отказ = Истина;               
		   Иначе	   
			   УстановитьНовогоМенеджераКонтрагента();
              	//Найдем окно комментариев, при необходимости откроем его
				
				ФормаОбработки_CRM = Обработки.РабочееМестоМенеджераПоПродажам.ПолучитьФорму("ФормаCRM_Медицина");
	            Если ФормаОбработки_CRM.Открыта() Тогда
		             ФормаОбработки_CRM.СформироватьЗаголовки();
                КонецЕсли;
		 КонецЕсли;  
   КонецЕсли; 
