...
Функция ВыгрузитьДокумент(УникальныйИдентификатор = Неопределено) Экспорт
	...
	Отказ = Документы.УведомлениеОКонтролируемыхСделках.ПроверитьСведенияОВыгрузке(ОсновныеСведенияВыгрузки);
...
Процедура ЗаполнитьСведенияОСделке502(Контекст, ТекстСделки, Сведения)
	...
	ЭтоГруппаОднородныхСделок = (Сведения.Количество1Б > 1);
...
Процедура ЗаполнитьСведенияОПредметеСделки502(Контекст, ТекстСделки, Листы1Б, Сделка)
	...
	Для Каждого ТипПредмета ИЗ ТипыПредметовСделок Цикл
		...
		Для Каждого Лист1Б ИЗ ТипПредмета.Значение Цикл
			...
			УстановитьАтрибут(Шаблон_ПерПредСд, "ОКЕИ", Лист1Б.Строка110КодЕдиницыИзмерения);
