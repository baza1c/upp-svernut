...
Функция ПроверитьСведенияОВыгрузке(СведенияОВыгрузке) Экспорт
	
	Отказ = Ложь;
	
	Если НЕ (СведенияОВыгрузке.Свойство("КодНО") 
			И ЗначениеЗаполнено(СведенияОВыгрузке.КодНО)
			И СтрДлина(СведенияОВыгрузке.КодНО)=4) Тогда
		СообщениеОбОшибке = НСтр("ru = 'Укажите код налогового органа для организации'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеОбОшибке, СведенияОВыгрузке.Организация, "РегистрацияВНалоговомОргане");
		Отказ = Истина;
	КонецЕсли;
	
	Если НЕ (СведенияОВыгрузке.Свойство("ПоМесту") 
			И ЗначениеЗаполнено(СведенияОВыгрузке.ПоМесту)) Тогда
		СообщениеОбОшибке = НСтр("ru = 'Укажите код места представления документа в настройках заполнения'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеОбОшибке);
		Отказ = Истина;
	КонецЕсли;
	
	Если НЕ (СведенияОВыгрузке.Свойство("ОКАТО") 
			И ЗначениеЗаполнено(СведенияОВыгрузке.ОКАТО)) Тогда
		СообщениеОбОшибке = НСтр("ru = 'Укажите для организации код ОКАТО налогового органа'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеОбОшибке, СведенияОВыгрузке.Организация, "РегистрацияВНалоговомОргане");
		Отказ = Истина;
	КонецЕсли;
	
	Если СведенияОВыгрузке.ЭтоПБОЮЛ Тогда
		Если НЕ (СведенияОВыгрузке.Свойство("НПФЛФамилия") 
				И ЗначениеЗаполнено(СведенияОВыгрузке.НПФЛФамилия))
			ИЛИ НЕ (СведенияОВыгрузке.Свойство("НПФЛИмя") 
				И ЗначениеЗаполнено(СведенияОВыгрузке.НПФЛИмя))Тогда
			СообщениеОбОшибке = НСтр("ru = 'Укажите фамилию и имя индивидуального предпринимателя'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеОбОшибке, СведенияОВыгрузке.Организация, "ИндивидуальныйПредприниматель");
			Отказ = Истина;
		КонецЕсли;
		
		Если НЕ (СведенияОВыгрузке.Свойство("ИННФЛ") 
				И ЗначениеЗаполнено(СведенияОВыгрузке.ИННФЛ)) Тогда
			СообщениеОбОшибке = НСтр("ru = 'Укажите ИНН индивидуального предпринимателя'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеОбОшибке, СведенияОВыгрузке.Организация, "ИндивидуальныйПредприниматель");
			Отказ = Истина;
		КонецЕсли;
		
	Иначе
		Если НЕ (СведенияОВыгрузке.Свойство("НаимОрг") 
				И ЗначениеЗаполнено(СведенияОВыгрузке.НаимОрг)) Тогда
			СообщениеОбОшибке = НСтр("ru = 'Укажите полное наименование организации'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеОбОшибке, СведенияОВыгрузке.Организация, "НаименованиеПолное");
			Отказ = Истина;
		КонецЕсли;
		
		Если НЕ (СведенияОВыгрузке.Свойство("ИННЮЛ") 
				И ЗначениеЗаполнено(СведенияОВыгрузке.ИННЮЛ)) Тогда
			СообщениеОбОшибке = НСтр("ru = 'Укажите ИНН организации'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеОбОшибке, СведенияОВыгрузке.Организация, "ИНН");
			Отказ = Истина;
		КонецЕсли;
		
		Если НЕ (СведенияОВыгрузке.Свойство("КППЮЛ") 
				И ЗначениеЗаполнено(СведенияОВыгрузке.КППЮЛ)) Тогда
			СообщениеОбОшибке = НСтр("ru = 'Укажите КПП организации'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеОбОшибке, СведенияОВыгрузке.Организация, "ИНН");
			Отказ = Истина;
		КонецЕсли;
