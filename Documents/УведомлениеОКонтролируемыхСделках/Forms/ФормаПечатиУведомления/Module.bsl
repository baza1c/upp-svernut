...
&НаСервере
Функция ПроверитьСведенияОВыгрузкеНаСервере()
	
	ОсновныеСведенияВыгрузки = Документы.УведомлениеОКонтролируемыхСделках.ПолучитьОсновныеСведенияУведомленияДляВыгрузки(Уведомление);
	
	Возврат Документы.УведомлениеОКонтролируемыхСделках.ПроверитьСведенияОВыгрузке(ОсновныеСведенияВыгрузки);
...
Процедура СформироватьЛистыУведомленияВФоне()
	...
	Отказ = ПроверитьСведенияОВыгрузкеНаСервере();
	Если Отказ Тогда
		УстановитьСостояниеПоляТабличногоДокумента(Элементы.ОтображениеЛистов, "НеИспользовать");
		Возврат;
	КонецЕсли;
