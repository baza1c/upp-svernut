...
Функция ПодготовитьДанныеДляПечатиУниверсальногоПередаточногоДокумента()
	...
	ДанныеДляПечати.Вставить("ДатаСчетФактуры",	   		Шапка.ДатаДокумента);
...
Функция ПечатьМ15()
	...
	ЗапросПоТоварам.Текст = "ВЫБРАТЬ
	                        ...
	                        |		ПередачаТоваров.ХарактеристикаНоменклатуры КАК Характеристика,
	                        |		ПередачаТоваров.СерияНоменклатуры КАК Серия,
	                        |		ПередачаТоваров.ВесФакт КАК ВесФакт
	                        |	ИЗ
	                        |		Документ.ПередачаТоваров.Товары КАК ПередачаТоваров
	                        |	ГДЕ
	                        |		ПередачаТоваров.Ссылка = &ТекущийДокумент
	                        ...
	                        |		ПередачаТоваров.СерияНоменклатуры,
	                        |		ПередачаТоваров.Цена,
	                        |		ПередачаТоваров.ВесФакт
	                        |	
	                        |	ОБЪЕДИНИТЬ ВСЕ
	                        |	
	                        |	ВЫБРАТЬ
	                        |		1,
	                        |		МАКСИМУМ(ПередачаТоваров.НомерСтроки),
	                        |		ПередачаТоваров.Номенклатура,
	                        |		ПередачаТоваров.Номенклатура.ЕдиницаХраненияОстатков,
	                        |		СУММА(ПередачаТоваров.Количество),
	                        |		ПередачаТоваров.Цена * &Курс / &Кратность,
