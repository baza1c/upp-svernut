...
Процедура ЗаполнитьДанными(МодельВыгрузки, Параметры)
	...
	МодельВыгрузки.ЗаписатьНачалоЭлемента("Документ");
		...
		ЗаписатьАтрибут(МодельВыгрузки, "КНД", "1115102");
...
Процедура ЗаполнитьДаннымиВсего(МодельВыгрузки, Параметры)
	...
	МодельВыгрузки.ЗаписатьНачалоЭлемента("СвКнПок");
		...
		МодельВыгрузки.ЗаписатьНачалоЭлемента("Всего");
			ЗаписатьАтрибут(МодельВыгрузки, "СтТовУчНалВсего", ВсегоПокупок, "Ч, 17, 2, О");
...
Процедура ЗаполнитьДаннымиТабличнойЧасти(МодельВыгрузки, Параметры)
	...
	Для Каждого СтрокаТабличнойЧасти Из ТабличнаяЧасть Цикл
		...
		ЗаписатьАтрибут(МодельВыгрузки, "ДатаПринУчет", СтрокаТабличнойЧасти.ДатаОприходования, "Д, , , Н");
		
		ЗаписатьАтрибут(МодельВыгрузки, "НаимПрод", СтрокаТабличнойЧасти.Продавец, "Т, 1000, , О");
		...
		Иначе
			...
			ЗаписатьАтрибут(МодельВыгрузки, "КПП", СтрокаТабличнойЧасти.ПродавецКПП, "Т, 9, , Н");
		КонецЕсли;
		...
		Если РеквизитыПроисхожденияТовара.Количество() >= 2 Тогда
			ЗаписатьАтрибут(МодельВыгрузки, "КодПроисх", СокрЛП(РеквизитыПроисхожденияТовара[0]), "Т, 3, ,Н");
		КонецЕсли;
		
		ЗаписатьАтрибут(МодельВыгрузки, "СтТовУчНалВсего", СтрокаТабличнойЧасти.ВсегоПокупок, "Ч, 17, 2, О");
		...
		Для Инд = 1 По РеквизитыПроисхожденияТовара.Количество() - 1 Цикл
			Если ЗначениеЗаполнено(РеквизитыПроисхожденияТовара[Инд]) Тогда
				МодельВыгрузки.ЗаписатьНачалоЭлемента("НомерТД");
					МодельВыгрузки.ЗаписатьТекст(Лев(СокрЛП(РеквизитыПроисхожденияТовара[Инд]), 29));
				МодельВыгрузки.ЗаписатьКонецЭлемента();
			КонецЕсли;
		КонецЦикла;
