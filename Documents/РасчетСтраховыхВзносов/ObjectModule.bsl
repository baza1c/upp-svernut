...
//  Выборка результата запроса
//
Функция ПолучитьДанныеРасчетаЗа2012Год(ГоловнаяОрганизация, ОрганичениеНаСотрудников, ДанныеОДоходах, Отказ)
...
Процедура РассчитатьВзносы(ВыборкаПоШапкеДокумента, ОграничениеНаСотрудников = Неопределено)
	...
	ИначеЕсли ПериодРегистрации < ПроведениеРасчетов.ДатаВводаДополнительногоТарифаЗаРаботыСДосрочнойПенсией() Тогда
		ИсчисленныеСтраховыеВзносы.Загрузить(ПолучитьДанныеРасчетаЗа2012Год(ВыборкаПоШапкеДокумента.Организация, ОграничениеНаСотрудников, НаборЗаписейОДоходах, Отказ));	
	Иначе
