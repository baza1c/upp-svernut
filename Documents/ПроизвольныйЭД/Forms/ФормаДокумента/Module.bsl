...
Процедура ПодписатьДокумент(Команда)
	...
	МассивДоступныхСертификатов = ТаблицаДоступныхСертификатов(МассивСтруктурСертификатов);
	
	ПараметрыОбработкиОбъектов = Неопределено;
	Если Не ЭлектронныеДокументыСлужебныйКлиент.ПарольПользователя(МассивДоступныхСертификатов, ПараметрыОбработкиОбъектов,
		НСтр("ru = 'Подписание электронных документов'"), Вложения[0].Ссылка) Тогда
	
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	
	Если ТипЗнч(ПараметрыОбработкиОбъектов) = Тип("Структура") Тогда
		Если ЭлектронныеДокументыСлужебныйВызовСервера.ВыполнятьКриптооперацииНаСервере() Тогда
			ПодписатьДокументНаСервере(ПараметрыОбработкиОбъектов, Подписан);
		Иначе
			ПодписатьДокументНаКлиенте(ПараметрыОбработкиОбъектов, Подписан);
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Функция ТаблицаДоступныхСертификатов(МассивСтруктурСертификатов)
	...
	Если ЗначениеЗаполнено(НастройкиОбмена) И НастройкиОбмена.Свойство("СоглашениеЭД") Тогда
		СоглашениеЭД = НастройкиОбмена.СоглашениеЭД;
	КонецЕсли;
	
	Возврат ЭлектронныеДокументыСлужебный.МассивСтруктурДоступныхДляПодписиСертификатов(МассивСтруктурСертификатов, СоглашениеЭД);
