...
Процедура РассчитатьНДФЛ(КомментироватьРасчет = Ложь, ПроводитьПроверки = Истина, ВыборкаПоШапкеДокумента = Неопределено) Экспорт
	...
	Если ДанныеНДФЛ <> Неопределено Тогда
		...
		Для каждого Физлицо Из МассивФизлиц Цикл
			ПроведениеРасчетов.ВписатьПромежуточныйНДФЛвДокумент(НДФЛ, ДанныеНДФЛ.Скопировать(ДанныеНДФЛ.НайтиСтроки(Новый Структура("Физлицо",Физлицо))), ВыборкаПоШапкеДокумента.ДатаВыплатыДохода, МесяцыПолученияДохода);
		КонецЦикла; 
...
Процедура СформироватьВзаиморасчетыСРаботниками(ВыборкаПоШапкеДокумента, НаборЗаписей, Перерасчет = Ложь, Физлица = Неопределено)
	...
	Если Перерасчет Тогда
		Если Физлица = Неопределено Тогда
			...
			|		-РегистрацияРазовыхНачисленийРаботниковОрганизацийНДФЛ.Налог
			|	ИЗ
			...
			|	ГДЕ
			|		РегистрацияРазовыхНачисленийРаботниковОрганизацийНДФЛ.Налог <> 0
		...
		Иначе
			...
			|		И РегистрацияРазовыхНачисленийРаботниковОрганизацийНДФЛ.Налог <> 0
