...
Функция ПечатьДоверенности(НазваниеМакета) Экспорт
	...
	//АБС ИЗМЕНЕНИЕ   02.04.2014 17:24:10  Балашенко
	Запрос.Текст = 
	"ВЫБРАТЬ
	...
	ОбластьМакета.Параметры.ОрганизацияПредставление      = ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОбОрганизации, "ПолноеНаименование") + Символы.ПС + ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОбОрганизации, "ИНН,КПП,ЮридическийАдрес,Телефоны,");
	...
	//абс вставка 38913
	Если абс_ВидДоверенности=Перечисления.абс_ВидыДоверенностей.СдачаТМЦ Тогда
	...
	//абс изменение 38913
	Если абс_ВыводитьТабличнуюЧастьСтрокой Тогда
		ОбластьМакета = Макет.ПолучитьОбласть("Строка");
		ОбластьМакета.Параметры.Номер="1";
		Если абс_ВидДоверенности=Перечисления.абс_ВидыДоверенностей.СдачаТМЦ Тогда
			ОбластьМакета.Параметры.ЦеннностиПредставление="Сдача ТМЦ";
		Иначе
			Дополнение = ПоДокументу;
			// {Лео Катанович
			Если ЗначениеЗаполнено(Шапка.ДокументОснование) Тогда
				Если ТипЗнч(Шапка.ДокументОснование) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
					Дополнение = ""; 
			...
			// Лео Катанович}
			// ОбластьМакета.Параметры.ЦеннностиПредставление="ТМЦ согласно товарной накладной " + ПоДокументу;
		КонецЕсли;
...
Процедура ОбработкаЗаполнения(Основание)
	...
	//АБС ВСТАВКА   07.04.2014 17:55:55  Балашенко
	абс_ВыводитьТабличнуюЧастьСтрокой = Истина;
	//АБС ВСТАВКА  КОНЕЦ
...
Процедура ПриКопировании(ОбъектКопирования)
	 абс_ДокументОснование = Неопределено;
	 Сделка = Неопределено; 
	 ПоДокументу = "";
