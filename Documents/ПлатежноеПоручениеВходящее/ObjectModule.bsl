...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//АБС ВСТАВКА   28.03.2014 11:33:09  Балашенко
	Если Не ЗначениеЗаполнено(Подразделение) Тогда
		Подразделение = Контрагент.ОсновнойМенеджерПокупателя.абс_Подразделение;
	КонецЕсли;
	//АБС ВСТАВКА  КОНЕЦ
	...
	Если Не Проведен И Не абс_РучнойУчетВзаиморасчетов Тогда
		Попытка
		ТекущаяСтатьяДвиженияДенежныхСредств=РасшифровкаПлатежа[0].СтатьяДвиженияДенежныхСредств;
		Исключение
		КонецПопытки;
		...
		Если НЕ ТекущаяСтатьяДвиженияДенежныхСредств = Неопределено Тогда
			Для Каждого Строка из РасшифровкаПлатежа Цикл
				Если НЕ ЗначениеЗаполнено(Строка.СтатьяДвиженияДенежныхСредств) Тогда
						Строка.СтатьяДвиженияДенежныхСредств = ТекущаяСтатьяДвиженияДенежныхСредств;
				КонецЕсли;
			КонецЦикла;	
		КонецЕсли;			
	КонецЕсли;
	...
	////+Лео Сафонов ЕА
	//Если ЭтоНовый()  Тогда	
	//		Если ЭтотОбъект.Дата = НачалоДня(ЭтотОбъект.Дата) Тогда
	//		ЭтотОбъект.Дата = Дата(Формат(ЭтотОбъект.Дата, "ДФ=dd.MM.yyyy") +" 23:59:59");
	//   	КонецЕсли;    
	//КонецЕсли;
	////-Лео Сафонов ЕА
