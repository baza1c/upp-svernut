...
Функция ПечатьСписаниеТоваров()
	...
	|	ПРЕДСТАВЛЕНИЕ(ВЫБОР
	|			КОГДА СписаниеТоваров.ОтражатьВБухгалтерскомУчете
	...
	|		ПО (ТаблицаСоответствиеОрганизации.Подразделение = СписаниеТоваров.Подразделение)
...
Процедура ДвиженияПоРегистрам(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам, Отказ, Заголовок);
	...
	ТаблицаПоТоварамОбщая = ТаблицаПоТоварам.Скопировать();
	НаборЗначений = Новый Структура;
	НаборЗначений.Вставить("ВидДвижения", "Расход");
	
	ТаблицаПоТоварамРасход = ТаблицаПоТоварамОбщая.Скопировать(НаборЗначений);
	ДвиженияПоСписаннымТоварам(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварамРасход, Отказ, Заголовок);
...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам, Отказ, Заголовок)
	...
	Если СтруктураШапкиДокумента.ОтражатьВУправленческомУчете Тогда
		...
		Если НЕ Отказ Тогда
			...
			СтруктТаблицДокумента.Вставить("ТаблицаПоТоварам", ТаблицаПоТоварамРасход);
			...
			//-----------------------------------------------------------------------------------------------------------------------------------
			НаборЗначений = Новый Структура;
			НаборЗначений.Вставить("ВидДвижения", "Приход");
			ТаблицаПоТоварамПриход = ТаблицаПоТоварамОбщая.Скопировать(НаборЗначений);
			
			СтруктТаблицДокумента.Вставить("ТаблицаПоТоварам", ТаблицаПоТоварамПриход);
			ТаблицыДанныхДокумента = ОбщегоНазначения.ЗагрузитьТаблицыДокументаВСтруктуру(НаборДвижений, СтруктТаблицДокумента);
...
Процедура ОбработкаЗаполнения(Основание)
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ПеремещениеТоваров") Тогда
		...
		//По заданию С. Рангеловой 06.09.2023
		//данные могут попадать как в левую так и в правую часть.
		Режим = РежимДиалогаВопрос.ДаНет;
		Текст = "Данные должны быть помещены в колонку -С КОТОРОЙ- нажмите ДА
				|даннные поместить в колонки -ТРЕБУЕМОЕ- нажмите НЕТ";
		...
		Иначе
			...
			Для Каждого СтрокаТовары Из Товары Цикл
				СтрокаТовары.НоменклатураПосле 		= СтрокаТовары.Номенклатура;	
				СтрокаТовары.СерияНоменклатурыПосле = СтрокаТовары.СерияНоменклатуры;	
				СтрокаТовары.ЕдиницаИзмеренияПосле	= СтрокаТовары.ЕдиницаИзмерения;	
				СтрокаТовары.КоличествоПосле 		= СтрокаТовары.Количество;
				
				//заменим номенклатуру
				Номенклатура	=	Справочники.Номенклатура.НайтиПоРеквизиту("Артикул",Сред(СтрокаТовары.Номенклатура.Артикул,1,8));
				Если ЗначениеЗаполнено(Номенклатура) Тогда
					СтрокаТовары.Номенклатура=Номенклатура;
					//Ищем серию по остаткам
					Запрос	=	Новый Запрос;
					Запрос.Текст	=	"ВЫБРАТЬ
					            	 	...
					            	 	|	И ТоварыНаСкладахОстатки.КоличествоОстаток >= &Количество
					            	 	|
					            	 	|УПОРЯДОЧИТЬ ПО
					            	 	|	ТоварыНаСкладахОстатки.СерияНоменклатуры.СрокГодности";
					Запрос.УстановитьПараметр("Склад",Склад);					
					Запрос.УстановитьПараметр("Номенклатура",Номенклатура);					
					Запрос.УстановитьПараметр("Количество",СтрокаТовары.Количество);					
					Запрос.УстановитьПараметр("СрокГодности",СтрокаТовары.СерияНоменклатурыПосле.СрокГодности);
					Выборка	=	Запрос.Выполнить().Выбрать();
					Если Выборка.Следующий() Тогда
						СтрокаТовары.СерияНоменклатуры	=	Выборка.СерияНоменклатуры;	
					Иначе
						СтрокаТовары.СерияНоменклатуры	=	Справочники.СерииНоменклатуры.ПустаяСсылка();	
					КонецЕсли;	
				Иначе 
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//-----------------------------------------------------------------------------------------------------------------------
	//ДополнительныеСвойства.Вставить("СтруктураТабличныхЧастей", Новый Структура("ТаблицаПоТоварамРасход", ТаблицаПоТоварамРасход));
	//ДополнительныеСвойства.Вставить("СтруктураТабличныхЧастей", Новый Структура("ТаблицаПоТоварамПриход", ТаблицаПоТоварамПриход));
	
	ДополнительныеСвойства.Вставить("СтруктураТабличныхЧастей", Новый Структура("ТаблицаПоТоварамРасход, ТаблицаПоТоварамПриход", ТаблицаПоТоварамРасход,ТаблицаПоТоварамПриход));
...
Функция СформироватьЗапросПоТабличнойЧасти(ДокументОбъект, ИмяТабличнойЧасти, СтруктураПолей) 
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	Док.ПринятыеСчетУчетаБУ КАК ПринятыеСчетУчетаБУ,
	               |	""Расход"" КАК ВидДвижения
	               |ИЗ
	               |	Документ.Пересортица.Товары КАК Док
	               |ГДЕ
	               ...
	               |
	               |ВЫБРАТЬ
	               |	ПересортицаТовары.НомерСтроки,
	               |	ПересортицаТовары.НоменклатураПосле,
	               |	ПересортицаТовары.НоменклатураПосле.Услуга,
	               |	ПересортицаТовары.НоменклатураПосле.Набор,
	               |	ПересортицаТовары.НоменклатураПосле.Комплект,
	               |	ПересортицаТовары.КоличествоПосле * ПересортицаТовары.Коэффициент / ПересортицаТовары.НоменклатураПосле.ЕдиницаХраненияОстатков.Коэффициент,
	               |	ПересортицаТовары.Цена,
	               |	ПересортицаТовары.Сумма,
	               |	ПересортицаТовары.Качество,
	               |	ПересортицаТовары.ХарактеристикаНоменклатуры,
	               |	ПересортицаТовары.СерияНоменклатурыПосле,
	               |	ПересортицаТовары.Ссылка.Склад.ВидСклада,
	               |	ПересортицаТовары.СчетУчетаБУ,
	               |	ПересортицаТовары.СчетУчетаНУ,
	               |	ПересортицаТовары.ПринятыеСчетУчетаБУ,
	               |	""Приход""
	               |ИЗ
	               |	Документ.Пересортица.Товары КАК ПересортицаТовары
                   |ГДЕ
	               |	ПересортицаТовары.Ссылка = &ДокументСсылка";
