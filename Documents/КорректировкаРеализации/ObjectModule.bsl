...
Функция ПечатьУниверсальногоКорректировочногоДокумента()
	...
	НомерИсходногоДокумента = Номер;
	ДатаИсходногоДокумента	= Дата;
	Если ЗначениеЗаполнено(ИсправляемыйДокументРеализации) Тогда
		РеквизитыИсправляемогоДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			ИсправляемыйДокументРеализации, "Номер, Дата");
		НомерИсходногоДокумента 	= РеквизитыИсправляемогоДокумента.Номер;
		ДатаИсходногоДокумента	= РеквизитыИсправляемогоДокумента.Дата;
	КонецЕсли;
...
Процедура ДвиженияПоРегиструТоварыОрганизаций(РежимПроведения, СтруктураШапкиДокумента, 
	...
	//АБС ИЗМЕНЕНИЕ   17.07.2014 16:33:20  Балашенко
...
Процедура ПриЗаписи(Отказ)
	...
	ЭтотОбъект.Дата = ПолучитьДатуКорректировки();
	
КонецПроцедуры

// +Лаврухин
Функция ПолучитьДатуКорректировки()
	
	ДатаОснования = ЭтотОбъект.ИсправляемыйДокументРеализации.Ссылка.Дата;
	ДатаКорректировки = ЭтотОбъект.Ссылка.Дата; 
	
	Если Формат(ДатаКорректировки,"ДФ=dd.MM.yyyy") = Формат(ДатаОснования,"ДФ=dd.MM.yyyy") Тогда
		Если ДатаКорректировки <= ДатаОснования Тогда
			Если  ДатаОснования = КонецДня(ДатаОснования) Тогда
				ДатаКорректировки = ДатаОснования;
			Иначе
				ДатаКорректировки = ДатаОснования + 2*60;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;	
	
	Возврат ДатаКорректировки;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если СтруктураШапкиДокумента.КорректироватьБУиНУ Тогда
		...
		УправлениеВзаиморасчетами.ПодготовитьТаблицыДляПроведенияПоВзаиморасчетам(
			...
			Отказ,
			Заголовок);
		//{Лео Катанович 19.05.2016 
		////АБС ВСТАВКА   30.05.2014 12:23:25  Балашенко
		//Если абс_ДокументыРасчетовСКонтрагентом.Количество()>0 Тогда
		//	ТаблицаПоВзаиморасчетам = абс_ДокументыРасчетовСКонтрагентом.Выгрузить();
		//	ТаблицаПоВзаиморасчетам.Колонки.Добавить("СуммаУпр");
		//	Для каждого СтрокаТЗ из ТаблицаПоВзаиморасчетам Цикл
		//		СтрокаТЗ.СуммаУпр = СтрокаТЗ.СуммаВзаиморасчетов;
		//		Если СтрокаТЗ.ДокументРасчетовСКонтрагентом = Неопределено Тогда
		//			СтрокаТЗ.ДокументРасчетовСКонтрагентом = Ссылка;
		//		КонецЕсли;
		//	КонецЦикла;
		//КонецЕсли;
		////АБС ВСТАВКА  КОНЕЦ	
