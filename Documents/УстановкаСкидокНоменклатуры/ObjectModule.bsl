...
Процедура ПроверитьЗаполнениеШапки(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	//АБС ИЗМЕНЕНИЕ   08.04.2014 18:03:55  Балашенко
...
Процедура ПроверитьЗаполнениеТабличнойЧастиТовары(ТаблицаПоТоварам, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	// Укажем, что надо проверить.
	//АБС ИЗМЕНЕНИЕ   08.04.2014 18:22:01  Балашенко
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	//АБС ИЗМЕНЕНИЕ   08.04.2014 18:24:40  Балашенко
	...
	Если Не Отказ Тогда
		//АБС ИЗМЕНЕНИЕ   08.04.2014 17:32:51  Балашенко
...
//АБС ВСТАВКА   08.04.2014 15:35:44  Балашенко

Процедура ОбработкаЗаполнения(Основание, СтандартнаяОбработка)
	
	ВидСкидки = Перечисления.ВидыСкидок.Оптовая;
	Валюта = глЗначениеПеременной("ВалютаУправленческогоУчета");
	
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.Лео_ЗаявкаНаАкцию") Тогда
		
		ДополнительныеСвойства.Вставить("ОткрыватьФорму", Ложь);
		Если Основание.ВидКомпенсации  = "Автоматическая скидка" И Основание.Состояние = Перечисления.СостоянияОбъектов.Утвержден Тогда
			ДополнительныеСвойства.ОткрыватьФорму = Истина;
			/////////////////////////////////////////////
			Запрос = Новый Запрос;
			Запрос.УстановитьПараметр("ДокументОснование",	Основание.Ссылка);
			
			Запрос.Текст = "ВЫБРАТЬ
			|	абс_СкидкиНаценки.Регистратор
			|ИЗ
			|	РегистрСведений.абс_СкидкиНаценки КАК абс_СкидкиНаценки
			|ГДЕ
			|	абс_СкидкиНаценки.Регистратор.Лео_ДокументОснование = &ДокументОснование
			|
			|СГРУППИРОВАТЬ ПО
			|	абс_СкидкиНаценки.Регистратор";
			...
			Пока Выборка.Следующий() Цикл
				ДополнительныеСвойства.ОткрыватьФорму = Ложь;
				Предупреждение("Для ТМА № " + Строка(Основание.НомерТМА) +  " документ установки скидок уже существует!");
			Конеццикла;	   
		КонецЕсли;
		 
		 ДатаНачала = Основание.ДатаНачалаОтгрузки;
		 ДатаОкончания = Основание.ДатаОкончанияОтгрузки;
		 //Лео_ТипАкции = Справочники.Лео_ТипАкции.НайтиПоНаименованию("Акции и промо");
		 Лео_НомерАктивности = Основание.НомерТМА;
		 Лео_ДокументОснование =  Основание;
		 Комментарий =  Основание.Контрагент.Наименование + ", ТМА №" + Основание.НомерТМА + ", (" + Основание.Участник.Наименование + ")";
		 абс_НазначаетсяВручную = Истина;
		 //абс_ПрименятьСкидкуДоОграничения = Истина;
         абс_ДатаНачалаНакопления =  ДатаНачала; 
		 Лео_ТипАкции =  Основание.ТипАкции;
		 Если Лео_ТипАкции = Справочники.Лео_ТипАкции.НайтиПоНаименованию("Спец.цена") Тогда
			 Условие = Перечисления.УсловияСкидкиНаценки.абс_СпециальнаяЦена;
		 Иначе
			 Условие = Перечисления.УсловияСкидкиНаценки.абс_ПроцентомСОграничением;
		 КонецЕсли;	  

		 Лео_СтатьяОборотовПоБюджету =  Основание.СтатьяБюджета;
		 ТипСкидкиНаценки = Основание.ТипСкидкиНаценки;
		 
		 //Для Каждого Строка из Основание.ПланОтгрузкиТМА Цикл
		 Для Каждого Строка из Основание.Эффективность Цикл

			 НоваяСтрока =  Товары.Добавить();
			 НоваяСтрока.Номенклатура = Строка.Номенклатура;
			 НоваяСтрока.Лео_ЛимитШтук = Строка.Количество;
			 НоваяСтрока.Лео_ЛимитСумма = Строка.БюджетПоставщикаБезНДС;
			 НоваяСтрока.Качество = Справочники.Качество.Новый;
			 НоваяСтрока.Лео_Цена = Строка.Цена;
			 НайденнаяСтрока = Основание.УсловияФормированияСкидки.Найти(Строка.КлючСтроки, "КлючСтроки");
			 НоваяСтрока.ПроцентСкидкиНаценки = НайденнаяСтрока.СкидкаПоставщика;
		 КонецЦикла;
		 Если    Основание.ПолучателиСкидки.Количество() = 0 Тогда
		 СтрокаПолучатели = ПолучателиСкидки.Добавить();
		 СтрокаПолучатели.Контрагент = Основание.Контрагент;
	     Иначе
			 Для каждого Строка из Основание.ПолучателиСкидки Цикл
				 СтрокаПолучатели = ПолучателиСкидки.Добавить();
                 ЗаполнитьЗначенияСвойств(СтрокаПолучатели,Строка);
				 СтрокаПолучатели.Контрагент = Основание.Контрагент;
...
Процедура абс_ДвижениеПоРегистрам(Отказ)
	
	Движения.абс_СкидкиНаценки.Записывать = Истина;
	Движения.абс_СкидкиНаценки.Очистить();
	
	Если Условие = Перечисления.УсловияСкидкиНаценки.БезУсловий Тогда
		мОграничениеСкидкиНаценки = 0;
		мПрименятьСкидкуДоОграничения = Ложь;
		мПроцентСкидкиНаценки = ПроцентСкидкиНаценки;
		мСкидкаКоличеством = 0;
		мСкидкаСуммой = 0;
		мТипЦен = Справочники.ТипыЦенНоменклатуры.ПустаяСсылка();
		мУсловиеДляСкидкиКоличеством = 0;
		мДатаНачалаНакопления = Дата(1,1,1);
	ИначеЕсли Условие = Перечисления.УсловияСкидкиНаценки.абс_ПроцентомСОграничением Тогда
		мОграничениеСкидкиНаценки = ОграничениеСкидкиНаценки;
		мПрименятьСкидкуДоОграничения = абс_ПрименятьСкидкуДоОграничения;
		мПроцентСкидкиНаценки = ПроцентСкидкиНаценки;
		мСкидкаКоличеством = 0;
		мСкидкаСуммой = 0;
		мТипЦен = Справочники.ТипыЦенНоменклатуры.ПустаяСсылка();;
		мУсловиеДляСкидкиКоличеством = 0;
		мДатаНачалаНакопления = абс_ДатаНачалаНакопления;
        //{Лео Катанович
        Если мДатаНачалаНакопления = Дата(1,1,1) Тогда
             мДатаНачалаНакопления = НачалоДня(Дата);
		 КонецЕсли;	 
		//Лео Катанович}
	ИначеЕсли Условие = Перечисления.УсловияСкидкиНаценки.абс_Количеством Тогда
		мОграничениеСкидкиНаценки = 0;
		мПрименятьСкидкуДоОграничения = Ложь;
		мПроцентСкидкиНаценки = 0;
		мСкидкаКоличеством = абс_СкидкаКоличеством;
		мСкидкаСуммой = 0;
		мТипЦен = Справочники.ТипыЦенНоменклатуры.ПустаяСсылка();;
		мУсловиеДляСкидкиКоличеством = абс_УсловиеДляСкидкиКоличеством;
		мДатаНачалаНакопления = Дата(1,1,1);
	ИначеЕсли Условие = Перечисления.УсловияСкидкиНаценки.абс_СуммойНаДокумент Тогда
		мОграничениеСкидкиНаценки = 0;
		мПрименятьСкидкуДоОграничения = Ложь;
		мПроцентСкидкиНаценки = 0;
		мСкидкаКоличеством = 0;
		мСкидкаСуммой = абс_СкидкаСуммой;
		мТипЦен = Справочники.ТипыЦенНоменклатуры.ПустаяСсылка();;
		мУсловиеДляСкидкиКоличеством = 0;
		мДатаНачалаНакопления = Дата(1,1,1);
	ИначеЕсли Условие = Перечисления.УсловияСкидкиНаценки.абс_СпециальнаяЦена Тогда
		мОграничениеСкидкиНаценки = 0;
		мПрименятьСкидкуДоОграничения = Ложь;
		мПроцентСкидкиНаценки = 0;
		мСкидкаКоличеством = 0;
	...
	ТабКонтрагентов = ПолучателиСкидки.Выгрузить();
	ТабНоменклатуры = Товары.Выгрузить();
	
	Если ДляВсехПолучателей Тогда
		ТабКонтрагентов.Очистить();
		НоваяСтрока = ТабКонтрагентов.Добавить();
		НоваяСтрока.Контрагент = Справочники.Контрагенты.ПустаяСсылка();
	КонецЕсли;
	Если ДляВсейНоменклатуры ИЛИ Условие = Перечисления.УсловияСкидкиНаценки.абс_СуммойНаДокумент Тогда
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ТЗКонтрагенты.Контрагент КАК Контрагент,
	|	ТЗКонтрагенты.Грузополучатель
	|ПОМЕСТИТЬ ТаблицаКонтрагентов
	|ИЗ
	|	&ТабКонтрагентов КАК ТЗКонтрагенты
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТЗНоменклатура.Номенклатура,
	|	ТЗНоменклатура.Лео_ЛимитШтук КАК Лимит_Количество,
	|   ТЗНоменклатура.Лео_ЛимитСумма КАК Лимит_Сумма,
	|   ТЗНоменклатура.ПроцентСкидкиНаценки КАК ПроцентСкидкиНаценки
	|ПОМЕСТИТЬ ТаблицаНоменклатуры
	|ИЗ
	|	&ТабНоменклатуры КАК ТЗНоменклатура
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаНоменклатуры.Номенклатура,
	|	ТаблицаНоменклатуры.Лимит_Количество,
	|	ТаблицаНоменклатуры.Лимит_Сумма,
	|	ТаблицаНоменклатуры.ПроцентСкидкиНаценки,
	|	ТаблицаКонтрагентов.Контрагент,
	|	ТаблицаКонтрагентов.Грузополучатель
	|ИЗ
	|	ТаблицаКонтрагентов КАК ТаблицаКонтрагентов
	|		ПОЛНОЕ СОЕДИНЕНИЕ ТаблицаНоменклатуры КАК ТаблицаНоменклатуры
	|		ПО (ИСТИНА)";
	Запрос.УстановитьПараметр("ТабКонтрагентов",ТабКонтрагентов);
	Запрос.УстановитьПараметр("ТабНоменклатуры",ТабНоменклатуры);
//	Запрос.УстановитьПараметр("СкидкаСОграничением",Условие = Перечисления.УсловияСкидкиНаценки.абс_ПроцентомСОграничением);
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Движение = Движения.абс_СкидкиНаценки.Добавить();
		Движение.Период = ДатаНачала;
		Движение.Валюта = Валюта;
		Движение.Условие = Условие;
		Движение.Контрагент = Выборка.Контрагент;
		Движение.Получатель = Выборка.Грузополучатель;
		Движение.Номенклатура = Выборка.Номенклатура;
		Движение.Лимит_Количество = Выборка.Лимит_Количество;
		Движение.ДатаОкончания = ДатаОкончания;
		Движение.ОграничениеСкидкиНаценки = ?(Выборка.Лимит_Сумма=0, мОграничениеСкидкиНаценки, Выборка.Лимит_Сумма);
		Движение.ПрименятьСкидкуДоОграничения = мПрименятьСкидкуДоОграничения;
		Движение.ПроцентСкидкиНаценки = ?(Выборка.ПроцентСкидкиНаценки=0, мПроцентСкидкиНаценки, Выборка.ПроцентСкидкиНаценки);
		Движение.СкидкаКоличеством = мСкидкаКоличеством;
		Движение.СкидкаСуммой = мСкидкаСуммой;
		Движение.ТипЦен = мТипЦен;
		Движение.УсловиеДляСкидкиКоличеством = мУсловиеДляСкидкиКоличеством;
		Движение.ДатаНачалаНакопления = мДатаНачалаНакопления;
		Движение.НазначаетсяВручную = абс_НазначаетсяВручную;
	КонецЦикла;
