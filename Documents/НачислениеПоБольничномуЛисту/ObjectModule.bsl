...
Функция ПолучитьСтрокуЗаполняемыхРеквизитов()
	...
	СтрокаРеквизитов = "Организация,Сотрудник,ФизЛицо,ПервичныйБольничныйЛист,ДатаНарушенияРежима,"
			...
			+ "ПеречислятьПособиеПочтовымПереводом,Банк,НомерЛицевогоСчета,АдресПочтовый,АдресРегистрации,АдресПочтовыйКодПоКЛАДР,АдресРегистрацииКодПоКЛАДР,Фамилия,Имя,Отчество,Телефон,"
...
Функция ПечатьОборотаБольничного2011()
	...
	Выборка.Следующий();
	ЗаполнитьЗначенияСвойств(СекцияШапка.Параметры,Выборка);
	Запрос.УстановитьПараметр("ДоляММОТ",Выборка.Доля);
...
Функция ПечатьРасчетПособияКЛисткуНетрудоспособности2011()
	...
	ИначеЕсли ЭтоПособиеПоМатеринству() И (НовыйПорядокРасчетаПособияПоМатеринствуПриИсправлении Или ДатаСобытия() >= ПроведениеРасчетов.ДатаОтдельногоРасчетаПособийПоМатеринству() И ПериодРегистрации >= ПроведениеРасчетов.ДатаОтдельногоРасчетаПособийПоМатеринству()) Тогда
...
Функция ПечатьСправкаКЛисткуНетрудоспособности2007() 
	...
	Иначе
		ПечатьРасчетаСреднегоЗаработкаОтчет = ФормированиеПечатныхФормЗК.РасчетСреднегоЗаработкаПособий2011(ЭтотОбъект, Ложь, СтруктураПоказателей);
		Если Окр(СтруктураПоказателей.Средний, 2) * 730 > Выборка.ДоляМРОТ * 24 Тогда
			...
			ФормированиеПечатныхФормЗК.ВывестиДанныеПоБуквенно(СуммаДляПечатиПоБуквенно(Окр(СтруктураПоказателей.Средний, 2), 9), Макет, "СреднийДневнойЗаработок_", 9);
		Иначе
			ФормированиеПечатныхФормЗК.ВывестиДанныеПоБуквенно(СуммаДляПечатиПоБуквенно(Выборка.ДоляМРОТ * 24, 10), Макет, "СреднийЗаработок_", 10);
			ФормированиеПечатныхФормЗК.ВывестиДанныеПоБуквенно(СуммаДляПечатиПоБуквенно(Окр(Выборка.ДоляМРОТ * 24 / 730, 2), 9), Макет, "СреднийДневнойЗаработок_", 9);
		КонецЕсли;
...
Функция СтруктураДанныхДляПечатиЗаявлений()
	...
	|			ИНАЧЕ """"
	|		КОНЕЦ, НачислениеПоБольничномуЛисту.Уполномоченный.Наименование) КАК ФИОРуководителя,
...
Процедура РассчитатьНДФЛ(КомментироватьРасчет = Ложь, ПроводитьПроверки = Истина, ВыборкаПоШапкеДокумента = Неопределено) Экспорт
	...
	Запрос.УстановитьПараметр("Физлицо",						ВыборкаПоШапкеДокумента.Физлицо);
	Запрос.УстановитьПараметр("ДатаВыплатыДохода",				ВыборкаПоШапкеДокумента.ДатаВыплатыДохода);
	...
	|ИЗ
	|	&Начисления КАК ДанныеДокумента";
	Запрос.Выполнить();
	...
	|	ДанныеДокумента.НомерСтроки,
	|	ДанныеДокумента.ВидРасчета,
	|	ДанныеДокумента.ВидРасчета.КодДоходаНДФЛ КАК КодДохода,
	|	ДанныеДокумента.ДатаНачала,
	|	ДанныеДокумента.ДатаОкончания,
	|	ДанныеДокумента.Результат,
	|	ДанныеДокумента.Сотрудник,
	|	ДанныеДокумента.ПодразделениеОрганизации,
	|	ЕСТЬNULL(ДанныеДокумента.ПодразделениеОрганизации.Владелец, &ОбособленноеПодразделение) КАК ОбособленноеПодразделение,
	|	ДанныеДокумента.СторнируемыйДокумент,
	|	ДанныеДокумента.Сторно
	|ПОМЕСТИТЬ ВТСтрокиДокумента
	|ИЗ
	|	ВТДокумент КАК ДанныеДокумента
	|ГДЕ
	|	ДанныеДокумента.ВидРасчета.КодДоходаНДФЛ <> ЗНАЧЕНИЕ(Справочник.ДоходыНДФЛ.ПустаяСсылка)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ИЗ
	|	ВТСтрокиДокумента КАК ДанныеДокумента
	|
	|СГРУППИРОВАТЬ ПО
	|	ДанныеДокумента.ВидРасчета,
	|	ДанныеДокумента.КодДохода,
	...
	|	ДоходыСУточненнымиДатами.ОбособленноеПодразделение,
	|	ДоходыСУточненнымиДатами.Период,
	|	ДоходыСУточненнымиДатами.Физлицо,
	|	ДоходыСУточненнымиДатами.КодДохода,
	|	ДоходыСУточненнымиДатами.ПериодРегистрации,
	|	ДоходыСУточненнымиДатами.ПодразделениеОрганизации,
	|	СУММА(ДоходыСУточненнымиДатами.СуммаДохода) КАК СуммаДохода,
	|	0 КАК СуммаВычета,
	|	ЗНАЧЕНИЕ(Справочник.ВычетыНДФЛ.ПустаяСсылка) КАК КодВычета,
	|	ДоходыСУточненнымиДатами.ВидРасчета
	|ИЗ
	|	(ВЫБРАТЬ
	|		МАКСИМУМ(ВЫБОР
	|				КОГДА ДанныеДокумента.Сторно
	|						И НДФЛСведенияОДоходах.Период ЕСТЬ НЕ NULL 
	...
	|					ТОГДА КОНЕЦПЕРИОДА(ДанныеДокумента.ПериодРегистрации, МЕСЯЦ)
	...
	|		ДанныеДокумента.ОбособленноеПодразделение КАК ОбособленноеПодразделение,
	...
	|			ПО ДанныеДокумента.СторнируемыйДокумент = НДФЛСведенияОДоходах.Регистратор
	...
	|		ДанныеДокумента.ВидРасчета) КАК ДоходыСУточненнымиДатами
	|
	|СГРУППИРОВАТЬ ПО
	|	ДоходыСУточненнымиДатами.Период,
	|	ДоходыСУточненнымиДатами.Физлицо,
	...
	|	ДоходыСУточненнымиДатами.ПодразделениеОрганизации,
	|	ДоходыСУточненнымиДатами.ВидРасчета";
	ДоходыИзДокумента = Запрос.Выполнить().Выгрузить();
...
Функция СформироватьЗапросПоНачислениям(ВыборкаПоШапкеДокумента) Экспорт 
	...
	|	ВЫБОР
	|		КОГДА СтрокиНачисления.ВидРасчета.КодДоходаНДФЛ В (ЗНАЧЕНИЕ(Справочник.ДоходыНДФЛ.КодДоходаПоУмолчанию), ЗНАЧЕНИЕ(Справочник.ДоходыНДФЛ.Код2530))
