...
Функция ПечатьТТН()
	...
	//АБС ВСТАВКА   26.03.2014 12:23:03  Балашенко
	ОбработкаПечати.ТипЦен = ТипЦен;
	ОбработкаПечати.СрокДоставки = абс_СрокДоставки;
	ОбработкаПечати.МаркаАвтомобиля = абс_ТранспортноеСредство.Марка;
	ОбработкаПечати.МаркаПрицепа = абс_ТранспортноеСредство.МаркаПрицепа;
	ОбработкаПечати.ГосНомерАвтомобиля = абс_ТранспортноеСредство.ГосНомер;
	ОбработкаПечати.ГосНомерПрицепа = абс_ТранспортноеСредство.ГосНомерПрицепа;
	ОбработкаПечати.ПунктПогрузки = абс_ПунктПогрузки;
    ОбработкаПечати.ПунктРазгрузки = абс_ПунктРазгрузки;
    ОбработкаПечати.Водитель = абс_Водитель;
	ОбработкаПечати.Перевозчик = ?(абс_Перевозчик.НаименованиеПолное="",абс_Перевозчик.Наименование,абс_Перевозчик.НаименованиеПолное);
	ОбработкаПечати.Заказчик = ?(абс_Заказчик.НаименованиеПолное="",абс_Заказчик.Наименование,абс_Заказчик.НаименованиеПолное);
	ОбработкаПечати.ВидПеревозки = абс_ВидПеревозки;
	ОбработкаПечати.ЛицензионнаяКарточка = абс_ЛицензионнаяКарточка;
	ОбработкаПечати.ВодительскоеУдостоверение = абс_Водитель.абс_СерияВУ+" "+абс_Водитель.абс_НомерВУ;
    ОбработкаПечати.абс_НомерПломбы = абс_НомерПломбы;
	ОбработкаПечати.абс_КоличествоПалет = абс_КоличествоПалет;
	ОбработкаПечати.абс_ТранспортноеСредство = абс_ТранспортноеСредство;
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	//АБС ВСТАВКА 38854  13.02.2014 10:35:42  Шамов
	СтруктураМакетов.Вставить("М15",                "М-15 (Отпуск материалов на сторону)");
	СтруктураМакетов.Вставить("МХ18",                "MХ-18 (Передача готовой продукции в места хранения)");
...
Процедура ЗаполнитьПоСериям() Экспорт
	...
	//Лео + ZAA
	_ПромТЗ = ТаблицаСерий.Скопировать();
	_ПромТЗ.Свернуть("Номенклатура","Остаток");
	
	Для каждого тт из ВнутреннийЗаказ.Товары.Выгрузить() цикл
		//мСтрок = ТоварыТабличнойЧасти.НайтиСтроки(Новый Структура("Номенклатура,Количество",тт.Номенклатура,тт.Количество));
		мСтрок = _ПромТЗ.НайтиСтроки(Новый Структура("Номенклатура",тт.Номенклатура));
		Если мСтрок.Количество() = 0 тогда
			оо = ТоварыТабличнойЧасти.Добавить();    		 
			ЗаполнитьЗначенияСвойств(оо,тт);
			оо.Качество = Справочники.Качество.Новый;
			сообщить("Добалена номенклатура: " + сокрлп(тт.Номенклатура) + " без серии, в количестве: " + сокрлп(тт.Количество),СтатусСообщения.Внимание);
		иначе
			Если тт.Количество >  мСтрок[0].Остаток тогда
				оо = ТоварыТабличнойЧасти.Добавить();    		 
				ЗаполнитьЗначенияСвойств(оо,тт);
				оо.Качество = Справочники.Качество.Новый;
				оо.Количество = тт.Количество - мСтрок[0].Остаток;
				сообщить("Добалена номенклатура: " + сокрлп(тт.Номенклатура) + " без серии, в количестве: " + сокрлп(оо.Количество),СтатусСообщения.Внимание);
			КонецЕсли;	 
		КонецЕсли;	 
	КонецЦикла;
	//Лео - ZAA
	...
	Для Каждого ИсходнаяСтрока ИЗ ТоварыТабличнойЧасти Цикл
		...
		Для Каждого Строка Из НайденныеСтроки Цикл
			...
			//Проверка ОСГ
			Если ИсходнаяСтрока.Лео_МинПроцОСГ>0 Тогда
				СрокГодности	= (1-(НачалоДня(ТекущаяДата())-Строка.СерияНоменклатуры.абс_ДатаПроизводства)/(Строка.СерияНоменклатуры.СрокГодности-Строка.СерияНоменклатуры.абс_ДатаПроизводства))*100; 	
				Если СрокГодности<ИсходнаяСтрока.Лео_МинПроцОСГ Тогда
					Продолжить;
				КонецЕсли;
			КонецЕсли;
...
Процедура ОбработкаЗаполнения(Основание)
	...
	//АБС ВСТАВКА   02.04.2014 19:47:53  Балашенко
	абс_Статус = Перечисления.абс_СтатусыПеремещенийТоваров.Создан;
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.ВнутреннийЗаказ") Тогда
		...
		// {ZAA
		СкладОтправитель   = Основание.Лео_ИсходныйСклад;
		СкладПолучатель    = Основание.Заказчик;
	...
	//АБС ВСТАВКА Заполненим на основании выпуска продукции  04.06.2014 22:21:34  Фролов
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ВыпускПродукции") Тогда
		...
		Для Каждого СтрокаПродукция Из Основание.Продукция Цикл
			ЗаполнитьЗначенияСвойств(Товары.Добавить(), СтрокаПродукция, "Номенклатура,СерияНоменклатуры,КоличествоМест,ЕдиницаИзмеренияМест,Количество,ЕдиницаИзмерения,Коэффициент,Качество,Цена");
		КонецЦикла;
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ПоступлениеТоваровИзПереработки") Тогда
		...
		|	ПоступлениеТоваровИзПереработкиПродукция.СерияНоменклатуры,
		|	ПоступлениеТоваровИзПереработкиПродукция.КоличествоМест,
		|	ПоступлениеТоваровИзПереработкиПродукция.ЕдиницаИзмеренияМест,
		|	ПоступлениеТоваровИзПереработкиПродукция.Количество,
		|	ПоступлениеТоваровИзПереработкиПродукция.ЕдиницаИзмерения,
		|	ПоступлениеТоваровИзПереработкиПродукция.Коэффициент,
		|	ПоступлениеТоваровИзПереработкиПродукция.Качество,
		...
		|		ПО ПоступлениеТоваровИзПереработкиПродукция.Номенклатура = ТоварыНаСкладахОстатки.Номенклатура
		|			И ПоступлениеТоваровИзПереработкиПродукция.СерияНоменклатуры = ТоварыНаСкладахОстатки.СерияНоменклатуры
		|ГДЕ
		|	ПоступлениеТоваровИзПереработкиПродукция.Ссылка = &Ссылка";
		
		Запрос.УстановитьПараметр("Ссылка"	, Основание);
		Запрос.УстановитьПараметр("Склад"	, Основание.СкладОрдер);
		...
		//Для Каждого СтрокаПродукция Из Основание.Продукция Цикл
		//	ЗаполнитьЗначенияСвойств(Товары.Добавить(), СтрокаПродукция, "Номенклатура,СерияНоменклатуры,КоличествоМест,ЕдиницаИзмеренияМест,Количество,ЕдиницаИзмерения,Коэффициент,Качество");
		//КонецЦикла;	
	...
	//АБС ВСТАВКА Заполненим на основании выпуска продукции КОНЕЦ 	
	//абс Вставка 38757
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
		ЗаполнениеДокументов.ЗаполнитьШапкуДокументаПоОснованию(ЭтотОбъект, Основание);
		ВидОперации 					= Перечисления.ВидыОперацийПеремещениеТоваров.ТоварыПродукция;
		СкладОтправитель 				= Основание.ДокументРеализации.Склад;
		СкладПолучатель 				= Неопределено;
		ТипЦен							= Основание.ДокументРеализации.ТипЦен;

		//заполнение табличной части 
		Для Каждого ТекСтрокаТовары Из Основание.Товары Цикл
			Если  ТекСтрокаТовары.КоличествоДоИзменения = 0 или (ТекСтрокаТовары.КоличествоДоИзменения - ТекСтрокаТовары.Количество) = 0 Тогда
			      Продолжить;
			КонецЕсли;	  
			НоваяСтрока = Товары.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекСтрокаТовары);
			НоваяСтрока.Количество = ТекСтрокаТовары.КоличествоДоИзменения - ТекСтрокаТовары.Количество;
			ЗаполнитьСчетаУчетаВСтрокеТабЧастиРегл( НоваяСтрока, "Товары", ОтражатьВБухгалтерскомУчете, ОтражатьВНалоговомУчете);
			ОбработкаТабличныхЧастей.РассчитатьКоличествоМестТабЧасти(НоваяСтрока, ЭтотОбъект);
		КонецЦикла;
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
	   ...
	   Для Каждого ТекСтрокаТовары Из Основание[Основание.ПолучитьФорму().ЭлементыФормы.ОсновнаяПанель.ТекущаяСтраница.Имя] Цикл
		...
		//загрузим список коробов
		Если Основание.СписокКоробок.Количество()>0 Тогда
		СписокКоробок.Загрузить(Основание.СписокКоробок.Выгрузить());
		КонецЕсли;
	...
	//+Лео ZAA 
		...
		ВидОперации 					= Перечисления.ВидыОперацийПеремещениеТоваров.ТоварыПродукция;
		Организация 					= Основание.Организация;
		абс_ХозяйственнаяОперация       =  Справочники.абс_ХозяйственныеОперацииПУБ77.НайтиПоКоду("000000014");//Поступление товаров с производства
		
		Для Каждого СтрокаТовары Из Основание.Продукция Цикл
			пСтр = Товары.Добавить();
			ЗаполнитьЗначенияСвойств(пСтр, СтрокаТовары);
			пСтр.Качество = Справочники.Качество.Новый;
		КонецЦикла;	
	...
	//АБС ВСТАВКА   25.03.2014 11:53:32  Балашенко
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//{Катанович
	Если  глЗначениеПеременной("ПрофильПолномочийПользователя") = Справочники.ПрофилиПолномочийПользователей.НайтиПоНаименованию("Оператор производства") Тогда
		Отбор = Новый Структура;
		Отбор.Вставить("Объект",СкладОтправитель);
		Отбор.Вставить("Свойство", ПланыВидовХарактеристик.СвойстваОбъектов.НайтиПоНаименованию("Вид склада"));
		Структура = РегистрыСведений.ЗначенияСвойствОбъектов.Получить(Отбор);
		Если ЗначениеЗаполнено(Структура.Значение) Тогда
			Если Структура.Значение.Наименование <> "Производственный" Тогда
				Если Проведен = Истина Тогда 
			         Сообщить("Недостаточно прав");
			         Отказ = Истина;
			         Возврат;
                КонецЕсли;
				Сообщить("Возможна только запись документа");
		...
		Отбор = Новый Структура;
		Отбор.Вставить("Объект",СкладПолучатель);
		Отбор.Вставить("Свойство", ПланыВидовХарактеристик.СвойстваОбъектов.НайтиПоНаименованию("Вид склада"));
		Структура = РегистрыСведений.ЗначенияСвойствОбъектов.Получить(Отбор);
		Если ЗначениеЗаполнено(Структура.Значение) Тогда
			Если Структура.Значение.Наименование <> "Производственный" Тогда
	...
	//АБС ВСТАВКА   03.04.2014 10:12:15  Балашенко
	
	Если СокрЛП(абс_НомерЛЛ) = "" Тогда
		абс_НомерЛЛ = ОбщегоНазначения.ПолучитьНомерНаПечать(ЭтотОбъект);
	КонецЕсли;
