...
Процедура ДвиженияПоРегистрам(РежимПроведения, ТаблицаПоСоставПлана, Отказ, Заголовок)
	...
	//АБС ВСТАВКА   27.03.2014 12:31:54  Балашенко
	ТаблицаДвижений.ЗаполнитьЗначения(абс_ВидПланаПроизводства, "абс_ВидПланаПроизводства");	
	//АБС ВСТАВКА  КОНЕЦ
...
//АБС ВСТАВКА   27.03.2014 8:52:59  Балашенко
Процедура ОбработкаЗаполнения(ДанныеЗаполнения,СтандартнаяОбработка)
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПланПроизводства") Тогда
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, ДанныеЗаполнения);
		Дата = ТекущаяДата();
		Номер = "";
		абс_ВидПланаПроизводства = Перечисления.абс_ВидыПлановПроизводства.Корректировочный;
		абс_ОсновнойПланПроизводства = ДанныеЗаполнения;
		Ответственный = Неопределено;
		Комментарий = "";
		СуммаДокумента = 0;
		Для каждого СтрокаТЧ из ДанныеЗаполнения.СоставПлана Цикл
			НоваяСтрока = СоставПлана.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока,СтрокаТЧ);
			НоваяСтрока.Количество = 0;
			НоваяСтрока.Сумма = 0;
			НоваяСтрока.СуммаНДС = 0;
			НоваяСтрока.КоличествоИзИсточника = СтрокаТЧ.Количество;
		КонецЦикла;
	Иначе
		ДатаПланирования = ТекущаяДата();
		абс_ВидПланаПроизводства = Перечисления.абс_ВидыПлановПроизводства.Основной;
	КонецЕсли;
КонецПроцедуры
//АБС ВСТАВКА  КОНЕЦ
