...
Функция ПечатьПлатежногоПоручения() Экспорт
	...
	Обл.Параметры.НазначениеПлатежа       = ?(ЗначениеЗаполнено(ДоговорКонтрагента.Лео_ИГК), "Государственный контракт " + ДоговорКонтрагента.Лео_ИГК + Символы.ПС,"") + СокрЛП(НазначениеПлатежа);
	Если ЗначениеЗаполнено(ДоговорКонтрагента.Лео_ИГК) Тогда
	КонецЕсли;	
...
Процедура ПолучитьТаблицуПлатежейППУпр()
	...
	Иначе
		...
		Иначе
			...
			|	ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.ПустаяСсылка) КАК лео_СтатьяДвиженияДенежныхСредств,
			|	ЗНАЧЕНИЕ(Справочник.СтатьиОборотовПоБюджетам.ПустаяСсылка) КАК Лео_СОБР,
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Не Проведен И ВидОперации = Перечисления.ВидыОперацийППИсходящее.ВозвратДенежныхСредствПокупателю И Не абс_РучнойУчетВзаиморасчетов Тогда
		...
		Кратность = СтруктураКурсаДокумента.Кратность;
		
		ОбработкаОбъект=Обработки.ПодборПараметровРасшифровкиПлатежа.Создать();
		ОбработкаОбъект.КурсДокумента						= Курс;
		ОбработкаОбъект.КратностьДокумента					= Кратность;
		ОбработкаОбъект.ТипЗадолженности					= ">0";
		ОбработкаОбъект.Контрагент							= Контрагент;
		ОбработкаОбъект.ФормаОплаты							= Перечисления.ВидыДенежныхСредств.Безналичные;
		ОбработкаОбъект.ВидОперацииПлан						= УправлениеДенежнымиСредствами.ОпределитьВидОперацииПлан(ВидОперации);
		ОбработкаОбъект.БанковскийСчетКасса					= СчетОрганизации;
		ОбработкаОбъект.Организация							= Организация;
		ОбработкаОбъект.ВалютаДокумента						= ВалютаДокумента;
		ОбработкаОбъект.ДатаДок								= Дата;
		ОбработкаОбъект.ВидОперацииДок						= ВидОперации;
		ОбработкаОбъект.РасшифровкаПлатежаДок				= РасшифровкаПлатежа;
		ОбработкаОбъект.СуммаДляПодбора						= СуммаДокумента;
		ОбработкаОбъект.ПлатежПоСуммеВзаиморасчетов			= Ложь;
		ОбработкаОбъект.ЕстьПодбор							= Ложь;
		ОбработкаОбъект.УчитыватьФактическиеЗадолженности   = Истина;
		ОбработкаОбъект.УчитыватьОперативныеЗадолженности   = Ложь;
		ОбработкаОбъект.ПодбиратьСумму					    = Истина;
		ОбработкаОбъект.СпособЗаполнения		  		    = "ФИФО";
		ОбработкаОбъект.ПоЗаявкам							= Ложь;
		ОбработкаОбъект.ПодбиратьПланируемыеДвижения		= Ложь;
		
		ОбработкаОбъект.ЗаполнитьРасшифровкуПоДолгам(Ложь, Ложь);
