...
Процедура ДвиженияПоРегиструТоварыОрганизацийРегл(РежимПроведения, ТаблицаПоТоварам, ТаблицаПоТаре, ТаблицаПоОборудованию, Отказ, Заголовок, СтруктураШапкиДокумента)
	...
	//АБС ИЗМЕНЕНО Делаем движения по регистру ТоварыОрганизаций  27.03.2014 10:01:52  Фролов
...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, 
	...
	//АБС ВСТАВКА   24.03.2014 19:34:08  Балашенко
	Если ТипЗнч(абс_ДокументОснование) = Тип("ДокументСсылка.РеализацияТоваровУслуг") и 
		ТипЗнч(Сделка) = Тип("ДокументСсылка.ЗаказПокупателя") И
		СтруктураШапкиДокумента.ВидОперации <> Перечисления.ВидыОперацийПоступлениеТоваровУслуг.ВПереработку // {Лео Катанович}
		Тогда
		ТаблицаПоВзаиморасчетам.ЗаполнитьЗначения(Сделка, "Сделка");
		ТаблицаПоРасчетам.ЗаполнитьЗначения(Сделка, "Сделка");
	КонецЕсли;
	//АБС ВСТАВКА  КОНЕЦ
...
Процедура ОбработкаЗаполнения(Основание)
	...
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		...
		//АБС ВСТАВКА   24.03.2014 17:17:25  Балашенко
		//если реализация сформирована не обработкой ФормированиеПоступленийРеализаций, то считаем что это возврат, оформляемый через обратную продажу
		Если Не Основание.абс_СформированАвтоматически Тогда
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//АБС ИЗМЕНЕНИЕ   24.03.2014 17:53:12  Балашенко
	ПроводитьПоВзаиморасчетам = ((СтруктураШапкиДокумента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СПоставщиком или СтруктураШапкиДокумента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СПокупателем)
		И ВидОперации <> Перечисления.ВидыОперацийПоступлениеТоваровУслуг.ВПереработку);
	мСтруктураПараметровВзаиморасчетов.ИмяЗаказаВТабличныхЧастях = "ЗаказПокупателя";
