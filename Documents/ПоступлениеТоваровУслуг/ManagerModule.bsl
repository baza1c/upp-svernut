...
Функция ПечатьМ4(МассивОбъектов, ОбъектыПечати)
	...
	ТекстЗапросаШапка = "ВЫБРАТЬ
	                    ...
	                    |	ВЫБОР
	                    |		КОГДА ПоступлениеТоваровУслуг.ВидПоступления = &ВидПоступленияОрдер
	                    |			ТОГДА ПоступлениеТоваровУслуг.СкладОрдер.Склад
	                    |		ИНАЧЕ ПоступлениеТоваровУслуг.СкладОрдер
	                    |	КОНЕЦ КАК МестоПриемки,
	                    |	ВЫБОР
	                    |		КОГДА ПоступлениеТоваровУслуг.ВидПоступления = &ВидПоступленияОрдер
	                    |			ТОГДА ПоступлениеТоваровУслуг.СкладОрдер.Склад.Представление
	                    |		ИНАЧЕ ПоступлениеТоваровУслуг.СкладОрдер.Представление
	                    |	КОНЕЦ КАК СкладНаименование,
	                    |	ВЫБОР
	                    |		КОГДА ПоступлениеТоваровУслуг.ОтражатьВБухгалтерскомУчете
	                    |			ТОГДА ПоступлениеТоваровУслуг.ПодразделениеОрганизации
	                    |		ИНАЧЕ ПоступлениеТоваровУслуг.Подразделение
	                    |	КОНЕЦ КАК Подразделение,
	                    |	ВЫБОР
	                    |		КОГДА ПоступлениеТоваровУслуг.ОтражатьВБухгалтерскомУчете
	                    |			ТОГДА ПРЕДСТАВЛЕНИЕ(ПоступлениеТоваровУслуг.ПодразделениеОрганизации)
	                    |		ИНАЧЕ ПРЕДСТАВЛЕНИЕ(ПоступлениеТоваровУслуг.Подразделение)
	                    |	КОНЕЦ КАК ПредставлениеПодразделения,
