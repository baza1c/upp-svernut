...
Функция ПечатьТОРГ12()
	...
	СтруктураУполномоченноеЛицо = ПолучитьПодписьОтветственногоДляВозврата(Шапка.Руководители, Шапка.ДатаДокумента);
	Если СтруктураУполномоченноеЛицо <> Неопределено Тогда
			ОбластьПодвала.Параметры.ФИОРуководителя       = СтруктураУполномоченноеЛицо.ФИОПолное + Символы.ПС + СтруктураУполномоченноеЛицо.Документ;
	        ОбластьПодвала.Параметры.ДолжностьРуководителя = СтруктураУполномоченноеЛицо.Должность;
	КонецЕсли;	
...
Функция ПолучитьПодписьОтветственногоДляВозврата(Организация, ДатаСреза)
			...
			|	ОтветственныеЛицаОрганизацийСрезПоследних.Документ
			|ИЗ
			...
			|ГДЕ
			|	ОтветственныеЛицаОрганизацийСрезПоследних.ОтветственноеЛицо = ЗНАЧЕНИЕ(Перечисление.ОтветственныеЛицаОрганизаций.УполномоченныйПредставитель)";
			Выборка = ЗапросПоЛицам.Выполнить().Выбрать();
			Пока Выборка.Следующий() Цикл
				 
				 Возврат Новый Структура("ФИОПолное, Должность, Документ", Выборка.ФИОПолное, Выборка.Должность, Выборка.Документ);
