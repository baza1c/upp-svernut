...
Функция Файл2010(Отказ, СоответствиеКодовСтран, СоответствиеИменСтран, СоответствиеДокументовУдостоверяющиеЛичность, СоответствиеКодовРегионов, СоответствиеСтавок, СоответствиеКодовДоходов, СоответствиеКодовВычетов)
	...
	СтруктураДанныхНА = СправкиПоНДФЛ.ДанныеНалоговогоАгентаДля2НДФЛ(Организация, НалоговыйПериод, ОКАТО_КПП, КодИФНС, Дата, Телефон, ДолжностьПодписавшегоЛица, СправкуПодписал);
...
Процедура Автозаполнение() Экспорт
	...
	КонецЦикла;
	СтруктураДанных = СправкиПоНДФЛ.ДанныеОДоходах(СотрудникиОрганизации.Выгрузить(), НалоговыйПериод, Организация, ОбщегоНазначенияЗК.ГоловнаяОрганизация(Организация), ОКАТО_КПП, НачалоМесяца(Дата));
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если НалоговыйПериод = 2013 Тогда
		ОКТМО_КПП = СправкиПоНДФЛ.ПреобразоватьОКТМО_КППвОКАТО_КПП(ОКАТО_КПП, СправкиПоНДФЛ.СоответствиеОКАТОиОКТМО(Организация));
	КонецЕсли;
