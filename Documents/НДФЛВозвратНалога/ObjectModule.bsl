...
Процедура Рассчитать() Экспорт
	...
	Если Организация.ЮрФизЛицо <> Перечисления.ЮрФизЛицо.ФизЛицо Тогда
		...
		ПоляСтруктурыПоиска = "ДополнительноеПоле, ФизЛицо, КодПоОКАТО, КодПоОКТМО, КПП";
	Иначе
		НФДЛКВозврату = ПолучитьНДФЛКВозвратуФизЛицо(ФизЛица);
		ПоляСтруктурыПоиска = "ДополнительноеПоле, ФизЛицо, КодПоОКАТО, КодПоОКТМО";
	КонецЕсли;
...
Функция ПолучитьНДФЛКВозвратуЮрЛицо(ФизЛица = Неопределено)
	...
	|						СтрокиРегистра.ФизЛицо
	|					ИЗ
	|						ВТСтрокиРегистра КАК СтрокиРегистра)) КАК СтатусНаНачалоГода
	...
	|						СтрокиРегистра.ФизЛицо
	|					ИЗ
	|						ВТСтрокиРегистра КАК СтрокиРегистра)) КАК СтатусНаКонецПериода
	...
	|			ПО НДФЛРасчетыСБюджетом.ФизЛицо = ФизлицаПолучившиеСтатусРезидента.ФизЛицо
	|	ГДЕ
	|		ФизлицаПолучившиеСтатусРезидента.ФизЛицо ЕСТЬ NULL 
	|	
	|	СГРУППИРОВАТЬ ПО
	|		НДФЛРасчетыСБюджетом.НомерСтроки,
	...
	|		НДФЛРасчетыСБюджетом.КодПоОКТМО) КАК СальдоПоСтавкам
	|ГДЕ
