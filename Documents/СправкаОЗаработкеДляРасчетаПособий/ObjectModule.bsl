...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Если ИмяМакета = "Справка" Тогда
		ТабДокумент = ПечатьСправки(ИмяМакета);
		Возврат УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ОбщегоНазначенияЗК.СформироватьЗаголовокДокумента(ЭтотОбъект,Метаданные().Синоним) + " (расчет среднего заработка)",,ТабДокумент);
	ИначеЕсли ИмяМакета = "СправкаРасширенная" Тогда
		ТабДокумент = ПечатьСправки2013(ИмяМакета);
		Возврат УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ОбщегоНазначенияЗК.СформироватьЗаголовокДокумента(ЭтотОбъект,Метаданные().Синоним) + " (расчет среднего заработка)",,ТабДокумент);
	КонецЕсли;
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	СтруктураПечатныхФорм.Вставить("СправкаРасширенная", "Справка (2013 г.)");
	СтруктураПечатныхФорм.Вставить("Справка", "Справка (2011 г.)");
	...
	Иначе
		...
		Пока Н > 0 Цикл
			...
			Если ТекущаяЗапись.ИмеетПравоНаСтрахование = ПредшествующаяЗапись.ИмеетПравоНаСтрахование Тогда
				...
				ПредшествующаяЗапись.Период = ТекущаяЗапись.Период;
...
Процедура ЗаполнитьДниБолезниУходаЗаДетьми()
	...
	|	""Отпуск по уходу за ребенком"" Наименование,
...
//
Функция ПечатьСправки(ИмяМакета)
...
//	Табличный документ - печатная форма
//
Функция ПечатьСправки2013(ИмяМакета)
...
Процедура ПроверитьЗаполнениеСтрокиДанныеОДняхОтсутствий(ВыборкаПоСтрокамДокумента, Отказ, Заголовок) 
	...
	Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.НомерПовторнойСтроки) Тогда
		ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "указанный период отсутствия пересекается с периодом из строки " + ВыборкаПоСтрокамДокумента.НомерПовторнойСтроки + "!", Отказ, Заголовок);
	КонецЕсли;
