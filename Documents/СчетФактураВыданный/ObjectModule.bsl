...
Функция ПолучитьТаблицуАвансовПоСчетуНаОплату(Сумма, СуммаНДС, СтавкаНДС, СчетНаОплату) Экспорт
	...
	Если ЗначениеЗаполнено(СчетНаОплату) Тогда
		...
		Если Не ЗначениеЗаполнено(СтавкаНДС) Тогда
			...
			СтавкиНДС.Добавить(Перечисления.СтавкиНДС.НДС5);  
		...
		Иначе
			...
			СоответствиеСтавокИзСчета.Вставить(Перечисления.СтавкиНДС.НДС5_105, Перечисления.СтавкиНДС.НДС5);
			СоответствиеСтавокИзСчета.Вставить(Перечисления.СтавкиНДС.НДС7_107, Перечисления.СтавкиНДС.НДС7);
...
Процедура ОбработкаПроведения(Отказ)
	...
	//Если абс_СогласованиеДокументов.ИспользуетсяСогласованиеДокументов(Организация,Перечисления.абс_ТипыДокументовДляСогласования.ПоступлениеТоваровУслуг,Дата) Тогда
	...
	//				МаршрутСогласования = абс_СогласованиеДокументов.ОпределитьМаршрутСогласования(ПодразделениеДляОпределенияМаршрута,Перечисления.абс_ТипыДокументовДляСогласования.ПоступлениеТоваровУслуг);
...
// 
Функция ПодготовитьДанныеДляПечатиУниверсальногоКорректировочногоДокумента() Экспорт
	
	ДанныеДляПечати = ПодготовитьОбщиеДанныеДляПечатиКорректировочногоСчетаФактуры(Истина);
	Если ДанныеДляПечати = Неопределено Тогда
		Возврат ДанныеДляПечати;
	КонецЕсли;
	
	РеквизитыДоговора = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(ДоговорКонтрагента, "Наименование, Дата, Номер");
	ДанныеДляПечати.Вставить("Основание",      РеквизитыДоговора.Наименование);
	ДанныеДляПечати.Вставить("ОснованиеНомер", РеквизитыДоговора.Номер);
	ДанныеДляПечати.Вставить("ОснованиеДата",  РеквизитыДоговора.Дата);
...
// по всем документам-основаниям
// 
Функция ПодготовитьОбщиеДанныеДляПечатиКорректировочногоСчетаФактуры(ЭтоПечатьУКД = Ложь) Экспорт
	...
	ДанныеСчетаФактуры.Вставить("Продавец",                         Продавец);
	...
	Для каждого ТекущееОснование из СписокОснований Цикл
		...
		Если ТипОснования = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
			ДанныеДляПечати = СобратьДанныеДляПечатиКорректировочногоСчетаФактуры(ТекущееОснование, ДанныеСчетаФактуры);
		КонецЕсли;
	...
	Если ОбщиеДанныеДляПечати <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(Руководитель) Тогда
			Если ОбщиеДанныеДляПечати.Свойство("ФИОРуководителя") Тогда 
				Если НЕ ЗначениеЗаполнено(ОбщиеДанныеДляПечати.ФИОРуководителя) Тогда
		...
		Если ЗначениеЗаполнено(ГлавныйБухгалтер) И ОбщиеДанныеДляПечати.Свойство("ФИОГлавногоБухгалтера") Тогда
			Если НЕ ЗначениеЗаполнено(ОбщиеДанныеДляПечати.ФИОГлавногоБухгалтера) Тогда
			     ОбщиеДанныеДляПечати.ФИОГлавногоБухгалтера = ОбщегоНазначения.ФамилияИнициалыФизЛица(ГлавныйБухгалтер);
			КонецЕсли;
		КонецЕсли;
		
		ОбщиеДанныеДляПечати.Вставить("КППКонтрагента",  	КППКонтрагента);
...
// Функция собирает данные по документу-основанию Корректировка реализации и
// возвращает типизированную структуру с данными для печати корректировочного 
// счета-фактуры
// 
Функция СобратьДанныеДляПечатиКорректировочногоСчетаФактуры(ТекущееОснование, ДанныеСчетаФактуры)
	
	Перем ПодразделениеОтветственныхЛиц;
	
	ВерсияУчетаНДС = УчетНДС.ПолучитьВерсиюПостановления(ДанныеСчетаФактуры.Дата);
	...
	ЕдиницаИзмеренияШтукиКод = ЕдиницаИзмеренияШтуки.Код;
	
	Запрос.УстановитьПараметр("ЕдиницаИзмеренияШтуки", 	  ЕдиницаИзмеренияШтуки);
	Запрос.УстановитьПараметр("ЕдиницаИзмеренияШтукиКод", ЕдиницаИзмеренияШтукиКод);
	
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	КорректировкаРеализации.Организация,
	|	КорректировкаРеализации.Организация КАК Поставщик,
	|	КорректировкаРеализации.ДоговорКонтрагента.ВидДоговора КАК ВидДоговора,
	...
	//{Лео Катанович
	//|	КорректировкаРеализации.ОтветственныйЗаОформление,
	//|	КорректировкаРеализации.СопроводительныеДокументы
	|	КорректировкаРеализации.Ответственный КАК ОтветственныйЗаОформление,
	...
	Если ВерсияУчетаНДС = 2 Тогда
		Если ЗначениеЗаполнено(ДанныеСчетаФактуры.Продавец) И ДанныеСчетаФактуры.Продавец <> ДанныеСчетаФактуры.Контрагент Тогда
			Поставщик = ДанныеСчетаФактуры.Продавец;
		КонецЕсли;
	КонецЕсли;
	...
	//{Катанович
	СтруктураПодписи = Лео_ЗаполнениеРеквизитовДокументов.ПолучитьПодписьОтветственного( , ТекущееОснование);
	
	ДанныеДляПечати.Вставить(?(РегламентированнаяОтчетность.ЭтоПБОЮЛ(Шапка.Организация), "ФИОПБОЮЛ", "ФИОРуководителя"), СтруктураПодписи.РасшифровкаПодписиРук);
	ДанныеДляПечати.Вставить("ФИОГлавногоБухгалтера", СтруктураПодписи.РасшифровкаПодписиБух);
  ...
  //Катанович}
	...
	|	ВЫБОР
	|		КОГДА КорректировкаРеализацииТовары.Номенклатура ССЫЛКА Справочник.ОсновныеСредства
	|			ТОГДА &ЕдиницаИзмеренияШтуки
	|		ИНАЧЕ КорректировкаРеализацииТовары.Номенклатура.БазоваяЕдиницаИзмерения
	|	КОНЕЦ КАК НаименованиеЕдиницыИзмерения,
	|	ВЫБОР
	|		КОГДА КорректировкаРеализацииТовары.Номенклатура ССЫЛКА Справочник.ОсновныеСредства
	|			ТОГДА &ЕдиницаИзмеренияШтукиКод
	|		ИНАЧЕ КорректировкаРеализацииТовары.Номенклатура.БазоваяЕдиницаИзмерения.Код
	...
	|	ИсходныйДокумент.СуммаВключаетНДС КАК СуммаВключаетНДС,
	...
	|					ИНАЧЕ КорректировкаРеализацииУслуги.ЦенаДоИзменения
	|				КОНЕЦ
	|		КОНЕЦ КАК ЧИСЛО(15, 2)),
	|	ВЫБОР
	|		КОГДА ИсходныйДокумент.СуммаВключаетНДС
