...
Процедура ЗаполнитьДокумент() Экспорт
	...
	СуммаБезНДС5До = ИтогЗаПериод.СуммаБезНДС5;
	НДС5До = ИтогЗаПериод.НДС5;
    СуммаБезНДС7До = ИтогЗаПериод.СуммаБезНДС7;
	НДС7До = ИтогЗаПериод.НДС7;
...
Функция ПечатьДополнительногоЛистаКнигиПродаж()
	...
	ИтогЗаПериод.Вставить("СуммаБезНДС5", СуммаБезНДС5До);
	ИтогЗаПериод.Вставить("НДС5", НДС5До);
    ИтогЗаПериод.Вставить("СуммаБезНДС7", СуммаБезНДС7До);
	ИтогЗаПериод.Вставить("НДС7", НДС7До);
...
Процедура ЗаполнитьДаннымиИтого(Параметры, ДеревоВыгрузки)
	...
	КонецЕсли;   
		...
		// НДС5%.
	Если СуммаБезНДС5До <> 0 ИЛИ НДС5До <> 0 Тогда
		НовыйУзелВтЧисле = ПродублированныйУзелОбразец(ОбразецУзлаВтЧисле);
		ЗаполнитьДаннымиПоСтавкеНДС(НовыйУзелВтЧисле, "5", СуммаБезНДС5До, НДС5До);
		УдалятьОбразецУзлаВтЧисле = Истина;
	КонецЕсли;

		// НДС 7%.
	Если СуммаБезНДС7До <> 0 ИЛИ НДС7До <> 0 Тогда
		НовыйУзелВтЧисле = ПродублированныйУзелОбразец(ОбразецУзлаВтЧисле);
		ЗаполнитьДаннымиПоСтавкеНДС(НовыйУзелВтЧисле, "7", СуммаБезНДС7До, НДС7До);
		УдалятьОбразецУзлаВтЧисле = Истина;
	КонецЕсли;
...
Процедура ЗаполнитьДаннымиВсего(Параметры, ДеревоВыгрузки)
	...
	Если СуммаБезНДС5 <> 0 ИЛИ НДС5 <> 0 Тогда
		НовыйУзелВтЧисле = ПродублированныйУзелОбразец(ОбразецУзлаВтЧисле);
		ЗаполнитьДаннымиПоСтавкеНДС(НовыйУзелВтЧисле, "5", СуммаБезНДС5, НДС5);
		УдалятьОбразецУзлаВтЧисле = Истина;
	КонецЕсли;
	...
	Если СуммаБезНДС7 <> 0 ИЛИ НДС7 <> 0 Тогда
		НовыйУзелВтЧисле = ПродублированныйУзелОбразец(ОбразецУзлаВтЧисле);
		ЗаполнитьДаннымиПоСтавкеНДС(НовыйУзелВтЧисле, "7", СуммаБезНДС7, НДС7);
		УдалятьОбразецУзлаВтЧисле = Истина;
	КонецЕсли;
...
Процедура ЗаполнитьДаннымиТабличнойЧасти(Параметры, ДеревоВыгрузки)
	...
	Для Каждого СтрокаТабличнойЧасти Из ТабличнаяЧасть Цикл
		...
		Если СтрокаТабличнойЧасти.СуммаБезНДС5 <> 0 ИЛИ СтрокаТабличнойЧасти.НДС5 <> 0 Тогда
			НовыйУзелВтЧисле = ПродублированныйУзелОбразец(ОбразецУзлаВтЧисле);
			ЗаполнитьДаннымиПоСтавкеНДС(НовыйУзелВтЧисле, "5", СтрокаТабличнойЧасти.СуммаБезНДС5, СтрокаТабличнойЧасти.НДС5);
			УдалятьОбразецУзлаВтЧисле = Истина;
		КонецЕсли;
		...
		Если СтрокаТабличнойЧасти.СуммаБезНДС7 <> 0 ИЛИ СтрокаТабличнойЧасти.НДС7 <> 0 Тогда
			НовыйУзелВтЧисле = ПродублированныйУзелОбразец(ОбразецУзлаВтЧисле);
			ЗаполнитьДаннымиПоСтавкеНДС(НовыйУзелВтЧисле, "7", СтрокаТабличнойЧасти.СуммаБезНДС7, СтрокаТабличнойЧасти.НДС7);
			УдалятьОбразецУзлаВтЧисле = Истина;
		КонецЕсли;
