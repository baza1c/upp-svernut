...
Функция СформироватьЗапросПоИсполнители(Режим, ВыборкаПоШапкеДокумента)
	...
	Если ВыборкаПоШапкеДокумента.СпособВводаДанных = Перечисления.СпособыВводаДанныхОВремени.ВЦеломЗаПериод 
		...
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	СдельныйНарядВыработка.НомерСтроки,
		|	СдельныйНарядВыработка.Физлицо КАК Физлицо,
		|	СдельныйНарядВыработка.Количество КАК ОтработаноЧасов,
		|	СдельныйНарядВыработка.Сумма КАК СуммаКНачислению,
		|	СдельныйНарядВыработка.Количество,
		|	СдельныйНарядВыработка.ТехнологическаяОперация,
		|	СдельныйНарядВыработка.Ссылка.Подразделение,
		|	СдельныйНарядВыработка.Ссылка.Контрагент,
		|	СдельныйНарядВыработка.Ссылка.Организация
		|ИЗ
		...
		|
		|УПОРЯДОЧИТЬ ПО
		|	СдельныйНарядВыработка.Сотрудник";
		//
		//		"ВЫБРАТЬ
					...
					Если ВыборкаПоШапкеДокумента.СпособВводаДанных = Перечисления.СпособыВводаДанныхОВремени.ПоДням 
						...
						Для ИндексДня = 1 По КоличествоДнейВПериодеРегистрации Цикл
							Если МассивРаспределенияНачисленнойСуммыПоДням[ИндексДня-1] <> 0 Тогда
								...
								НоваяСтрока.ТехнологическаяОперация = ВыборкаПоРаботники.ТехнологическаяОперация;
								НоваяСтрока.Количество          = ВыборкаПоРаботники.Количество;
								НоваяСтрока.Организация         = ВыборкаПоРаботники.Организация;
								НоваяСтрока.Контрагент          = ВыборкаПоРаботники.Контрагент;
								НоваяСтрока.Подразделение       = ВыборкаПоРаботники.Подразделение;
...
Процедура ДобавитьСтрокуГрафикиРаботыПоВидамВремениЧасы(ВыборкаПоОтработанноеВремя, НаборЗаписей)
	...
	Если ВыборкаПоОтработанноеВремя.ВидВремени = Справочники.КлассификаторИспользованияРабочегоВремени.ПустаяСсылка() Тогда
		Движение.Документ = Документы.Лео_СдельныйНаряд.ПустаяСсылка();
	Иначе
			...
			Иначе
				Для Каждого Запись Из НаборЗаписей Цикл
					Если (Запись.Документ = Ссылка) Тогда
						...
						Запись.Документ = Документы.Лео_СдельныйНаряд.ПустаяСсылка();
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
			НаборЗаписей.Записать();
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	// регистр Лео_ФактическаяВыработкаРаботников 
	Движения.Лео_ФактическаяВыработкаРаботников.Очистить();
	Для Каждого ТекСтрокаВыработка Из Выработка Цикл
		Движение = Движения.Лео_ФактическаяВыработкаРаботников.Добавить();
		Движение.Период = Дата;
		Движение.Физлицо = ТекСтрокаВыработка.ФизЛицо;
		Движение.ТехнологическаяОперация = ТекСтрокаВыработка.ТехнологическаяОперация;
		Движение.Контрагент = Контрагент;
		Движение.Подразделение = Подразделение;
		Движение.Организация = Организация;
		Движение.Госконтракт = ТекСтрокаВыработка.Госконтракт;
		Движение.Количество = ТекСтрокаВыработка.Количество;
		Движение.СпособВводаДанных = СпособВводаДанных; 
		Движение.Выработка = ТекСтрокаВыработка.Сумма;
        Движение.Должность = ТекСтрокаВыработка.Должность;
	КонецЦикла;

	//}}__КОНСТРУКТОР_ДВИЖЕНИЙ_РЕГИСТРОВ
КонецПроцедуры

Процедура ОбработкаПроведения_Старый(Отказ, Режим)
	...
	Если ОтражатьВБухгалтерскомУчете И УчитыватьОтработанноеВремя И Не УчитыватьВремяТолькоПриРасчете Тогда
		...
		Если НЕ Отказ Тогда
				...
				Пока ВыборкаПоОтработанноеВремя.Следующий() Цикл
					...
					Для Сч = 1 По 5 Цикл
						...
						Если ЗначениеЗаполнено(ВыборкаПоОтработанноеВремя["ВидВремени" + Сч + "Сводно"]) Тогда
							...
							//катанович
							Запись.Контрагент					= ВыборкаПоОтработанноеВремя.Контрагент;
							Запись.Подразделение				= ВыборкаПоОтработанноеВремя.Подразделение;
			...
			Иначе
				...
				ПустойДокумент = Документы.Лео_СдельныйНаряд.ПустаяСсылка();
