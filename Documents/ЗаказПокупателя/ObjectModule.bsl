...
//АБС ВСТАВКА   24.03.2014 15:18:47  Балашенко
Перем мТекущийГрузополучатель Экспорт;
//АБС ВСТАВКА  КОНЕЦ
...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, 
	...
	Если Не Отказ Тогда
		...
		//АБС ВСТАВКА не делаем движения по резервам если заказ не согласован
		СписокСтатусовБезРезерва = Новый Массив();
		
		СписокСтатусовБезРезерва.Добавить(Перечисления.абс_СтатусыОтгрузки.Создан);
		СписокСтатусовБезРезерва.Добавить(Перечисления.абс_СтатусыОтгрузки.НеСогласован);
		СписокСтатусовБезРезерва.Добавить(Перечисления.абс_СтатусыОтгрузки.Аннулирован);
		
		Если НЕ СписокСтатусовБезРезерва.Найти(абс_Статус) = Неопределено Тогда
			Возврат;
		КонецЕсли;
		
		УчетПоСериям = абс_Дистрибуция.ПроверитьУчетПоСериям(Дата);
		
		Если УчетПоСериям Тогда
			Для Каждого СтрокаТовары Из ТаблицаПоТоварам Цикл
				Если НЕ ЗначениеЗаполнено(СтрокаТовары.СерияНоменклатуры) И 
					НЕ абс_Дистрибуция.ПолучитьДоступнуюРольИсполнителя(Справочники.РолиИсполнителей.абс_ЗаписьЗаказБезКонтроляПоСериям) Тогда
					   Если СтрокаТовары.Номенклатура.ВидНоменклатуры = Справочники.ВидыНоменклатуры.НайтиПоНаименованию("Конечная продукция") Тогда 
					        ОбщегоНазначения.СообщитьОбОшибке("Не зарезервирована серия номенклатуры в строке " + СтрокаТовары.НомерСтроки + " !", Отказ);
					   КонецЕсли;				
                КонецЕсли;										
			КонецЦикла;
		КонецЕсли;
		//АБС ВСТАВКА не делаем движения по резервам если заказ не согласован КОНЕЦ
		...
		Если ТаблицаПоТоварамРезервирование.Количество() > 0 ИЛИ ТаблицаПоТареРезервирование.Количество() > 0 Тогда
			...
			//АБС ВСТАВКА   22.07.2014 19:04:21  Балашенко
			ВыполнятьКонтрольОстатков = абс_Дистрибуция.ВыполнятьКонтрольОстатковПоЗаказу(Ссылка);
			//АБС ВСТАВКА  КОНЕЦ
			
			Если Товары.Количество() <> 0 И ВыполнятьКонтрольОстатков Тогда
				//АБС ВСТАВКА 37613
				//проверяем резервы при любом режиме проведения 			
				...
				//мРежимПроведения = РежимПроведенияДокумента.Оперативный;
				
				//в структуру шапки передаем реальный режим проведения, чтобы при оперативном режиме брать актуальные остатки, а не на дату документа как для неоперативного
				СтруктураШапкиДокумента.Вставить("РежимПроведения",РежимПроведения); 
				ОтказДо = Отказ;
				ПроцедурыКонтроляОстатков.ТоварыВРезервеНаСкладахКонтрольОстатков("Товары", СтруктураШапкиДокумента, Отказ, Заголовок, РежимПроведенияДокумента.Оперативный);
				Если абс_Статус = Перечисления.абс_СтатусыОтгрузки.Создан ИЛИ абс_Статус = Перечисления.абс_СтатусыОтгрузки.НеСогласован Тогда
					Если Не ОтказДо И Отказ Тогда
						Отказ = ОтказДо;
					КонецЕСли;	
				КонецЕСли;
				//АБС КОНЕЦ
...
Процедура ПриИзмененииДоговора() Экспорт
	...
	Для каждого СтрокаТчТовары из Товары Цикл
		абс_Дистрибуция.Лео_РасчитатьОптимальнуюЦенуСоСкидкой(СтрокаТчТовары);
	КонецЦикла;
	//-Лео Сафонов ЕА Поиск оптимальной цены с учетом скидки

	//АБС ИЗМЕНЕНИЕ 14.07.2014 Балашенко
	//УправлениеЗаказами.УстановитьДатуОплатыПоДоговору(ЭтотОбъект,"ДоговорКонтрагента",мТекущийДоговорКонтрагента);
	абс_Дистрибуция.УстановитьДатуОплаты(ЭтотОбъект,"ДоговорКонтрагента",мТекущийДоговорКонтрагента);
	//АСБ ИЗМЕНЕНИЕ КОНЕЦ
	РассчитатьСумму();
...
Процедура ПриИзмененииКонтрагента() Экспорт
	...
	//АБС ВСТАВКА   27.05.2014 17:00:41  Балашенко
	Если ЗначениеЗаполнено(Грузополучатель) 
		И ЗначениеЗаполнено(Грузополучатель.ГоловнойКонтрагент) 
		И Грузополучатель.ГоловнойКонтрагент<>Контрагент Тогда
		Грузополучатель = Неопределено;
		абс_ПриИзмененииГрузополучателя();
	Иначе
		абс_ЗаполнитьСпособДоставки();
	КонецЕсли;
	//АБС ВСТАВКА  КОНЕЦ
	
	//АБС ВСТАВКА Заполнение организации по контрагенту  04.06.2014 22:09:41  Фролов
	//Если НЕ ЗначениеЗаполнено(Организация) И ЗначениеЗаполнено(Контрагент.ОсновнойДоговорКонтрагента) Тогда
	Если ЗначениеЗаполнено(Контрагент.ОсновнойДоговорКонтрагента) Тогда
		Если Контрагент.ОсновнойДоговорКонтрагента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СПокупателем Тогда
			...
			Если ЗначениеЗаполнено(Организация.абс_ОсновнойСклад) Тогда
				СкладГруппа = Организация.абс_ОсновнойСклад;
				
				абс_УстановитьГрузоотправителя();
			КонецЕсли;
			
		КонецЕсли;
	КонецЕсли;
	//АБС ВСТАВКА Заполнение организации по контрагенту КОНЕЦ 
...
Процедура ОбработкаЗаполнения(Основание)
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.СчетНаОплатуПокупателю") Тогда
		...
		абс_Дистрибуция.УстановитьДатуОплаты(ЭтотОбъект);
		...
		КонецЦикла;
		//абс Вставка 38757
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.абс_ЗаявкаНаБюджетОтделаТМ") Тогда
		
		ЗаполнениеДокументов.ЗаполнитьШапкуДокументаПоОснованию(ЭтотОбъект, Основание);
		ЗаполнениеДокументов.ЗаполнитьШапкуДокумента(ЭтотОбъект);
		Подразделение = Основание.ЦФО;
		абс_ЗаявкаНаБюджетОтделаТМ = Основание;
		абс_СуммаРасходаБюджетаОтделаТМ = Основание.СуммаДокумента;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//АБС ВСТАВКА ПроверкаМораторнойЗадолженности  29.05.2014 17:47:59  Фролов
	абс_ПроверитьКонтрагентаНаНаличиеМораторнойЗадолженности(Отказ);	
	//АБС ВСТАВКА ПроверкаМораторнойЗадолженности КОНЕЦ 
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//АБС ВСТАВКА   31.03.2014 10:34:21  Балашенко
	Если абс_Статус = Перечисления.абс_СтатусыОтгрузки.Аннулирован Тогда
		ОбщегоНазначения.УдалитьДвиженияРегистратора(ЭтотОбъект, Отказ, Истина, РежимПроведения);
		абс_Дистрибуция.СформироватьДвиженияПоНеотгруженнымТоварам(ЭтотОбъект);
		Возврат;
	КонецЕсли;	
	//АБС ВСТАВКА  КОНЕЦ
	...
	//+Лео  Сафонов ЕА Продажной ценой является Лео_ЦенаСУчетомСкидки, данная цена идет в реализацию
	//СтруктураПолей.Вставить("Цена"                        , "Цена");
	СтруктураПолей.Вставить("Цена"                        , "Лео_ЦенаСУчетомСкидки");
    //-Лео  Сафонов ЕА Продажной ценой является Лео_ЦенаСУчетомСкидки, данная цена идет в реализацию
	...
	Если Ссылка.абс_Статус = Перечисления.абс_СтатусыОтгрузки.Отгружен ИЛИ Ссылка.абс_Статус = Перечисления.абс_СтатусыОтгрузки.Закрыт  Тогда
		абс_Дистрибуция.СформироватьДвиженияПоНеотгруженнымТоварам(Ссылка);
	КонецЕсли;

	//+Лео Сафонов Движения по регистру Лно_СкидкиТрейдМаркетинг
	КомпенсирующаяСкидкаСНДС = Товары.Итог("Лео_КомпенсирующаяСкидкаСНДС");
	ОстатокПоРегистру		 =	Лео_СкидкиТрейдМаркетингКонтрольОстатков();
	Если КомпенсирующаяСкидкаСНДС > 0 Тогда
		Если не КомпенсирующаяСкидкаСНДС > ОстатокПоРегистру Тогда
			Лео_ДвиженияПоСкидкиТрейдМаркетинг(КомпенсирующаяСкидкаСНДС, Отказ);
		Иначе
			Отказ = Истина;
			ОбщегоНазначения.СообщитьОбОшибке("Сумма компенсирующей скидки " + КомпенсирующаяСкидкаСНДС + " превышает остаток нераспределенных скидок по контрату!!!", Отказ);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриЗаписи(Отказ)
	...
	//АБС ВСТАВКА 38466  10.02.2014 17:25:49  Манжела
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		абс_СтатусыОтгрузок.ИзменитьСтатусОбъекта(ВыборкаДетальныеЗаписи.Ссылка, абс_Статус);
	КонецЦикла;
	//АБС ВСТАВКА 38466 КОНЕЦ
	
	//АБС ВСТАВКА 39056  18.02.2014 12:57:46  Шамов
	СтатусПоРегистру = абс_СтатусыОтгрузок.ПолучитьСтатусЗаказаПоРегистру(Ссылка);
	
	Если НЕ абс_Статус = СтатусПоРегистру Тогда
		ЗаписатьНовыйСтатус(абс_Статус);
	КонецЕсли;
	//АБС ВСТАВКА 39056 КОНЕЦ
	
	// + Лаврухин 17.03.2015
	// Принудительная блокировка Реализации по Заказам предоплатных Покупателей (только для условия 100% предоплата)	
	ЗаказПокупателя = ЭтотОбъект.Ссылка;
	СтатусДЗ = ЗапросСтатусаБлокировкиЗаказа(ЗаказПокупателя);
	ПринудительнаяБлокировка = ЗапросБлокировкиЗаказа(ЗаказПокупателя);
	ПредоплатныйЗаказ = ЗапросПредоплатностиЗаказа(ЗаказПокупателя);
	Если ПредоплатныйЗаказ = Ложь Тогда
		Если ЗаказПокупателя.абс_Статус <> Перечисления.абс_СтатусыОтгрузки.Создан Тогда 
			Если ЗаказПокупателя.абс_Статус <> Перечисления.абс_СтатусыОтгрузки.Согласован Тогда
				Если СтатусДЗ <> Перечисления.абс_СтатусыДЗПоЗаказам.Блокирован Тогда
					Если ЭтотОбъект.Ссылка.ДоговорКонтрагента.ПроцентПредоплаты > 1 Тогда
						Запрос = Новый Запрос;
						Запрос.Текст = 
						"ВЫБРАТЬ
						|	РеализацияТоваровУслуг.Ссылка
...
// + Лаврухин 17.03.2015
Функция ЗапросСтатусаБлокировкиЗаказа(Заказ)
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	абс_КонтрольДЗПоЗаказам.СтатусДЗ КАК СтатусДЗ	             
	               |ИЗ
...
Функция ЗапросБлокировкиЗаказа(Заказ)
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ	               
	               |	абс_КонтрольДЗПоЗаказам.ПринудительнаяБлокировка КАК ПринудительнаяБлокировка
	               |ИЗ
	               |	РегистрСведений.абс_КонтрольДЗПоЗаказам КАК абс_КонтрольДЗПоЗаказам
	               |ГДЕ
	               |	абс_КонтрольДЗПоЗаказам.ЗаказПокупателя = &Заказ";
	...
	//СтатусДЗ = Перечисления.абс_СтатусыДЗПоЗаказам.ПустаяСсылка();
	Для Каждого Строка Из Результат Цикл
		ПринудительнаяБлокировка = Строка.ПринудительнаяБлокировка;
	КонецЦикла;
	
	Возврат ПринудительнаяБлокировка;
	
КонецФункции

Функция ЗапросПредоплатностиЗаказа(Заказ)
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	абс_ЗаявкаНаБюджетОтделаТМ = Документы.абс_ЗаявкаНаБюджетОтделаТМ.ПустаяСсылка();
	абс_СуммаРасходаБюджетаОтделаТМ = 0;
	абс_СкидкиНаценки.Очистить();
	ЛеоСтатусОбменаСЛогоператором = Перечисления.Лео_СтатусыОбменаСЛогистическимОператором.НеПередавалсяЛогОператору;
	//// {Катанович
	//ОсновнойАдресДоставки = ПолучитьОсновнойАдресДоставки();
	//Если ОсновнойАдресДоставки <> Неопределено Тогда
	//	 АдресДоставки = ОсновнойАдресДоставки;
	//КонецЕсли;	
	// // Катанович}
	...
	Для каждого СтрокаТабличнойЧасти из Товары Цикл
		...
		СтрокаТабличнойЧасти.абс_СуммаРучнойСкидки = 0;
		СтрокаТабличнойЧасти.абс_КомментарийРучнойСкидки = "";
		СтрокаТабличнойЧасти.абс_СуммаАвтоматическойСкидки = 0;
		СтрокаТабличнойЧасти.Лео_КомпенсирующаяСкидка = 0;
		СтрокаТабличнойЧасти.Лео_КомпенсирующаяСкидкаСНДС = 0;
		СтрокаТабличнойЧасти.Лео_СуммаОкругляющейСкидки = 0;
		СтрокаТабличнойЧасти.Лео_ТипРучнойСкидки = справочники.ТипыСкидокНаценок.ПустаяСсылка();
		СтрокаТабличнойЧасти.Лео_ЦенаСУчетомСкидки = СтрокаТабличнойЧасти.Цена;
		СтрокаТабличнойЧасти.Лео_НачалоПериодаВозникновенияРасхода =  Дата("00010101000000"); 
		// Рассчитываем реквизиты табличной части.
		ОбработкаТабличныхЧастей.РассчитатьСуммуТабЧасти(СтрокаТабличнойЧасти, ЭтотОбъект);
		ОбработкаТабличныхЧастей.РассчитатьСуммуНДСТабЧасти(СтрокаТабличнойЧасти, ЭтотОбъект);
	КонецЦикла;
	//-Лео Сафонов ЕА
	//-лео Никитин Р.А.
	  абс_Неотгружено.Очистить();
	  абс_СкидкиНаценки.Очистить();
	//-лео Никитин Р.А.

КонецПроцедуры

//АБС ВСТАВКА 39056  18.02.2014 12:53:40  Шамов
Процедура ЗаписатьНовыйСтатус(НовыйСтатус) Экспорт
	...
	Запись.Период = абс_СерверныеФункции.ПолучитьДатуСервера() + 1;
...
//АБС ВСТАВКА скидки  09.04.2014 21:03:39  Балашенко
Процедура абс_ОчиститьСкидкиПоСтроке(СтрокаТабличнойЧасти) Экспорт
...
//АБС ВСТАВКА скидки КОНЕЦ

//АБС ВСТАВКА   27.05.2014 17:05:09  Балашенко
Процедура абс_ПриИзмененииГрузополучателя() Экспорт
	
	абс_ЗаполнитьСпособДоставки();
	
	//АБС ВСТАВКА Проставляем контрагента на основании грузополучателя  01.04.2014 19:57:12  Фролов
	Если ЗначениеЗаполнено(Грузополучатель.ГоловнойКонтрагент) И Не Грузополучатель.ГоловнойКонтрагент = Контрагент Тогда
		Контрагент = Грузополучатель.ГоловнойКонтрагент;
		ПриИзмененииКонтрагента();
	КонецЕсли;
	//АБС ВСТАВКА Проставляем контрагента на основании грузополучателя КОНЕЦ
	
	//АБС ВСТАВКА   11.04.2014 10:29:54  Балашенко
	абс_Дистрибуция.УстановитьДатуОтгрузкиПоДиспоСхеме(ЭтотОбъект, "Грузополучатель", мТекущийГрузополучатель);
	мТекущийГрузополучатель = Грузополучатель;
	//АБС ВСТАВКА  КОНЕЦ
	
КонецПроцедуры

Процедура абс_ЗаполнитьСпособДоставки()
	
	абс_СпособДоставкиКонтрагента = Грузополучатель.абс_ОсновнойСпособДоставки;
	
	Если Не ЗначениеЗаполнено(абс_СпособДоставкиКонтрагента) Тогда
		абс_СпособДоставкиКонтрагента = Контрагент.абс_ОсновнойСпособДоставки;
	КонецЕсли;
	
	АдресДоставки = абс_СпособДоставкиКонтрагента.АдресДоставки;
	
КонецПроцедуры
//АБС ВСТАВКА  КОНЕЦ

//АБС ВСТАВКА ПроверкаМораторнойЗадолженности  29.05.2014 17:33:05  Фролов
Процедура абс_ПроверитьКонтрагентаНаНаличиеМораторнойЗадолженности(Отказ)
	...
	Если Выборка.Следующий() Тогда
		...
		Если ЗначениеЗаполнено(Выборка.Значение) Тогда
			...
			ОбщегоНазначения.СообщитьОбОшибке("У контрагента """ + ЭтотОбъект.Контрагент + """ есть мораторная задолженность, оформление заказов по контрагенту запрещено!", Отказ);
...
//АБС ВСТАВКА   29.05.2014 18:49:04  Фролов
Процедура абс_УстановитьГрузоотправителя() Экспорт
	//{Катанович
	//Если (ЭтоНовый() И Не ЗначениеЗаполнено(Грузоотправитель)) или (Не ЗначениеЗаполнено(Грузоотправитель)) Тогда
		//Если ЗначениеЗаполнено(СкладГруппа) И ТипЗнч(СкладГруппа) = Тип("СправочникСсылка.Склады") Тогда
		  Если ЗначениеЗаполнено(Организация) И ((ЗначениеЗаполнено(СкладГруппа) И ТипЗнч(СкладГруппа) = Тип("СправочникСсылка.Склады"))) Тогда
	     //Катанович}
			//+Лео Сафонов ЕА 30.06.2016
			//Грузоотправитель = СкладГруппа.абс_Контрагент;
				Запрос = Новый Запрос;
				Запрос.Текст = 
					"ВЫБРАТЬ ПЕРВЫЕ 1
                ...
                Если ВыборкаДетальныеЗаписи.Количество()> 0  тогда
					...
					СсылкаГрузоотправитель = ВыборкаДетальныеЗаписи.Контрагент;
					//Катанович}
				Иначе
					//{Катанович
					//Грузоотправитель = СкладГруппа.абс_Контрагент;
					СсылкаГрузоотправитель = СкладГруппа.абс_Контрагент;
					 //Катанович}
				КонецЕсли;
				//{Катанович
				Если Грузоотправитель <> СсылкаГрузоотправитель Тогда
					 Грузоотправитель = СсылкаГрузоотправитель;
				КонецЕсли;	
				 //Катанович}
			//-Лео Сафонов ЕА 30.06.2016
		КонецЕсли;
...
//+Лео Сафонов ЕА
Функция Лео_СкидкиТрейдМаркетингКонтрольОстатков()
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	СУММА(Лно_СкидкиТрейдМаркетингОстатки.СкидкаОстаток) КАК СкидкаОстаток,
		|	Лно_СкидкиТрейдМаркетингОстатки.Контрагент
		|ИЗ
		|	РегистрНакопления.Лно_СкидкиТрейдМаркетинг.Остатки(
		|			&ДатаДокумента,
		|			Контрагент = &Контрагент
		|				И Организация = &Организация И ВидОплаты = &ВидОплаты) КАК Лно_СкидкиТрейдМаркетингОстатки
		|
		|СГРУППИРОВАТЬ ПО
		|	Лно_СкидкиТрейдМаркетингОстатки.Контрагент";
	...
	Запрос.УстановитьПараметр("Организация", Организация);
	//{Лео Катанович
	Запрос.УстановитьПараметр("ВидОплаты", Перечисления.абс_ВидыОплатУслугКонтрагентов.ПредоставлениеСкидки);
    //Лео Катанович}
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		ОстатокПоРегистру = ВыборкаДетальныеЗаписи.СкидкаОстаток
	КонецЦикла;
		
	Возврат ОстатокПоРегистру	
КонецФункции // )

Процедура Лео_ДвиженияПоСкидкиТрейдМаркетинг (КомпенсирующаяСкидкаСНДС, Отказ)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	Лно_СкидкиТрейдМаркетингОстатки.Контрагент,
		|	Лно_СкидкиТрейдМаркетингОстатки.Сеть,
		|	Лно_СкидкиТрейдМаркетингОстатки.Номенклатура,
		|	Лно_СкидкиТрейдМаркетингОстатки.НомерАктивности,
		|	Лно_СкидкиТрейдМаркетингОстатки.ВидАктивности,
		|	СУММА(Лно_СкидкиТрейдМаркетингОстатки.СкидкаОстаток) КАК СкидкаОстаток,
		|	Лно_СкидкиТрейдМаркетингОстатки.Организация,
		|	Лно_СкидкиТрейдМаркетингОстатки.ВидОплаты
		...
		|				И ВидОплаты = &ВидОплаты) КАК Лно_СкидкиТрейдМаркетингОстатки
		|
		|СГРУППИРОВАТЬ ПО
		|	Лно_СкидкиТрейдМаркетингОстатки.Контрагент,
		|	Лно_СкидкиТрейдМаркетингОстатки.Сеть,
		...
		|	Вт_Остатки.НомерАктивности,
		|	Вт_Остатки.ВидАктивности,
		|	СУММА(Вт_Остатки.СкидкаОстаток) КАК СкидкаОстаток,
		|	МинимальнаяДата.Период КАК Период,
		|	Вт_Остатки.Организация,
		|	Вт_Остатки.ВидОплаты
		|ИЗ
		|	Вт_Остатки КАК Вт_Остатки
		|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		|			МИНИМУМ(Лно_СкидкиТрейдМаркетинг.Период) КАК Период,
		|			Лно_СкидкиТрейдМаркетинг.Контрагент КАК Контрагент,
		|			Лно_СкидкиТрейдМаркетинг.Сеть КАК Сеть,
		|			Лно_СкидкиТрейдМаркетинг.Номенклатура КАК Номенклатура,
		|			Лно_СкидкиТрейдМаркетинг.НомерАктивности КАК НомерАктивности,
		|			Лно_СкидкиТрейдМаркетинг.ВидАктивности КАК ВидАктивности,
		|			Лно_СкидкиТрейдМаркетинг.Организация КАК Организация,
		|			Лно_СкидкиТрейдМаркетинг.ВидОплаты КАК ВидОплаты
		|		ИЗ
		|			Вт_Остатки КАК Вт_Остатки
		|				ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.Лно_СкидкиТрейдМаркетинг КАК Лно_СкидкиТрейдМаркетинг
		|				ПО Вт_Остатки.Контрагент = Лно_СкидкиТрейдМаркетинг.Контрагент
		|					И Вт_Остатки.Сеть = Лно_СкидкиТрейдМаркетинг.Сеть
		|					И Вт_Остатки.Номенклатура = Лно_СкидкиТрейдМаркетинг.Номенклатура
		|					И Вт_Остатки.НомерАктивности = Лно_СкидкиТрейдМаркетинг.НомерАктивности
		|					И Вт_Остатки.ВидАктивности = Лно_СкидкиТрейдМаркетинг.НомерАктивности
		|					И Вт_Остатки.ВидОплаты = Лно_СкидкиТрейдМаркетинг.ВидОплаты
		|		
		|		СГРУППИРОВАТЬ ПО
		|			Лно_СкидкиТрейдМаркетинг.Контрагент,
		|			Лно_СкидкиТрейдМаркетинг.Сеть,
		|			Лно_СкидкиТрейдМаркетинг.Номенклатура,
		|			Лно_СкидкиТрейдМаркетинг.НомерАктивности,
		|			Лно_СкидкиТрейдМаркетинг.ВидАктивности,
		|			Лно_СкидкиТрейдМаркетинг.Организация,
		|			Лно_СкидкиТрейдМаркетинг.ВидОплаты) КАК МинимальнаяДата
		|		ПО Вт_Остатки.Контрагент = МинимальнаяДата.Контрагент
		|			И Вт_Остатки.Сеть = МинимальнаяДата.Сеть
		|			И Вт_Остатки.Номенклатура = МинимальнаяДата.Номенклатура
		|			И Вт_Остатки.НомерАктивности = МинимальнаяДата.НомерАктивности
		|			И Вт_Остатки.ВидАктивности = МинимальнаяДата.ВидАктивности
		|			И Вт_Остатки.Организация = МинимальнаяДата.Организация
		|			И Вт_Остатки.ВидОплаты = МинимальнаяДата.ВидОплаты
		|
		|СГРУППИРОВАТЬ ПО
		|	Вт_Остатки.ВидАктивности,
		|	Вт_Остатки.НомерАктивности,
		|	Вт_Остатки.Номенклатура,
		|	МинимальнаяДата.Период,
		|	Вт_Остатки.Контрагент,
		|	Вт_Остатки.Сеть,
		|	Вт_Остатки.Организация,
		|	Вт_Остатки.ВидОплаты
	...
	ОсталосьРаспределить = КомпенсирующаяСкидкаСНДС;
	
	НаборДвижений = Движения.Лно_СкидкиТрейдМаркетинг;
	ТаблицаДвижений = НаборДвижений.Выгрузить();
	
	// Заполним таблицу движений.
	
	Для каждого СтрокаОстатки из ТзОстатки Цикл
		Если ОсталосьРаспределить > 0 Тогда
			СтрокаДвижений = ТаблицаДвижений.Добавить();
			СтрокаДвижений.Организация 		= СтрокаОстатки.Организация;
	        СтрокаДвижений.Контрагент 		= СтрокаОстатки.Контрагент;
			СтрокаДвижений.Сеть 			= СтрокаОстатки.Сеть;
			СтрокаДвижений.Номенклатура 	= СтрокаОстатки.Номенклатура;
			СтрокаДвижений.НомерАктивности 	= СтрокаОстатки.НомерАктивности;
			СтрокаДвижений.ВидАктивности	= СтрокаОстатки.ВидАктивности;
			СтрокаДвижений.ВидОплаты	=     СтрокаОстатки.ВидОплаты;

			Если НЕ ОсталосьРаспределить > СтрокаОстатки.СкидкаОстаток Тогда
				СтрокаДвижений.Скидка	= ОсталосьРаспределить;
				ОсталосьРаспределить = 0
			Иначе
				СтрокаДвижений.Скидка	= СтрокаОстатки.СкидкаОстаток;
				ОсталосьРаспределить = ОсталосьРаспределить - СтрокаОстатки.СкидкаОстаток;
			КонецЕсли;
		Иначе
			Прервать;
		КонецЕсли;
	...
	Если Не Отказ Тогда
		Движения.Лно_СкидкиТрейдМаркетинг.ВыполнитьРасход();
	КонецЕсли;
...
//{Катанович
 Функция ПолучитьОсновнойАдресДоставки()
	Запрос = Новый Запрос;
	Запрос.Текст = 
	      "ВЫБРАТЬ
	      |	абс_СпособыДоставкиКонтрагентов.АдресДоставки
	      |ИЗ
	      |	Справочник.абс_СпособыДоставкиКонтрагентов КАК абс_СпособыДоставкиКонтрагентов
	      |ГДЕ
	      |	абс_СпособыДоставкиКонтрагентов.Ссылка = &СпособДоставки";
          
Запрос.УстановитьПараметр("СпособДоставки", Контрагент.абс_ОсновнойСпособДоставки.Ссылка);
Выборка = Запрос.Выполнить().Выбрать();

Пока Выборка.Следующий() Цикл
	 АдресДоставкиСсылка = Выборка.АдресДоставки;
КонецЦикла;
Возврат АдресДоставкиСсылка;		  
