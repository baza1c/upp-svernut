...
Процедура ЗаполнитьТабличнуюЧастьПоЗаказуПокупателяУпр(ТабличнаяЧасть, ДокументОснование) Экспорт
	...
	//АБС ВСТАВКА   06.03.2014 16:05:40  Балашенко
	//для запроса заменим реальный склад на виртуальный
	СкладДляЗапроса=Склад;
	ТаблицаСоответствияСкладов = абс_Дистрибуция.ПолучитьТаблицуСоответствияРеальныхВиртуальныхСкладов();
	СтрокаСоответствия = ТаблицаСоответствияСкладов.Найти(Склад,"РеальныйСклад");
	Если СтрокаСоответствия<>Неопределено Тогда
		СкладДляЗапроса=СтрокаСоответствия.ВиртуальныйСклад; 
	КонецЕсли;
	//АБС ВСТАВКА  КОНЕЦ
	
	Запрос.УстановитьПараметр("Договор",      ДоговорКонтрагента);
	Запрос.УстановитьПараметр("Сделка",       Сделка);
	//АБС ИЗМЕНЕНИЕ   06.03.2014 16:06:12  Балашенко
		...
		Для Каждого СтрокаЗаказа Из СтрокиЗаказа Цикл
			...
			Для Сч = 1 По 2 Цикл
				...
				Для каждого СтрокаОстатков Из СтрокиОстатков Цикл
					...
					// АБС ИЗМЕНЕНИЕ Заполнение реализации по заказу Фролов
					Если СтрокаОстатков.ОстатокНаСкладе = 0 Тогда
						Продолжить;
					КонецЕсли;
					// АБС ИЗМЕНЕНИЕ Заполнение реализации по заказу Фролов КОНЕЦ
	...
	//абс вставка Балашенко 04.03.2014
	
	//заполним сертификаты в ТЧ Товары
	
	Если ТабличнаяЧасть=Товары Тогда
		ЗапросСертификаты=Новый Запрос;
		ЗапросСертификаты.Текст=
		"ВЫБРАТЬ
		|	ЗаказПокупателяТовары.абс_СертификатДекларацияСоответствия,
		|	ЗаказПокупателяТовары.абс_СертификатСоответствия,
		|	ЗаказПокупателяТовары.абс_СертификатСвидетельствоОГосрегистрации,
		|	ЗаказПокупателяТовары.абс_СертификатУдостоверениеКачества,
		|	ЗаказПокупателяТовары.Номенклатура,
		|	ЗаказПокупателяТовары.ХарактеристикаНоменклатуры,
		|	ЗаказПокупателяТовары.СерияНоменклатуры,
		|	ЗаказПокупателяТовары.абс_СертификатДекларацияТаможенногоСоюза
		|ИЗ
		|	Документ.ЗаказПокупателя.Товары КАК ЗаказПокупателяТовары
		|ГДЕ
		|	ЗаказПокупателяТовары.Ссылка = &Ссылка
		|	И (НЕ ЗаказПокупателяТовары.абс_СертификатДекларацияСоответствия = ЗНАЧЕНИЕ(Документ.абс_СертификатНаТовары.ПустаяСсылка)
		|			ИЛИ НЕ ЗаказПокупателяТовары.абс_СертификатСоответствия = ЗНАЧЕНИЕ(Документ.абс_СертификатНаТовары.ПустаяСсылка)
		|			ИЛИ НЕ ЗаказПокупателяТовары.абс_СертификатСвидетельствоОГосрегистрации = ЗНАЧЕНИЕ(Документ.абс_СертификатНаТовары.ПустаяСсылка)
		|			ИЛИ НЕ ЗаказПокупателяТовары.абс_СертификатУдостоверениеКачества = ЗНАЧЕНИЕ(Документ.абс_СертификатНаТовары.ПустаяСсылка)
		|			ИЛИ НЕ ЗаказПокупателяТовары.абс_СертификатДекларацияТаможенногоСоюза = ЗНАЧЕНИЕ(Документ.абс_СертификатНаТовары.ПустаяСсылка))";
		
		ЗапросСертификаты.УстановитьПараметр("Ссылка", ДокументОснование);
		
		Результат=ЗапросСертификаты.Выполнить();
		
		ВыборкаСертификаты=ЗапросСертификаты.Выполнить().Выбрать();
		Пока ВыборкаСертификаты.Следующий() Цикл
			ПараметрыОтбора=Новый Структура;
			ПараметрыОтбора.Вставить("Номенклатура",ВыборкаСертификаты.Номенклатура);
			ПараметрыОтбора.Вставить("ХарактеристикаНоменклатуры",ВыборкаСертификаты.ХарактеристикаНоменклатуры);
			ПараметрыОтбора.Вставить("СерияНоменклатуры",ВыборкаСертификаты.СерияНоменклатуры);
			МассивСтрок=ТабличнаяЧасть.НайтиСтроки(ПараметрыОтбора);
			Для каждого СтрокаТЧ ИЗ МассивСтрок Цикл
				ЗаполнитьЗначенияСвойств(СтрокаТЧ,ВыборкаСертификаты);
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
	
	//абс вставка Балашенко 04.03.2014 конец
	
	//АБС ВСТАВКА Балашенко   22.06.2014 22:10:13
	//Если количество в строке реализации равно количеству в строке заказа, заполним суммы из заказа
	Если ТабличнаяЧасть = Товары и ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
		Для Каждого СтрокаТЧ из Товары Цикл
			ПараметрыОтбора=Новый Структура;
			...
			ПараметрыОтбора.Вставить("СерияНоменклатуры",СтрокаТЧ.СерияНоменклатуры);
			...
			МассивСтрок = ДокументОснование.Товары.НайтиСтроки(ПараметрыОтбора);
			Если МассивСтрок.Количество()>0 Тогда
				ЗаполнитьЗначенияСвойств(СтрокаТЧ,МассивСтрок[0]);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	//АБС ВСТАВКА  КОНЕЦ
...
Процедура ПроверитьЗаполнениеШапкиУпр(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	//задание от Сан Саныча по заполнениюХозОперации
	Если ЭтотОбъект.Организация	= Справочники.Организации.НайтиПоКоду("О00000003") Тогда 
		Если НЕ ЭтотОбъект.абс_ХозяйственнаяОперация = Справочники.абс_ХозяйственныеОперацииПУБ77.НайтиПоКоду("000000123") Тогда
			ЭтотОбъект.абс_ХозяйственнаяОперация = лео_ДопФункции.ЗаполнитьХозяйственнуюОперациюПоДоговору(ЭтотОбъект);
		КонецЕсли;
	КонецЕсли;
...
Процедура ДвиженияПоРегистрам(РежимПроведения, СтруктураШапкиДокумента, 
	...
	//+Лео Сафонов ЕА
	Если абс_Статус = Перечисления.абс_СтатусыОтгрузки.Отгружен или
	абс_Статус = Перечисления.абс_СтатусыОтгрузки.ВПути или 
	абс_Статус = Перечисления.абс_СтатусыОтгрузки.Доставлен Тогда
	//-Лео Сафонов ЕА
	...
	//АБС ВСТАВКА   25.02.2014 16:21:47  Шамов
	абс_ДвиженияПоРегиструБюджетОтделаТМ(СтруктураШапкиДокумента, Отказ);
	//АБС ВСТАВКА  КОНЕЦ
...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, 
	...
	Если СтруктураШапкиДокумента.ОтражатьВУправленческомУчете Тогда
		...
		//Проведение только отгруженных документов	
		//+Лео Сафонов ЕА
		Если абс_Статус = Перечисления.абс_СтатусыОтгрузки.Отгружен или
		абс_Статус = Перечисления.абс_СтатусыОтгрузки.ВПути или 
		абс_Статус = Перечисления.абс_СтатусыОтгрузки.Доставлен Тогда
		...
		КонецЕсли;
	//Конец условия. Проведение только отгруженных документов	
	//+Лео Сафонов ЕА
	КонецЕсли;		
	//-Лео Сафонов ЕА

	//АБС ВСТАВКА Бюджетирование  28.10.2014 10:56:24  Тест
	ДвиженияПоРегиструЗаявкиНаРасходованиеСредств(ТаблицаПоВзаиморасчетам, Отказ);
	//АБС ВСТАВКА Бюджетирование КОНЕЦ 
	
	//+Лео Сафонов ЕА
	ДвиженияПоРегиструЛео_УникальныеИдентификаторПаллет( Отказ)
	//-Лео Сафонов ЕА
...
Процедура ДвиженияПоРегиструТоварыОрганизаций(РежимПроведения, СтруктураШапкиДокумента, 
	...
	//АБС ИЗМЕНЕНО Делаем движения по регистру ТоварыОрганизаций  27.03.2014 9:54:57  Фролов
	Если НЕ СтруктураШапкиДокумента.ОтражатьВРегламентированномУчете Тогда
		//Возврат;
	КонецЕсли;
	//АБС ИЗМЕНЕНО Делаем движения по регистру ТоварыОрганизаций КОНЕЦ
...
Процедура ОбработкаЗаполнения(Основание)
	...
	Если ТипОснования = Тип("ДокументСсылка.ЗаказПокупателя")
		...
		//	абс_Статус 	= Основание.абс_Статус;
		...
		Если ТипОснования = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
			...
			//+Лео Сафонов ЕА
			//абс_ЗаполнитьРеализациюПоЗаказуПокупателя(Сделка);
			абс_Статус 	= Основание.абс_Статус;
			Дата 		= Основание.ДатаОтгрузки;
			Если НЕ абс_Статус = перечисления.абс_СтатусыОтгрузки.КОтгрузке Тогда
				Сообщить("Невозможно сформировать реализацию по документу " + Основание + ". Заказ находится не в статусе ""К отгрузке"".");
				Возврат;
			КонецЕсли;
            абс_ЗаполнитьРеализациюПоЗаказуПокупателя(Сделка);
			//-Лео Сафонов ЕА
	...
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.ПеремещениеТоваров") Тогда
		...
		// заполним склад, берем склад получатель
		ЭтотОбъект.Склад	=	Основание.СкладПолучатель;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//ЛЕО Владимир
	//По организации ЛеоТрейд при выставлении услуг статут ставим отгружено.
	//Письмо Папашенко А.А.
	Если Услуги.Количество()>0 И Товары.Количество()=0 И Организация=Справочники.Организации.НайтиПоКоду("О00000005") Тогда
		абс_Статус=Перечисления.абс_СтатусыОтгрузки.Отгружен;		
	КонецЕсли;
	...
	//АБС ВСТАВКА Расчет итоговой суммы без НДС  01.04.2014 19:32:02  Фролов
	абс_СуммаСНДС = ПолучитьСуммуСНДС();
	абс_СуммаБезНДС = абс_СуммаСНДС - ПолучитьСуммуНДС();
	//АБС ВСТАВКА Расчет итоговой суммы без НДС КОНЕЦ	
	...
	//АБС ВСТАВКА Заполним номер уведомления об отгрузке
	Если ТипЗнч(Сделка) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
		
		ЗаполнятьИзНомераНакладной = абс_Дистрибуция.ПолучитьЗначениеСвойстваОбъекта(Контрагент, ПланыВидовХарактеристик.СвойстваОбъектов.абс_ЗаполнятьНомерУведомленияОбОтгрузкеИзНомераНакладнойEDI);
		Если ЗаполнятьИзНомераНакладной = Истина Тогда
			абс_НомерУведомленияОбОтгрузке 	= ОбщегоНазначения.ПолучитьНомерНаПечать(ЭтотОбъект);
		Иначе 
			абс_НомерУведомленияОбОтгрузке = Сделка.абс_НомерЗаказаEDI;
		КонецЕсли;
		абс_ДатаУведомленияОбОтгрузке   = Сделка.абс_ДатаПоставкиВТЦ;
	КонецЕсли;
	//АБС ВСТАВКА КОНЕЦ
	
	//АБС ВСТАВКА 02.07.2014 Балашенко
	Если Не Проведен И Не абс_РучнойУчетВзаиморасчетов Тогда
		СтруктураПараметров = мСтруктураПараметровВзаиморасчетов;
		ДокументыРасчетовСКонтрагентом.Очистить();
		УправлениеВзаиморасчетами.ДополнитьСтруктуруПараметровДляЗаполненияТаблицыДокументовРасчетов(ЭтотОбъект, СтруктураПараметров);
		УправлениеВзаиморасчетами.ЗаполнитьТаблицуДокументовРасчетовСКонтрагентом(ЭтотОбъект, СтруктураПараметров);
	КонецЕсли;
	//АБС ВСТАВКА КОНЕЦ
	
	//АБС ВСТАВКА Проверка заказ по стоп-листу 10.07.2014 Балашенко
	Если ЭтоНовый() и ТипЗнч(Сделка) = Тип("ДокументСсылка.ЗаказПокупателя") И ЗначениеЗаполнено(Сделка) Тогда		
		//ПредоплатныйЗаказ = ЗапросПредоплатностиЗаказа(Сделка);
		//Если ПредоплатныйЗаказ = Ложь Тогда
			абс_Дистрибуция.ПроверитьПоСтопЛисту(Сделка, Отказ);
		//КонецЕсли;
	КонецЕсли;
	//АБС ВСТАВКА КОНЕЦ   
	
	//Зароднюк В. требование Щербининой
...
//+Лаврухин
Функция ЗапросПредоплатностиЗаказа(Сделка)
...
Процедура ПодготовитьСтруктуруШапкиДокумента(Заголовок, СтруктураШапкиДокумента, РежимПроведения = Неопределено,Отказ=ложь) Экспорт
	...
	Если ЗначениеЗаполнено(Сделка) 
		...
		//АБС ВСТАВКА   25.02.2014 18:31:08  Шамов
		УправлениеЗапасами.ДобавитьСтрокуВДеревоПолейЗапросаПоШапке(ДеревоПолейЗапросаПоШапке, "Сделка"          , "абс_ЗаявкаНаБюджетОтделаТМ"                 , "абс_ЗаявкаНаБюджетОтделаТМ");
		УправлениеЗапасами.ДобавитьСтрокуВДеревоПолейЗапросаПоШапке(ДеревоПолейЗапросаПоШапке, "Сделка"          , "абс_СуммаРасходаБюджетаОтделаТМ"			, "абс_СуммаРасходаБюджетаОтделаТМ");
	Иначе
		СтруктураШапкиДокумента.Вставить("абс_ЗаявкаНаБюджетОтделаТМ", Документы.абс_ЗаявкаНаБюджетОтделаТМ.ПустаяСсылка());
		СтруктураШапкиДокумента.Вставить("абс_СуммаРасходаБюджетаОтделаТМ", 0);
		//АБС ВСТАВКА  КОНЕЦ
	КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//АБС ВСТАВКА 38466  10.02.2014 10:33:31  Манжела
	Если абс_Статус = Перечисления.абс_СтатусыОтгрузки.Создан Тогда
		ОбщегоНазначения.УдалитьДвиженияРегистратора(ЭтотОбъект, Отказ, Истина, РежимПроведения);
		Возврат;
	КонецЕсли;
	//АБС ВСТАВКА 38466 КОНЕЦ
	
	//АБС ВСТАВКА   31.03.2014 10:18:24  Балашенко
	Если абс_Статус = Перечисления.абс_СтатусыОтгрузки.Аннулирован Тогда
		ОбщегоНазначения.УдалитьДвиженияРегистратора(ЭтотОбъект, Отказ, Истина, РежимПроведения);
		Возврат;
	КонецЕсли;
	//АБС ВСТАВКА  КОНЕЦ	
	...
	//АБС ВСТАВКА Замечание 130  15.05.2014 14:08:12  Балашенко
	абс_Дистрибуция.ПроверитьЗаполнениеСкладаВРеализации(Ссылка, Отказ);
	//АБС ВСТАВКА Замечание 130 КОНЕЦ
	...
	ПроводитьПоВзаиморасчетам = СтруктураШапкиДокумента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СПокупателем
		...
		//АБС ВСТАВКА   25.02.2014 18:01:39  Шамов
		И ?(ЗначениеЗаполнено(СтруктураШапкиДокумента.абс_ЗаявкаНаБюджетОтделаТМ),СтруктураШапкиДокумента.абс_ЗаявкаНаБюджетОтделаТМ.Пустая(),Истина);  //Зароднюк ВВ 16.01.2024
		//И ?(НЕ СтруктураШапкиДокумента.абс_ЗаявкаНаБюджетОтделаТМ = NULL,СтруктураШапкиДокумента.абс_ЗаявкаНаБюджетОтделаТМ.Пустая(),Истина); // ## Жигалев 2022-09-05 ##
		//АБС ВСТАВКА  КОНЕЦ
	...
	//// ## Жигалев 2022-09-05 +
	//Если ТипЗнч(СтруктураШапкиДокумента.Сделка) = Тип("ДокументСсылка.ВозвратТоваровОтПокупателя") тогда
	//	Для каждого сс из Товары цикл
	//	сс.СпособСписанияОстаткаТоваров	= Перечисления.СпособыСписанияОстаткаТоваров.СоСклада;	
	//	КонецЦикла;		
	//КонецЕсли;	
	//// ## Жигалев 2022-09-05 -	
...
//АБС ВСТАВКА   25.02.2014 16:24:41  Шамов
Процедура абс_ДвиженияПоРегиструБюджетОтделаТМ(СтруктураШапкиДокумента, Отказ)
	
	ссЗаявка = СтруктураШапкиДокумента.абс_ЗаявкаНаБюджетОтделаТМ;
	чСумма = СтруктураШапкиДокумента.абс_СуммаРасходаБюджетаОтделаТМ;
	
	// ## Жигалев 2022-09-05 +
	Если ссЗаявка = null ИЛИ чСумма = null Тогда
		Возврат;
	КонецЕсли;
	// ## Жигалев 2022-09-05 -
	
	Если ссЗаявка.Пустая() ИЛИ чСумма = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	абс_БюджетОтделаТМОстатки.СуммаОстаток
		|ИЗ
		|	РегистрНакопления.абс_БюджетОтделаТМ.Остатки(&Период, ЗаявкаНаБюджетОтделаТМ = &ЗаявкаНаБюджетОтделаТМ) КАК абс_БюджетОтделаТМОстатки";

	Запрос.УстановитьПараметр("ЗаявкаНаБюджетОтделаТМ", ссЗаявка);
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Выборка.Следующий();
		бПревышение = чСумма > Выборка.СуммаОстаток;
	Иначе
		бПревышение = Истина;
	КонецЕсли;
	
	Если бПревышение Тогда
		ОбщегоНазначения.СообщитьОбОшибке("Превышение остатка по заявке на бюджет отдела ТМ!",,Отказ);
		Возврат;
	КонецЕсли;
	
	нзБюджет = Движения.абс_БюджетОтделаТМ;
	нзБюджет.Очистить();
	
	Движение = нзБюджет.ДобавитьРасход();
	Движение.Регистратор = Ссылка;
	Движение.Период = Дата;
	Движение.ЗаявкаНаБюджетОтделаТМ = ссЗаявка;
...
//АБС ВСТАВКА Заполнение документа по заказу Фролов
Процедура абс_ЗаполнитьРеализациюПоЗаказуПокупателя(ЗаказПокупателя = Неопределено) Экспорт

	Если НЕ ЗначениеЗаполнено(ЗаказПокупателя) Тогда
		ЗаказПокупателя = ЭтотОбъект.Сделка;
	КонецЕсли;
	
	Реализация = ЭтотОбъект;
	...
	//// Инициализируем документ, заполним шапку документа
	//Если ЗначениеЗаполнено(РеализацияСсылка) Тогда
	//	Реализация = РеализацияСсылка.ПолучитьОбъект();
	//Иначе
	//	Реализация      = Документы.РеализацияТоваровУслуг.СоздатьДокумент();
	//КонецЕсли;

	Реализация.Дата = ?(ЗначениеЗаполнено(ЗаказПокупателя.ДатаОтгрузки), ЗаказПокупателя.ДатаОтгрузки, ТекущаяДата());
	ЗаполнениеДокументов.ЗаполнитьШапкуДокумента(Реализация, "Продажа");
	...
	абс_Дистрибуция.УстановитьДатуОплатыРеализации(Реализация, Ложь);
		...
		Если Выборка.ИмяТабличнойЧасти = "Товары" Тогда
			...
			Для Каждого СтрокаТаблицыОстатков Из СтрокиТаблицыОстатков Цикл
				...
				Если Количество > 0 Тогда
					...
					РеализацияСклад = ЗаказПокупателя.СкладГруппа;
					...
					Если НЕ ЗначениеЗаполнено(СтрокаТабличнойЧасти.СтавкаНДС) Тогда
						ОбработкаТабличныхЧастей.ЗаполнитьСтавкуНДСТабЧасти(СтрокаТабличнойЧасти, Реализация);
					КонецЕсли;
	...
	Реализация.ЗаполнитьСчетаУчетаВТабЧасти(Реализация.Товары        , "Товары"        , ОтражатьВБУ, ОтражатьВНУ);
	Реализация.ЗаполнитьСчетаУчетаВТабЧасти(Реализация.ВозвратнаяТара, "ВозвратнаяТара", ОтражатьВБУ, ОтражатьВНУ);
	Реализация.ЗаполнитьСчетаУчетаВТабЧасти(Реализация.Услуги, 		   "Услуги", 		 ОтражатьВБУ, ОтражатьВНУ);
	Реализация.АдресДоставки = ЗаказПокупателя.АдресДоставки;
	Если ОтражатьВБУ Тогда
		СчетаУчета = БухгалтерскийУчетРасчетовСКонтрагентами.ПолучитьСчетаРасчетовСКонтрагентом(ЗаказПокупателя.Организация, ЗаказПокупателя.Контрагент, ЗаказПокупателя.ДоговорКонтрагента);
	...
	// Заполним единицы измерения мест из заказа покупателя
	ТабЗаказа = ЗаказПокупателя.Товары.Выгрузить();
	
	Для Каждого СтрТоварыРеализация Из Реализация.Товары Цикл
		
		СтруктураПоискаНоменклатура = Новый Структура("Номенклатура", СтрТоварыРеализация.Номенклатура);
		
		СтрЗаказаПоНоменклатуре = ТабЗаказа.НайтиСтроки(СтруктураПоискаНоменклатура);
		
		ТекСтрЗаказа = Неопределено;
		
		Если СтрЗаказаПоНоменклатуре.Количество() > 0 Тогда
			ТекСтрЗаказа = СтрЗаказаПоНоменклатуре[0];
		КонецЕсли;
		
		Если НЕ ТекСтрЗаказа = Неопределено Тогда
			СтрТоварыРеализация.ЕдиницаИзмеренияМест = ТекСтрЗаказа.ЕдиницаИзмеренияМест;
			Если ЗначениеЗаполнено(СтрТоварыРеализация.ЕдиницаИзмеренияМест.Коэффициент) Тогда
				СтрТоварыРеализация.КоличествоМест = СтрТоварыРеализация.Количество / СтрТоварыРеализация.ЕдиницаИзмеренияМест.Коэффициент;
			КонецЕсли;
		КонецЕсли;		
	КонецЦикла;
	...
	//АБС ВСТАВКА Заполнение сертификатов
	ОбработкаЗаполнения = Обработки.абс_ЗаполнитьСертификаты.Создать();
    ОбработкаЗаполнения.Инициализировать(Реализация, "Товары", Неопределено);		
	//АБС ВСТАВКА Заполнение сертификатов КОНЕЦ
	
	//АБС ВСТАВКА Балашенко   22.06.2014 22:10:13
	//Если количество в строке реализации равно количеству в строке заказа, заполним суммы из заказа
	Для Каждого СтрокаТЧ из Реализация.Товары Цикл
		...
		МассивСтрок = Реализация.Сделка.Товары.НайтиСтроки(ПараметрыОтбора);
		
		Если МассивСтрок.Количество()>0 Тогда
			ЗаполнитьЗначенияСвойств(СтрокаТЧ,МассивСтрок[0],,"Цена");
		КонецЕсли;
		
	КонецЦикла;
	//АБС ВСТАВКА  КОНЕЦ
	
	//АБС ВСТАВКА Упорядочим ТЧ по порядку из заказа
	
	ТЧТоварыЗаказа = Сделка.Товары.Выгрузить();
	ТЧТовары = Реализация.Товары.Выгрузить();
	
	ТЧТовары.Колонки.Добавить("абс_НомерСтрокиЗаказа", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(15,2));
	
	Для Каждого СтрТЧТовары Из ТЧТовары Цикл
		
		СтрокиЗаказа = ТЧТоварыЗаказа.НайтиСтроки(Новый Структура("Номенклатура,СерияНоменклатуры,Количество", СтрТЧТовары.Номенклатура, СтрТЧТовары.СерияНоменклатуры, СтрТЧТовары.Количество));
		...
		СтрокиЗаказа = ТЧТоварыЗаказа.НайтиСтроки(Новый Структура("Номенклатура,СерияНоменклатуры", СтрТЧТовары.Номенклатура, СтрТЧТовары.СерияНоменклатуры));
		...
		СтрокиЗаказа = ТЧТоварыЗаказа.НайтиСтроки(Новый Структура("Номенклатура", СтрТЧТовары.Номенклатура));
	...
	ТЧТовары.Сортировать("абс_НомерСтрокиЗаказа Возр");
	
	Реализация.Товары.Загрузить(ТЧТовары);
	
	Если ЗначениеЗаполнено(Реализация.ДоговорКонтрагента) Тогда
		Реализация.абс_ДатаОплаты = Реализация.Дата + 86400*Реализация.ДоговорКонтрагента.ДопустимоеЧислоДнейЗадолженности;
	КонецЕсли;
	Если  Деньнедели(Реализация.абс_ДатаОплаты)=6 Тогда
		 Реализация.абс_ДатаОплаты = НачалоДня(Реализация.абс_ДатаОплаты) + 86400 * 2;
	ИначеЕсли  Деньнедели(Реализация.абс_ДатаОплаты)=7 Тогда
		 Реализация.абс_ДатаОплаты = НачалоДня(Реализация.абс_ДатаОплаты) + 86400 * 1;
	КонецЕсли;
	...
	//лео Никитин
	Если ЗначениеЗаполнено(ЗаказПокупателя.комментарий) Тогда
		Реализация.комментарий = ЗаказПокупателя.комментарий;
	КонецЕсли;
    //
...
//+Лаврухин
Процедура АктуализироватьВзаиморасчетыСервер() Экспорт
    	
	Обработка = Справочники.абс_МодулиДляВыполнения.НайтиПоКоду("00000066");
	МодульОбъект = Обработка.ПолучитьОбъект(); 
    МодульОбъект.Параметры[0].ЗначениеПараметра = ЭтотОбъект.Контрагент.Ссылка;		
	МодульОбъект.Параметры[1].ЗначениеПараметра = ЭтотОбъект.Дата;		
...
//АБС ВСТАВКА Бюджетирование  28.10.2014 10:36:19  Тест
Процедура ДвиженияПоРегиструЗаявкиНаРасходованиеСредств(ТаблицаПоВзаиморасчетам, Отказ)
		
	Если НЕ ЗначениеЗаполнено(абс_ЗаявкаНаРасходованиеСредств) Тогда
		Возврат;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(абс_СуммаСкидкиПоЗРС) Тогда
		Возврат;
	КонецЕсли;
	
	ДвиженияЗРС = Движения.ЗаявкиНаРасходованиеСредств;
		
	СтрДвижениеЗРС = ДвиженияЗРС.Добавить();
	
	СтрДвижениеЗРС.Активность 					= Истина;
	СтрДвижениеЗРС.ВидДвижения 					= ВидДвиженияНакопления.Расход;
	СтрДвижениеЗРС.Период						= Дата;		
	
	СтрДвижениеЗРС.Организация					= Организация;
	СтрДвижениеЗРС.Контрагент					= Контрагент;
	СтрДвижениеЗРС.ДоговорКонтрагента   		= абс_ЗаявкаНаРасходованиеСредств.РасшифровкаПлатежа[0].ДоговорКонтрагента;

	СтрДвижениеЗРС.ЗаявкаНаРасходование			= абс_ЗаявкаНаРасходованиеСредств;

	СтрДвижениеЗРС.СуммаВзаиморасчетов			= абс_СуммаСкидкиПоЗРС * КурсВзаиморасчетов / КратностьВзаиморасчетов;
	СтрДвижениеЗРС.Сумма						= абс_СуммаСкидкиПоЗРС;
	СтрДвижениеЗРС.СуммаУпр						= абс_СуммаСкидкиПоЗРС;
	
	ДвиженияЗРС.Записывать = Истина;
	
КонецПроцедуры
//АБС ВСТАВКА Бюджетирование КОНЕЦ 

//+Лео Сафонов 28.06.2016
Процедура ДвиженияПоРегиструЛео_УникальныеИдентификаторПаллет(Отказ)
	
	Если Лео_Паллеты.Количество()= 0 Тогда
		Возврат;
	КонецЕсли;

	ДвиженияУИП = Движения.Лео_УникальныеИдентификаторПаллет;
	СписокПаллет = Лео_Паллеты.Выгрузить(,"УникальныйИндификаторПалеты");
	СписокПаллет.Свернуть("УникальныйИндификаторПалеты");

	Для Каждого СтрокаПалеты из СписокПаллет Цикл
		СтрДвиженияУИП = ДвиженияУИП.Добавить();
		СтрДвиженияУИП.УникальныйИдентификатор = СтрокаПалеты.УникальныйИндификаторПалеты;
		СтрДвиженияУИП.Период = Дата;
	КонецЦикла;
	
	ДвиженияУИП.Записывать = Истина;
