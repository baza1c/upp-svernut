...
Функция ПечатьАктаСверки() Экспорт
	...
	НазваниеОрганизацииСокр = Организация.Наименование;
	...
	//СтрЗаголовокТаблица = "Мы, нижеподписавшиеся, "
	//			+ "Директор департамента учета и отчетности - Главный бухгалтер"+" "
	//			+ НазваниеОрганизации 
	//			+ " " + "Папашенко Галина Сергеевна" + ", с одной стороны, "
	//			+ "и " + ?(ДолжностьПредставителяКонтрагента<>"",ДолжностьПредставителяКонтрагента,"___________________________") 
	//			+ " " + НаименованиеКонтрагента + " " 
	//			+ ?(НЕ ЗначениеЗаполнено(ПредставительКонтрагента),"_____________________________________________",ПредставительКонтрагента) + ", с другой стороны, "
	//			+ "составили настоящий акт сверки в том, что состояние взаимных расчетов по данным учета следующее:";
...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	ИначеЕсли ИмяМакета = "АктСверкиУПД" Тогда
		// Получить экземпляр документа на печать
		ТабДокумент = ПечатьАктаСверкиУПД();
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	СтруктПечатныхФорм.Вставить("АктСверки",    "Акт сверки");
	СтруктПечатныхФорм.Вставить("АктСверкиУПД", "Акт сверки по УПД");
	
	Возврат СтруктПечатныхФорм;
	
	//Возврат Новый Структура("АктСверки, АктСверкиУПД","Акт сверки, Акт сверки по УПД");
...
//+Лео Сафонов ЕА
Функция ПечатьАктаСверкиФаксимиль() Экспорт
	
	ТабДок = Новый ТабличныйДокумент;
	ТабДок.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_АктСверкиВзаиморасчетов_АктСверки";
	Макет  = ПолучитьМакет("Лео_АктСверкиФаксимиль");
	...
	ОбластьОборотыИтог  = Макет.ПолучитьОбласть("ОборотыИтог");
	ОбластьКонОстатки   = Макет.ПолучитьОбласть("КонОстатки");
	//+Выбор области с печатями в зависимости от пользователя и организации
	ТекПользователь = ПараметрыСеанса.ТекущийПользователь;
	ПечатьИсполнителя = Ложь;

	//Леотон и польователь Соотс Татьяна Александровна
	Если Организация = Справочники.Организации.НайтиПоКоду("О00000001") и ТекПользователь = Справочники.Пользователи.НайтиПоНаименованию("Соотс Татьяна Александровна") Тогда
		ОбластьПодвал       = Макет.ПолучитьОбласть("ПодвалЛеотонСоотс");
		
	//Леотон и польователь Жебрович А.В.
	ИначеЕсли Организация = Справочники.Организации.НайтиПоКоду("О00000001") и ТекПользователь = Справочники.Пользователи.НайтиПоНаименованию("Жебрович Анна Владимировна") Тогда
		ОбластьПодвал  = Макет.ПолучитьОбласть("ПодвалЛеотонЖебрович");	
		
		//Леотон и польователь Леонтьева О.В.
	ИначеЕсли Организация = Справочники.Организации.НайтиПоКоду("О00000001") и ТекПользователь = Справочники.Пользователи.НайтиПоНаименованию("Леонтьева Ольга Владимировна") Тогда
		ОбластьПодвал  = Макет.ПолучитьОбласть("ПодвалЛеотонЛеонтьева");	

	//Леотон Другие Пользователи	
	ИначеЕсли Организация = Справочники.Организации.НайтиПоКоду("О00000001") Тогда
		
		ОбластьПодвал = Макет.ПолучитьОбласть("ПодвалЛеотонСоотс");
		//ОбластьПодвал = Макет.ПолучитьОбласть("ПодвалЛеотонВетрова");

		ПечатьИсполнителя = Истина;
	//ООО «ЛЕОВИТ нутрио» 	
	ИначеЕсли Организация = Справочники.Организации.НайтиПоКоду("О00000003") Тогда
		ОбластьПодвал = Макет.ПолучитьОбласть("ПодвалЛеовит");
	//ООО «ЛефЭкспо»
	ИначеЕсли Организация = Справочники.Организации.НайтиПоКоду("О00000002") Тогда
		ОбластьПодвал = Макет.ПолучитьОбласть("Подвал");
	//ООО УК "ЭЙКОСЪ" 
	ИначеЕсли Организация = Справочники.Организации.НайтиПоКоду("О00000004") Тогда	
		ОбластьПодвал = Макет.ПолучитьОбласть("Подвал");
	ИначеЕсли Организация = Справочники.Организации.НайтиПоКоду("О00000005") Тогда	
		ОбластьПодвал = Макет.ПолучитьОбласть("ПодвалЛеотрейд");
	...
	Если ПечатьИсполнителя тогда
		ОбластьИсполнитель = Макет.ПолучитьОбласть("ПодвалИсполнитель");
		Если  ТекПользователь.абс_ДанныеПодписи = Справочники.абс_ПодписиОтветственныхЛиц.ПустаяСсылка() Тогда
			ОбластьИсполнитель.Параметры.Должность =  "";
		Иначе
       		ОбластьИсполнитель.Параметры.Должность   = ТекПользователь.абс_ДанныеПодписи.Должность;
		КонецЕсли;
		ОбластьИсполнитель.Параметры.Исполнитель = ОбщегоНазначения.ФамилияИнициалыФизЛица(ТекПользователь.ФизЛицо);
...
//+Лео Зароднюк
Функция ПечатьАктаСверкиУПД() Экспорт
	...
	Для Инд = 0 По МаксИндекс Цикл
		Если Инд < ПоДаннымОрганизации.Количество() Тогда
			...
			ИтСуммаОборотДт = ИтСуммаОборотДт + СуммаОборотДт;
			ИтСуммаОборотКт = ИтСуммаОборотКт + СуммаОборотКт;
			Если ТипЗнч(СтрокаОрганизации.ДокументРегистратор)=Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда  
				
				СчетФактура = УчетНДС.НайтиПодчиненныйСчетФактуру(СтрокаОрганизации.ДокументРегистратор);
                Если ЗначениеЗаполнено(СчетФактура) Тогда
					//ОбластьОбороты.Параметры.РегистраторПредставление = ""+ТипЗнч(СчетФактура.Ссылка)+" № "+СчетФактура.Ссылка.Номер+" от "+Формат(СчетФактура.Ссылка.Дата,"ЧЦ=21; ЧДЦ=2; ДФ=dd.MM.yyyy");
					ОбластьОбороты.Параметры.РегистраторПредставление = "УПД № "+СчетФактура.Ссылка.Номер+" от "+Формат(СчетФактура.Ссылка.Дата,"ЧЦ=21; ЧДЦ=2; ДФ=dd.MM.yyyy");
					ОбластьОбороты.Параметры.Регистратор = СтрокаОрганизации.ДокументРегистратор;
					ОбластьОбороты.Параметры.ДатаДокумента = Формат(СчетФактура.Дата,"ЧЦ=21; ЧДЦ=2; ДФ=dd.MM.yy");
				Иначе
					ОбластьОбороты.Параметры.РегистраторПредставление = СтрокаОрганизации.Представление;
					ОбластьОбороты.Параметры.Регистратор = СтрокаОрганизации.ДокументРегистратор;
					ОбластьОбороты.Параметры.ДатаДокумента = Формат(СтрокаОрганизации.Дата,"ЧЦ=21; ЧДЦ=2; ДФ=dd.MM.yy"); 
