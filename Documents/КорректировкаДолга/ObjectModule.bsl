...
Процедура ЗаполнитьОстаткамиДебиторскойЗадолженностиУпр() Экспорт
	...
	Запрос.Текст = 
	 ...
	 |	И ВзаиморасчетыПоДокументам.ДокументРасчетовСКонтрагентом.Дата <= &ДатаДокумента
	 |
	 |ИНДЕКСИРОВАТЬ ПО
	 |	ДоговорКонтрагента,
	 |	Сделка
	...
	//Лео+ЕНикитин
	Если Выборка.ДокументРасчетовСКонтрагентом <> Неопределено  Тогда
		НоваяСтрока.Лео_ДатаДокументаРасчетов = Выборка.ДокументРасчетовСКонтрагентом.Дата; 
	КонецЕсли;	
        //

	КонецЦикла;
	   СуммыДолга.Сортировать("Лео_ДатаДокументаРасчетов Возр");  
...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоСуммамДолга, Отказ, Заголовок)
	...
	Если СтруктураШапкиДокумента.ОтражатьВУправленческомУчете Тогда
		...
		//АБС ВСТАВКА Бюджетирование  28.10.2014 10:35:59  Тест
		ДвиженияПоРегиструЗаявкиНаРасходованиеСредств(ТаблицаУменьшениеДолга, Отказ);
...
Процедура ОбработкаЗаполнения(Основание)
	...
	//АБС ВСТАВКА Бюджетирование  28.10.2014 10:12:33  Тест
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ЗаявкаНаРасходованиеСредств") Тогда
		
		абс_ЗаявкаНаРасходованиеСредств = Основание;
		ДокументОснование 				= Основание;
		
		
		Дата 							= Основание.ДатаРасхода;
		Организация 					= Основание.Организация;
		ВидОперации 					= Перечисления.ВидыОперацийКорректировкаДолга.ПроведениеВзаимозачета;
		...
		КурсДокумента					= Основание.КурсДокумента;
		КратностьДокумента				= Основание.КратностьДокумента;
				
		Ответственный = ПараметрыСеанса.ТекущийПользователь;
		
		Для Каждого СтрРасшифровкаПлатежа Из Основание.РасшифровкаПлатежа Цикл
			
			СтрСуммыДолга = СуммыДолга.Добавить();
			
			СтрСуммыДолга.ВидЗадолженности 				= Перечисления.ВидыЗадолженности.Кредиторская;
			СтрСуммыДолга.ДоговорКонтрагента 			= СтрРасшифровкаПлатежа.ДоговорКонтрагента;
			СтрСуммыДолга.Сделка			 			= СтрРасшифровкаПлатежа.Сделка;
			СтрСуммыДолга.ДокументРасчетовСКонтрагентом	= СтрРасшифровкаПлатежа.ДокументРасчетовСКонтрагентом;
			
			СтрСуммыДолга.КратностьВзаиморасчетов 		= СтрРасшифровкаПлатежа.КратностьВзаиморасчетов;
			СтрСуммыДолга.КурсВзаиморасчетов			= СтрРасшифровкаПлатежа.КурсВзаиморасчетов;
			СтрСуммыДолга.Сумма 						= СтрРасшифровкаПлатежа.СуммаПлатежа;		
...
Процедура ДвиженияПоРегиструЗаявкиНаРасходованиеСредств(ТаблицаУменьшениеДолга, Отказ)
	
	Если НЕ ВидОперации = Перечисления.ВидыОперацийКорректировкаДолга.ПроведениеВзаимозачета Тогда
		Возврат;
	КонецЕсли;
	
	//Если НЕ ЗначениеЗаполнено(абс_ЗаявкаНаРасходованиеСредств) Тогда
	//	
	//	ОбщегоНазначения.СообщитьОбОшибке("Не выбрана заявка на расходование средств!");
	//	
	//	Возврат;		
	//	
	//КонецЕсли;
	...
	Для Каждого СтрУменьшениеДолга Из ТаблицаУменьшениеДолга Цикл
		...
		СтрДвижениеЗРС.Организация					= СтрУменьшениеДолга.Организация;
		СтрДвижениеЗРС.Контрагент					= СтрУменьшениеДолга.Контрагент;
		СтрДвижениеЗРС.ДоговорКонтрагента   		= СтрУменьшениеДолга.ДоговорКонтрагента;
		СтрДвижениеЗРС.ДокументРасчетовСКонтрагентом= СтрУменьшениеДолга.ДокументРасчетовСКонтрагентом;		
		СтрДвижениеЗРС.ЗаявкаНаРасходование			= абс_ЗаявкаНаРасходованиеСредств;

		СтрДвижениеЗРС.СуммаВзаиморасчетов			= СтрУменьшениеДолга.СуммаВзаиморасчетов;
		СтрДвижениеЗРС.Сумма						= СтрУменьшениеДолга.Сумма;
		СтрДвижениеЗРС.СуммаУпр						= СтрУменьшениеДолга.СуммаУпр;
