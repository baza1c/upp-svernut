...
Процедура ЗаполнитьДокумент() Экспорт
	...
	СтруктураПараметров.Вставить("Период",		НалоговыйПериод);
	...
	Выборка = СтруктураЗаписейЖурнала.ВыставленныеСчетаФактуры.Выбрать();
	Пока Выборка.Следующий() Цикл
		
		НоваяСтрока	= ВыставленныеСчетаФактуры.Добавить();
		УчетНДСКлиент.ЗаполнитьСтрокуЖурналаУчетаСчетовФактур(НоваяСтрока, Выборка, СтруктураПараметров);
