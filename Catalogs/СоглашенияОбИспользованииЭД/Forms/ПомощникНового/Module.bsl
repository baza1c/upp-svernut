...
Процедура СсылкаНаСертификатНажатие(Элемент, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Организация) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ЭлектронныеДокументыКлиентСервер.ПолучитьТекстСообщения("Поле", "Заполнение", "Организация"), , "Организация");
		Возврат;
	КонецЕсли;
...
Функция УстановитьПароль(Команда)
	...
	Если ВариантВыбораСертификата = "Добавить новый" Тогда
		...
		ПарольУстановлен = ЭлектронныеДокументыСлужебныйКлиент.ПолучитьПарольКСертификату(ПредставлениеСертификата,
			ПараметрыСертификата, НСтр("ru = 'Сохранение пароля в сертификате'"));
...
Функция ТестСертификата()
	...
	Если Не ЭлектронныеДокументыСлужебныйКлиент.ПолучитьПарольКСертификату(Представление, ПараметрыСертификата,
		НСтр("ru = 'Тестовая проверка шифрования'")) Тогда
...
Процедура ЗаполнитьРеквизитыСертификатаНаСервере(СертификатСсылка)
	...
	конецЕсли;
	ВидыДокументов.Загрузить(СертификатСсылка.ВидыДокументов.Выгрузить());
...
Процедура ТестСвязиСТакском(ТестПройден)
	...
	Если ЭлектронныеДокументыСлужебныйКлиент.ПолучитьПарольКСертификату(СертификатПодписи,
		ПараметрыСертификата, НСтр("ru = 'Авторизация на сервере Такском'")) Тогда
		
		Если КонтекстАвторизации = ПредопределенноеЗначение("Перечисление.КонтекстыРаботыСЭД.НаСервере") Тогда
			ТестПройден = ЭлектронныеДокументыСлужебныйВызовСервера.ТестСвязиСОператоромЭДО(ПараметрыСертификата, Ложь);
		Иначе
