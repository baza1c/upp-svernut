...
КонецПроцедуры // ПередЗаписью()
 ...
 Процедура ПриКопировании(ОбъектКопирования)
	 ...
	 // Письмо Киселевой при копировании не переносим тех.карту для определенных папок номенклатуры
	 // Тех.карту привязывает конкретный сотрудник (Сержанина)
	 
	 НоменклатураВходитВПапку = Ложь; 
	 Если ЭтотОбъект.ВыходныеИзделия.Количество() Тогда
		  НоменклатураВходитВПапку = абс_Дистрибуция.НоменклатураВходитВПапку(ЭтотОбъект.ВыходныеИзделия[0].Номенклатура);
     КонецЕсли;
	 
	 Если НоменклатураВходитВПапку Тогда
		  Лео_ТехКарта = Справочники.ТехнологическиеКартыПроизводства.ПустаяСсылка();
	 Иначе	 
		 ТехнологическиеКарты = РегистрыСведений.ТехнологическиеКартыСпецификацийПланирования.СрезПоследних(ТекущаяДата(), Новый Структура("Спецификация", ОбъектКопирования.Ссылка));
		 Если ТехнологическиеКарты.Количество() Тогда
			 ...
			 Если НЕ ТехнологическаяКарта.Пустая() Тогда
				 ТекстВопроса = "Перенести технологическую карту " + Строка(ТехнологическаяКарта) + "?";
				 Если Вопрос(ТекстВопроса,РежимДиалогаВопрос.ДаНет)=КодВозвратаДиалога.Да Тогда
					 ...
					 НоваяЗапись.Спецификация = СсылкаНового;
	 ...
	 //240328 При копировании устанвливаем текущего отвественного
	 Если НЕ Ответственный = ПараметрыСеанса.ТекущийПользователь Тогда
...
Функция НоменклатураВходитВПапку() Экспорт
	...
	Если ЭтотОбъект.ВыходныеИзделия.Количество() тогда
		...
		Запрос.УстановитьПараметр("Номенклатура", ЭтотОбъект.ВыходныеИзделия[0].Номенклатура); 
