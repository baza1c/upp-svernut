...
Процедура УстановитьНачальныеНастройки(ДополнительныеПараметры = Неопределено) Экспорт
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               |	Лео_Настройки._Ссылка,
	               |	Лео_Настройки.Использовать,
	               |	Лео_Настройки.ДопРеквизит_1,
	               |	Лео_Настройки.ДопРеквизит_2 
	               |ПОМЕСТИТЬ ТЗ_Новинки
	               |ИЗ
	               |	Справочник.Лео_Настройки КАК Лео_Настройки
	               |ГДЕ
	               |	Лео_Настройки.Родитель = &ПапкаНовинки
	               |	И Лео_Настройки.Использовать
				   |	И(ВЫРАЗИТЬ(Лео_Настройки.ДопРеквизит_2 КАК ДАТА)) >= &ДатаНач1
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	Планы.абс_Организация КАК Организация,
	               |	ПРЕДСТАВЛЕНИЕ(Планы.абс_Организация) КАК ОрганизацияПредставление,
	               |	Планы.абс_БизнесРегион КАК БизнесРегион,
	               |	ПРЕДСТАВЛЕНИЕ(Планы.абс_БизнесРегион) КАК БизнесРегионПредставление,
	               |	Планы.Номенклатура КАК Номенклатура,
	               |	ПРЕДСТАВЛЕНИЕ(Планы.Номенклатура),
	               |	ВЫБОР
	               |		КОГДА ТЗ_Новинки._Ссылка ЕСТЬ NULL 
	               |			ТОГДА ЛОЖЬ
	               |		ИНАЧЕ ИСТИНА
	               |	КОНЕЦ КАК ПризнакНовинки,
	               ...
	               |						ТОГДА 10
				   |				    КОГДА Планы.Номенклатура.СтавкаНДС = &НДС5
	               |							ИЛИ Планы.Номенклатура.СтавкаНДС = &НДС5_105
	               |						ТОГДА 5
	               |					КОГДА Планы.Номенклатура.СтавкаНДС = &НДС7
	               |						    ИЛИ Планы.Номенклатура.СтавкаНДС = &НДС7_107
	               |					    ТОГДА 7
	               |					ИНАЧЕ 0
	               |				КОНЕЦ
	               |		ИНАЧЕ 0
	               ...
	               |	ПризнакНовинки}
	               |ИЗ
	               |	РегистрНакопления.ПланыПродаж.Обороты(&ДатаНач, &ДатаКон, Регистратор {(&Периодичность)}, {(Сценарий).* КАК Сценарий, (Подразделение).* КАК Подразделение, (абс_Организация).* КАК Организация, (абс_БизнесРегион) КАК БизнесРегион, (Номенклатура).* КАК Номенклатура, (ХарактеристикаНоменклатуры).* КАК ХарактеристикаНоменклатуры, (ДокументПланирования).* КАК ДокументПланирования, (Проект).* КАК Проект, (Заказ).* КАК Заказ, (Контрагент).* КАК Контрагент, (Договор).* КАК Договор, (ВариантРаспределения) КАК ВариантРаспределения}) КАК Планы
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ТЗ_Новинки КАК ТЗ_Новинки
	               |		ПО Планы.Номенклатура = ТЗ_Новинки._Ссылка
	...
	УниверсальныйОтчет.мСтруктураПредставлениеПолей.Вставить("ПризнакНовинки", "Новинки");

	
	// Добавление показателей
	
	//УниверсальныйОтчет.ДобавитьПоказатель("ПризнакНовинки", "Новинки", ложь, "Булево" , "ПризнакНовинки","Признак новинки");
	...
	УниверсальныйОтчет.ДобавитьОтбор("БизнесРегион");
...
Процедура СформироватьОтчет(ТабличныйДокумент) Экспорт
	...
	УниверсальныйОтчет.ПостроительОтчета.Параметры.Вставить("НДС5", Перечисления.СтавкиНДС.НДС5);
	УниверсальныйОтчет.ПостроительОтчета.Параметры.Вставить("НДС5_105", Перечисления.СтавкиНДС.НДС5_105);
	УниверсальныйОтчет.ПостроительОтчета.Параметры.Вставить("НДС7", Перечисления.СтавкиНДС.НДС7);
	УниверсальныйОтчет.ПостроительОтчета.Параметры.Вставить("НДС7_107", Перечисления.СтавкиНДС.НДС7_107);
	
	
	УниверсальныйОтчет.ПостроительОтчета.Параметры.Вставить("ПапкаНовинки", Справочники.Лео_Настройки.НайтиПоНаименованию("Новинки"));
