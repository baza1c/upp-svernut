...
Процедура УстановитьТекстЗапроса(ЕстьПолеРегистратор = Истина)
	...
	|   Взаиморасчеты.РазницаОстаток КАК РазницаОстаток
	|	//ПОЛЯ_СВОЙСТВА
	|	//ПОЛЯ_КАТЕГОРИИ
	|{ВЫБРАТЬ
	...
	|       СУММА(ВзаиморасчетыБезГруппировки.СуммаВзаиморасчетовКонечныйОстаток - ВзаиморасчетыБезГруппировки.СуммаУпрКонечныйОстаток) КАК РазницаОстаток
	|	{ВЫБРАТЬ
	|		Организация КАК Организация,
	|		Контрагент КАК Контрагент,
	|		ДоговорКонтрагента КАК ДоговорКонтрагента,
