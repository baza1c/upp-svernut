Процедура СформироватьОтчет() Экспорт
	...
	Если ВыводитьКоличество тогда
		ОбластьЗаголовок.Параметры.Заголовок = "Отчет бюджет-Факт отгрузки на " + Формат(ДатаОтчета, "ДЛФ=DD")+" (в шт.)" ;
	Иначе
		ОбластьЗаголовок.Параметры.Заголовок = "Отчет бюджет-Факт отгрузки на " + Формат(ДатаОтчета, "ДЛФ=DD")+ " (без НДС)" ;
	КонецЕсли;
	...
	ДатаМесяцаПозаПрошлогоГода = Дата(Год(ДатаОтчета)-2,Месяц(ДатаОтчета),1);
	...
	ПозаПрошлыйГод = Формат(ДатаМесяцаПозаПрошлогоГода, "ДФ = гггг");
	...
	РабочихДнейМесяцПозаПрошлогоГода = ЗаполнениеДокументов.ЧислоРабочихДней(НачалоМесяца(ДатаМесяцаПозаПрошлогоГода),КонецМесяца(ДатаМесяцаПозаПрошлогоГода));
	...
	//ОбластьШапка.Параметры.ПозаПрошлыйГод = ПозаПрошлыйГод;
	...
	//ОбластьШапка.Параметры.РабочихДнейМесяцПозаПрошлогоГода = РабочихДнейМесяцПозаПрошлогоГода;
	...
	|			НАЧАЛОПЕРИОДА(&ДатаМесяцаПозаПрошлогоГода, ГОД),
	...
	|	СУММА(ВЫБОР
	|			КОГДА ПланФактПолный.Период МЕЖДУ НАЧАЛОПЕРИОДА(&ДатаМесяцаПозаПрошлогоГода, МЕСЯЦ) И КОНЕЦПЕРИОДА(&ДатаМесяцаПозаПрошлогоГода, МЕСЯЦ)
	|				ТОГДА ВЫБОР
	|						КОГДА &ВыводитьКоличество
	|							ТОГДА ПланФактПолный.КоличествоФакт
	|						ИНАЧЕ ПланФактПолный.СуммаФакт
	|					КОНЕЦ
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК СуммаМесяцПозаПрошлогоГода,
	|	СУММА(ВЫБОР
	...
	|	ПланФакт.СуммаМесяцПозаПрошлогоГода КАК СуммаМесяцПозаПрошлогоГода,
	...
	|	ПланФакт.СуммаРасчетМесяц КАК СуммаРасчетМесяц,
	|	ПланФакт.СуммаПланКвартал КАК СуммаПланКвартал,
	|	ПланФакт.СуммаРасчетКвартал КАК СуммаРасчетКвартал,
	...
	|	ВЫРАЗИТЬ(СУММА(СуммаМесяцПозаПрошлогоГода) КАК ЧИСЛО(15, 2)) КАК СуммаМесяцПозаПрошлогоГода,
	...
	|	ВЫРАЗИТЬ(СУММА(СуммаРасчетМесяц) КАК ЧИСЛО(15, 2)) КАК СуммаРасчетМесяц,
	|	ВЫРАЗИТЬ(СУММА(СуммаПланКвартал) КАК ЧИСЛО(15, 2)) КАК СуммаПланКвартал,
	|	ВЫРАЗИТЬ(СУММА(СуммаРасчетКвартал) КАК ЧИСЛО(15, 2)) КАК СуммаРасчетКвартал,
	...
	Запрос.УстановитьПараметр("ДатаМесяцаПозаПрошлогоГода", ДатаМесяцаПозаПрошлогоГода);	
	
	РезультатЗапроса = Запрос.Выполнить();
	
	
	ТЗкоммерческийАссортимент = РезультатЗапроса.Выгрузить();
	ТЗкоммерческийАссортимент.Очистить();
	ТЗспециализированныйАссортимент = ТЗкоммерческийАссортимент.Скопировать();
	...
	Если ВыборкаКС.Следующий() Тогда
        ...
        //Зароднюк ВВ  22.09.2025
		ЗаполнитьДопКолонки(ОбластьСтрокаКС,ВыборкаКС,РабочихДнейМесяц,РабочихДнейКвартал);
		...
		Пока ВыборкаПодразделение.Следующий() Цикл
			...
			ЗаполнитьКоммерческийАссортимент(ТЗкоммерческийАссортимент,ТЗспециализированныйАссортимент,ВыборкаПодразделение);
	        ...
	        //Зароднюк ВВ  22.09.2025
			ЗаполнитьДопКолонки(ОбластьСтрокаПодразделение,ВыборкаПодразделение,РабочихДнейМесяц,РабочихДнейКвартал);
			...
			Пока ВыборкаБизнесРегион.Следующий() Цикл
		        ...
		        //Зароднюк ВВ  22.09.2025
				ЗаполнитьДопКолонки(ОбластьСтрокаБизнесРегион,ВыборкаБизнесРегион,РабочихДнейМесяц,РабочихДнейКвартал);
				...
				Пока ВыборкаТерритория.Следующий() Цикл
					...
					Если ЗначениеЗаполнено(ВыборкаТерритория.Территория) Тогда
				        ...
				        //Зароднюк ВВ  22.09.2025
						ЗаполнитьДопКолонки(ОбластьСтрокаТеритория,ВыборкаТерритория,РабочихДнейМесяц,РабочихДнейКвартал);
					...
					Пока ВыборкаГород.Следующий() Цикл
						...
						Если ЗначениеЗаполнено(ВыборкаГород.Город) Тогда
					        ...
					        //Зароднюк ВВ  22.09.2025
							ЗаполнитьДопКолонки(ОбластьСтрокаГород,ВыборкаГород,РабочихДнейМесяц,РабочихДнейКвартал);
						...
						Пока ВыборкаПорядок.Следующий() Цикл
							...
							Пока ВыборкаКонтрагент.Следующий() Цикл
						        ...
						        //Зароднюк ВВ  22.09.2025
								ЗаполнитьДопКолонки(ОбластьСтрокаКонтрагент,ВыборкаКонтрагент,РабочихДнейМесяц,РабочихДнейКвартал);
								...
								Если ВыводитьНоменклатуру Тогда
									...
									Пока ВыборкаПорядокНоменклатуры.Следующий() Цикл
										...
										Пока ВыборкаНоменклатура.Следующий() Цикл
									        ...
									        //Зароднюк ВВ  22.09.2025
											ЗаполнитьДопКолонки(ОбластьСтрокаНоменклатура,ВыборкаНоменклатура,РабочихДнейМесяц,РабочихДнейКвартал);
	...
	// Коммерческий, Специализировынный ассортимент
	        ТЗкоммерческийАссортимент.Свернуть(,"СуммаКварталПрошлогоГода,СуммаМесяцПозаПрошлогоГода,СуммаМесяцПрошлогоГода,СуммаПланГод,СуммаПланКвартал,СуммаРасчетКвартал,
			                                  |СуммаПланМесяц,СуммаРасчетМесяц,СуммаПрошлыйГод,СуммаПрошлыйМесяц,СуммаСортировкиКонтрагентов,СуммаСортировкиНоменклтуры,СуммаФактГод,СуммаФактКвартал,СуммаФактМесяц");
	        ТЗспециализированныйАссортимент.Свернуть(,"СуммаКварталПрошлогоГода,СуммаМесяцПозаПрошлогоГода,СуммаМесяцПрошлогоГода,СуммаПланГод,СуммаПланКвартал,СуммаРасчетКвартал,
			                                  |СуммаПланМесяц,СуммаРасчетМесяц,СуммаПрошлыйГод,СуммаПрошлыйМесяц,СуммаСортировкиКонтрагентов,СуммаСортировкиНоменклтуры,СуммаФактГод,СуммаФактКвартал,СуммаФактМесяц");
											  
			ОбластьСтрокаКоммерческийАссортимент = Макет.ПолучитьОбласть("СтрокаКоммерческийАссортимент");
			ОбластьСтрокаСпециализированныйАссортимент = Макет.ПолучитьОбласть("СтрокаСпециализированныйАссортимент");
			
			ЗаполнитьЗначенияСвойств(ОбластьСтрокаКоммерческийАссортимент.Параметры, ТЗкоммерческийАссортимент[0]);
			ЗаполнитьЗначенияСвойств(ОбластьСтрокаСпециализированныйАссортимент.Параметры, ТЗспециализированныйАссортимент[0]);
			
			ЗаполнитьПроценты(ОбластьСтрокаКоммерческийАссортимент,ТЗкоммерческийАссортимент[0], ПроцентРабочихДнейМесяцЧисло, ПроцентРабочихДнейКварталЧисло, ПроцентРабочихДнейГодЧисло);
			ЗаполнитьПроценты(ОбластьСтрокаСпециализированныйАссортимент,ТЗспециализированныйАссортимент[0], ПроцентРабочихДнейМесяцЧисло, ПроцентРабочихДнейКварталЧисло, ПроцентРабочихДнейГодЧисло);
			
		    ОбластьСтрокаПробел = Макет.ПолучитьОбласть("СтрокаПробел");
			Результат.Вывести(ОбластьСтрокаПробел);
			
			Результат.Вывести(ОбластьСтрокаКоммерческийАссортимент);
			Результат.Вывести(ОбластьСтрокаСпециализированныйАссортимент);
...
Процедура ЗаполнитьКоммерческийАссортимент(ТЗкоммерческийАссортимент,ТЗспециализированныйАссортимент,ВыборкаПодразделение);
	

	Если ВыборкаПодразделение.Подразделение.Код = "Б00000009" или ВыборкаПодразделение.Подразделение.Код = "Б00000012"  тогда	//ИНДУСТРИАЛЬНЫЙ, ГОС. УЧРЕЖДЕНИЯ
	
	_стр = ТЗспециализированныйАссортимент.Добавить();
	ЗаполнитьЗначенияСвойств(_стр,ВыборкаПодразделение);
	
	иначе // остальные
	
	стр = ТЗкоммерческийАссортимент.Добавить();
	ЗаполнитьЗначенияСвойств(стр,ВыборкаПодразделение);
...
Процедура ЗаполнитьПроценты(ОбластьМакета, ВыборкаЗапроса, ПроцентРабочихДнейМесяцЧисло, ПроцентРабочихДнейКварталЧисло, ПроцентРабочихДнейГодЧисло)
	...
	Если ПроцентМесяц<ПроцентРабочихДнейМесяцЧисло Тогда
		ОбластьМакета.Область(1,8).ЦветТекста = WebЦвета.Красный;
	КонецЕсли;
	...
	//	ОбластьМакета.Область(1,18).ЦветТекста = WebЦвета.Красный;
	//КонецЕсли;
	...
	//	ОбластьМакета.Область(1,8).ЦветТекста = WebЦвета.Синий;
	//КонецЕсли;
	...
	//	ОбластьМакета.Область(1,24).ЦветТекста = WebЦвета.Красный;
	// ИначеЕсли ПроцентКварталПрошлогоГода<10 Тогда
	//	ОбластьМакета.Область(1,24).ЦветТекста = WebЦвета.Синий;
	//КонецЕсли;
	...
	//	ОбластьМакета.Область(1,26).ЦветТекста = WebЦвета.Красный;
	//ИначеЕсли ПроцентПрошлыйГод<10 Тогда
	//	ОбластьМакета.Область(1,26).ЦветТекста = WebЦвета.Синий;
	//КонецЕсли;
	...
	//	ОбластьМакета.Область(1,21).ЦветТекста = WebЦвета.Красный;
	//ИначеЕсли ПроцентПрошлыйМесяц<10 Тогда
	//	ОбластьМакета.Область(1,21).ЦветТекста = WebЦвета.Синий;
	//КонецЕсли;
	
КонецПроцедуры

Процедура ЗаполнитьДопКолонки(ОбластьМакета, ВыборкаЗапроса, РабочихДнейМесяц, РабочихДнейКвартал) 
	
	СуммаРасчетМесяц 	= Окр(ВыборкаЗапроса.СуммаПланМесяц/РабочихДнейМесяц*ЗаполнениеДокументов.ЧислоРабочихДней(НачалоМесяца(ДатаОтчета), КонецДня(ДатаОтчета)),0);
	ОтклонениеМесяц		= Окр(ВыборкаЗапроса.СуммаФактМесяц-СуммаРасчетМесяц,0);
	
	ОбластьМакета.Параметры.СуммаРасчетМесяц 	= СуммаРасчетМесяц;
	ОбластьМакета.Параметры.ОтклонениеМесяц 	= ОтклонениеМесяц; 
	Если ОтклонениеМесяц<0 Тогда
		ОбластьМакета.Область(1,11).ЦветТекста = WebЦвета.Красный;
	КонецЕсли;
	
	СуммаРасчетКвартал 	= 	""+100*Окр(ЗаполнениеДокументов.ЧислоРабочихДней(НачалоКвартала(ДатаОтчета), КонецДня(ДатаОтчета))/	РабочихДнейКвартал,2)+" %" ;

	ОтклонениеКвартал	= 	Окр(ВыборкаЗапроса.СуммаФактКвартал-
	(ВыборкаЗапроса.СуммаПланКвартал/РабочихДнейКвартал)*ЗаполнениеДокументов.ЧислоРабочихДней(НачалоКвартала(ДатаОтчета), КонецДня(ДатаОтчета)),0);
	
	ОбластьМакета.Параметры.СуммаРасчетКвартал 	= СуммаРасчетКвартал;
	ОбластьМакета.Параметры.ОтклонениеКвартал 	= ОтклонениеКвартал; 
...
Процедура ЗаполнитьСтруктуруПрочих(СтруктураЗначений)
	...
	СтруктураЗначений.Вставить("СуммаМесяцПозаПрошлогоГода", 0);
...
Процедура ДобавитьСуммыВСтруктуруПрочих(СтруктураЗначений, ВыборкаЗапроса)
	...
	СтруктураЗначений.СуммаМесяцПозаПрошлогоГода = СтруктураЗначений.СуммаМесяцПозаПрошлогоГода + ВыборкаЗапроса.СуммаМесяцПозаПрошлогоГода;
