...
Процедура ВывестиПоказатели(Выборка, СтруктураПараметров)
	...
	//ЛЕО Зароднюк Владимир 
	Если ИмяГруппировки="Номенклатура" Тогда
		ОбластьЗначенияПоказателя.Параметры.Артикул = Выборка.Артикул;
	КонецЕсли;
	//ЛЕО Зароднюк Владимир 
...
// 			ТекстЗапроса (строка).
//
Функция ШаблонТекстаЗапросаКурсВалютыУпрУчета(ИмяДокумента, ИмяТЧ)
	...
	|ПО КурсыВалютыУпрУчета.Дата = "+ИмяДокумента+ ИмяТЧ +".Ссылка.Дата";
...
Функция ШаблонТекстаЗапросаПересчетСуммыВВалютуУпрУчетаВозвраты(ТаблицаДокумента,Источник, НДСИсточник)

		Возврат
	"	ВЫРАЗИТЬ(ВЫБОР
	|		КОГДА " + ТаблицаДокумента + ".Ссылка.ВалютаДокумента = Константы.ВалютаУправленческогоУчета ТОГДА
	|			ВЫБОР КОГДА " + ТаблицаДокумента + ".Ссылка.СуммаВключаетНДС ТОГДА
	|			" 	+ ТаблицаДокумента + "."+Источник  + "* (-1)" + "
	|			ИНАЧЕ
	|			(" 	+ ТаблицаДокумента + "."+Источник+" + "+ ТаблицаДокумента + "."+НДСИсточник + ") * (-1)" + "
	|			КОНЕЦ
	|		КОГДА " + ТаблицаДокумента + ".Ссылка.ВалютаДокумента = Константы.ВалютаРегламентированногоУчета ТОГДА
	|			ВЫБОР КОГДА " + ТаблицаДокумента + ".Ссылка.СуммаВключаетНДС ТОГДА
	|				" 	+ ТаблицаДокумента + "."+Источник+ "*КурсыВалютыУпрУчета.Кратность / КурсыВалютыУпрУчета.Курс * (-1)
	|			ИНАЧЕ
	|				("+ТаблицаДокумента+"."+Источник+" + "+ ТаблицаДокумента + "."+НДСИсточник+")*КурсыВалютыУпрУчета.Кратность / КурсыВалютыУпрУчета.Курс *(-1)
	|			КОНЕЦ
	|		КОГДА " + ТаблицаДокумента + ".Ссылка.ВалютаДокумента <> Константы.ВалютаУправленческогоУчета 
	|			И КурсыВалютыУпрУчета.Курс <> 0 ТОГДА
	|			ВЫБОР КОГДА " + ТаблицаДокумента + ".Ссылка.СуммаВключаетНДС ТОГДА
	|				" + ТаблицаДокумента + "."+Источник+" * " + ТаблицаДокумента + ".Ссылка.КурсВзаиморасчетов * КурсыВалютыУпрУчета.Кратность / (КурсыВалютыУпрУчета.Курс * 1) *(-1)// ограничение разрядов выполняется как при записи в регистр
	|			ИНАЧЕ
	|				("+ТаблицаДокумента+"."+Источник+" + "+ ТаблицаДокумента + "."+НДСИсточник+") * " + ТаблицаДокумента + ".Ссылка.КурсВзаиморасчетов * КурсыВалютыУпрУчета.Кратность / (КурсыВалютыУпрУчета.Курс * 1) * (-1) // ограничение разрядов выполняется как при записи в регистр
	|			КОНЕЦ
	|		ИНАЧЕ 
	|			0
	|		КОНЕЦ КАК ЧИСЛО (15,2))"
...
Функция СформироватьТекстЗапроса(Источник,КопияГруппировкиОтчета,СтруктураДоступныхГруппировок,СтруктураДоступныхПоказателей, СтруктураИсключений = Неопределено)
	...
	ИначеЕсли Источник="ОтчетКомиссионераОПродажах.Товары" Тогда
		...
		|" 	+ ТекстИсточникиСведений
			+ ШаблонТекстаЗапросаКурсВалютыУпрУчета("ОтчетКомиссионераОПродажах", "Товары") + "
		| ГДЕ 
		...
		КонецЕсли;
        //{Катанович
		ИначеЕсли Источник="ОтчетКомиссионераОПродажах.Возвраты" Тогда
		ТекстИсточника="Документ."+Источник+" КАК ОтчетКомиссионераОПродажахВозвраты
		|" 	+ ТекстИсточникиСведений
			+ ШаблонТекстаЗапросаКурсВалютыУпрУчета("ОтчетКомиссионераОПродажах", "Возвраты") + "
		| ГДЕ 
		|	ОтчетКомиссионераОПродажахВозвраты.Ссылка.Проведен";
		
		Если (ДатаНач<>'00010101000000') и (ДатаКон<>'00010101000000') Тогда
			ТекстИсточника=ТекстИсточника+"
			| И (ОтчетКомиссионераОПродажахВозвраты.Ссылка.Дата МЕЖДУ &ДатаНач И &ДатаКон)";
		ИначеЕсли ДатаНач<>'00010101000000' Тогда
			ТекстИсточника=ТекстИсточника+"
			| И (ОтчетКомиссионераОПродажахВозвраты.Ссылка.Дата>= &ДатаНач)";
		ИначеЕсли ДатаКон<>'00010101000000' Тогда
			ТекстИсточника=ТекстИсточника+"
			| И (ОтчетКомиссионераОПродажахВозвраты.Ссылка.Дата<= &ДатаКон)";
		КонецЕсли;

		Если ТолькоПросроченные Тогда
			ТекстИсточника=ТекстИсточника+"
			|И ОтчетКомиссионераОПродажахВозвраты.Ссылка.ДоговорКонтрагента В (&МассивКомиссионеров)";
		КонецЕсли;
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ВысотаЗаголовка, ТолькоЗаголовок = Ложь,  ЕстьОшибки = Ложь) Экспорт
	...
	Для Каждого СтрокаГруппировки Из КопияГруппировкиОтчета Цикл
		...
		КонецЕсли;
	//ЛЕО Зароднюк Владимир 
		Если Найти(СтрокаГруппировки.ИмяГруппировки,"Номенклатура")>0  Тогда
			ТекстЗапроса = ТекстЗапроса + ",
			|ЗапросПоТоварамОтданным.Номенклатура.Артикул КАК Артикул";
		КонецЕсли;
	//ЛЕО Зароднюк Владимир 
	КонецЦикла;
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|";
	// По документу "ОтчетКомиссионераОПродажах": Возвраты.

	СтруктураДоступныхГруппировок.Вставить("Контрагент",                "ОтчетКомиссионераОПродажахВозвраты.Ссылка.ДоговорКонтрагента.Владелец");
	СтруктураДоступныхГруппировок.Вставить("ДоговорКонтрагента",       "ОтчетКомиссионераОПродажахВозвраты.Ссылка.ДоговорКонтрагента");
	СтруктураДоступныхГруппировок.Вставить("Сделка",                    "ВЫБОР КОГДА ОтчетКомиссионераОПродажахВозвраты.Ссылка.Сделка = ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка) ТОГДА НЕОПРЕДЕЛЕНО ИНАЧЕ ОтчетКомиссионераОПродажахВозвраты.Ссылка.Сделка КОНЕЦ");
	СтруктураДоступныхГруппировок.Вставить("Организация",               "ОтчетКомиссионераОПродажахВозвраты.Ссылка.ДоговорКонтрагента.Организация");
	СтруктураДоступныхГруппировок.Вставить("Номенклатура",              "ОтчетКомиссионераОПродажахВозвраты.Номенклатура");
	СтруктураДоступныхГруппировок.Вставить("ХарактеристикаНоменклатуры","ОтчетКомиссионераОПродажахВозвраты.ХарактеристикаНоменклатуры");
	СтруктураДоступныхГруппировок.Вставить("СерияНоменклатуры",         "ОтчетКомиссионераОПродажахВозвраты.СерияНоменклатуры");
	СтруктураДоступныхГруппировок.Вставить("Регистратор",               "ОтчетКомиссионераОПродажахВозвраты.Ссылка");
	...
	СтруктураДоступныхПоказателей.Вставить("СуммаКонечныйОстаток",      "0");
	СтруктураДоступныхПоказателей.Вставить("СуммаПриход",               "0");
	СтруктураДоступныхПоказателей.Вставить("КоличествоРасход",          "ОтчетКомиссионераОПродажахВозвраты.Количество*(-1)");	
	СтруктураДоступныхПоказателей.Вставить("КоличествоЕдиницДляОтчетовРасход","(ОтчетКомиссионераОПродажахВозвраты.Количество*ОтчетКомиссионераОПродажахВозвраты.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент/ОтчетКомиссионераОПродажахВозвраты.Номенклатура.ЕдиницаДляОтчетов.Коэффициент)*(-1)");
	СтруктураДоступныхПоказателей.Вставить("СуммаРасход",               ШаблонТекстаЗапросаПересчетСуммыВВалютуУпрУчетаВозвраты("ОтчетКомиссионераОПродажахВозвраты","Сумма","СуммаНДС"));
	СтруктураДоступныхПоказателей.Вставить("СуммаНачальныйДолг",        "0");
	СтруктураДоступныхПоказателей.Вставить("СуммаКонечныйДолг",         "0");
	СтруктураДоступныхПоказателей.Вставить("СуммаОплата",               "0");
	СтруктураДоступныхПоказателей.Вставить("СуммаВознаграждение",       ШаблонТекстаЗапросаПересчетСуммыВВалютуУпрУчетаВозвраты("ОтчетКомиссионераОПродажахВозвраты","СуммаВознаграждения","СуммаНДСВознаграждения"));

	ТекстЗапроса=ТекстЗапроса+СформироватьТекстЗапроса("ОтчетКомиссионераОПродажах.Возвраты",
	КопияГруппировкиОтчета,СтруктураДоступныхГруппировок,СтруктураДоступныхПоказателей, Новый Структура("Сделка", "ОтчетКомиссионераОПродажахВозвраты.Ссылка.Сделка"));
	ТекстЗапроса=ТекстЗапроса+"
	//Катанович}
	...
	//{ Зароднюк В. по просьбе Давискибы
	Если ОтрицательныйОстсток Тогда
		Запрос.Текст = Запрос.Текст  + Символы.ПС +
		"ГДЕ ЗапросПоТоварамОтданным.КоличествоКонечныйОстаток < 0";
	КонецЕсли;	
    //}
	Если Не ПустаяСтрока(ТекстПоляУпорядочивания) Тогда
		Запрос.Текст = Запрос.Текст  + Символы.ПС +
		"УПОРЯДОЧИТЬ ПО Артикул, " + ТекстПоляУпорядочивания; 
	Иначе
