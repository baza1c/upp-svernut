Процедура СформироватьОтчет() Экспорт
	...
	ОбластьЗаголовок.Параметры.Заголовок = "Отчет План-Факт отгрузки на " + Формат(ДатаОтчета, "ДЛФ=DD")+ " (без НДС)" + "    Сценарий плана: " + Сценарий ;
	...
	|	0 КАК СуммаПлан,
	|	ЕСТЬNULL(ПродажиОбороты.СтоимостьОборот, 0) - ЕСТЬNULL(ПродажиОбороты.НДСОборот, 0) КАК СуммаФакт,
	...
	|	0,
	|	ЕСТЬNULL(ПродажиОбороты.СтоимостьОборот, 0) - ЕСТЬNULL(ПродажиОбороты.НДСОборот, 0),
	...
	|				ТОГДА ПланФактПолный.СуммаФакт
	|			ИНАЧЕ 0
	...
	|				ТОГДА ПланФактПолный.СуммаПлан
	|			ИНАЧЕ 0
	...
	|	СУММА(ВЫБОР
	|			КОГДА ПланФактПолный.Период = НАЧАЛОПЕРИОДА(&ДатаОтчета, МЕСЯЦ)
	|				ТОГДА ПланФактПолный.СуммаПрогноз
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК СуммаПрогнозМесяц,
	|	СУММА(ВЫБОР
	|			КОГДА ПланФактПолный.Период МЕЖДУ НАЧАЛОПЕРИОДА(&ДАтаОтчета, КВАРТАЛ) И КОНЕЦПЕРИОДА(&ДатаОтчета, КВАРТАЛ)
	|				ТОГДА ВЫБОР
	|						КОГДА ПланФактПолный.Период < НАЧАЛОПЕРИОДА(&ДатаОтчета, МЕСЯЦ)
	|							ТОГДА ПланФактПолный.СуммаФакт
	|						ИНАЧЕ ПланФактПолный.СуммаПрогноз
	|					КОНЕЦ
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК СуммаПрогнозКвартал,
	|	СУММА(ВЫБОР
	|			КОГДА ПланФактПолный.Период МЕЖДУ НАЧАЛОПЕРИОДА(&ДАтаОтчета, ГОД) И КОНЕЦПЕРИОДА(&ДатаОтчета, ГОД)
	|				ТОГДА ВЫБОР
	|						КОГДА ПланФактПолный.Период < НАЧАЛОПЕРИОДА(&ДатаОтчета, МЕСЯЦ)
	...
	|	ПланФакт.СуммаПрогнозМесяц КАК СуммаПрогнозМесяц,
	|	ПланФакт.СуммаПрогнозКвартал КАК СуммаПрогнозКвартал,
	|	ПланФакт.СуммаПрогнозГод КАК СуммаПрогнозГод
	|ИЗ
	|	ПланФакт КАК ПланФакт
	...
	|ГДЕ
	|	ПланФакт.Подразделение <> &ПодразделениеКУдалению
	|
	|УПОРЯДОЧИТЬ ПО
	...
	|	СУММА(ПорядокКонтрагентов),
	|	СУММА(ПорядокНоменклатуры),
	|	МАКСИМУМ(СуммаСортировкиНоменклтуры),
	|	МАКСИМУМ(СуммаСортировкиКонтрагентов),
	|	ВЫРАЗИТЬ(СУММА(СуммаПрогнозМесяц) КАК ЧИСЛО(15, 2)) КАК СуммаПрогнозМесяц,
	|	ВЫРАЗИТЬ(СУММА(СуммаПрогнозКвартал) КАК ЧИСЛО(15, 2)) КАК СуммаПрогнозКвартал,
	|	ВЫРАЗИТЬ(СУММА(СуммаПрогнозГод) КАК ЧИСЛО(15, 2)) КАК СуммаПрогнозГод
	|ПО
	|	ОБЩИЕ,
	|	Подразделение,
	|	БизнесРегион,
	...
	ПодразделениеКУдалению = Справочники.Регионы.НайтиПоНаименованию("ВНУТРЕННИЕ");
	...
	Запрос.УстановитьПараметр("СценарийПрогноз", СценарийПрогноз);
...
Процедура ЗаполнитьПроценты(ОбластьМакета, ВыборкаЗапроса, ПроцентРабочихДнейМесяцЧисло, ПроцентРабочихДнейКварталЧисло, ПроцентРабочихДнейГодЧисло)
	...
	Если ПроцентКвартал<ПроцентРабочихДнейКварталЧисло Тогда
		...
		ОбластьМакета.Область(1,13).ЦветТекста = WebЦвета.Красный;
	КонецЕсли;
	...
	Если ПроцентПрошлыйГод<5 Тогда
		ОбластьМакета.Область(1,27).ЦветТекста = WebЦвета.Красный;
	ИначеЕсли ПроцентПрошлыйГод<10 Тогда
		ОбластьМакета.Область(1,27).ЦветТекста = WebЦвета.Синий;
	КонецЕсли;
	...
	Если ПроцентПрошлыйМесяц<5 Тогда
		ОбластьМакета.Область(1,30).ЦветТекста = WebЦвета.Красный;
	ИначеЕсли ПроцентПрошлыйМесяц<10 Тогда
		ОбластьМакета.Область(1,30).ЦветТекста = WebЦвета.Синий;
	КонецЕсли;
	
	ПроцентПрогнозМесяц =Окр(?(ВыборкаЗапроса.СуммаПланМесяц=0,0,ВыборкаЗапроса.СуммаПрогнозМесяц/ВыборкаЗапроса.СуммаПланМесяц*100),1);
	ОбластьМакета.Параметры.ПроцентПрогнозМесяц = "" + Окр(ПроцентПрогнозМесяц) + "%";
	Если ПроцентПрогнозМесяц<ПроцентРабочихДнейМесяцЧисло Тогда
		ОбластьМакета.Область(1,10).ЦветТекста = WebЦвета.Красный;
	КонецЕсли;
	
	ПроцентПрогнозКвартал =Окр(?(ВыборкаЗапроса.СуммаПланКвартал=0,0,ВыборкаЗапроса.СуммаПрогнозКвартал/ВыборкаЗапроса.СуммаПланКвартал*100),1);
	ОбластьМакета.Параметры.ПроцентПрогнозКвартал = "" + Окр(ПроцентПрогнозКвартал) + "%";
	Если ПроцентПрогнозКвартал<ПроцентРабочихДнейМесяцЧисло Тогда
		ОбластьМакета.Область(1,15).ЦветТекста = WebЦвета.Красный;
	КонецЕсли;
	
	ПроцентПрогнозГод =Окр(?(ВыборкаЗапроса.СуммаПланГод=0,0,ВыборкаЗапроса.СуммаПрогнозГод/ВыборкаЗапроса.СуммаПланГод*100),1);
	ОбластьМакета.Параметры.ПроцентПрогнозГод = "" + Окр(ПроцентПрогнозГод) + "%";
	Если ПроцентПрогнозГод<ПроцентРабочихДнейМесяцЧисло Тогда
