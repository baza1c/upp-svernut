Процедура СформироватьОтчет() Экспорт
	...
	ОсталосьРабочихДнейМесяц  =  РабочихДнейМесяц-ПрошлоРабочихДнейМесяц;
	...
	ОсталосьРабочихДнейКвартал  = РабочихДнейКвартал-ПрошлоРабочихДнейКвартал;
	...
	ОбластьШапка.Параметры.ОсталосьРабочихДнейМесяц = ОсталосьРабочихДнейМесяц;
	ОбластьШапка.Параметры.ОсталосьРабочихДнейКвартал = ОсталосьРабочихДнейКвартал;
	...
	|	ПланФакт.СуммаФактМесяц - ПланФакт.СуммаПланМесяц КАК ОтклонениеСуммаМесяц,
	|	ПланФакт.СуммаФактКвартал - ПланФакт.СуммаПланКвартал КАК ОтклонениеСуммаКвартал
	|ИЗ
	|	ПланФакт КАК ПланФакт
	...
	|	ВЫРАЗИТЬ(СУММА(ОтклонениеСуммаМесяц) КАК ЧИСЛО(15, 2)) КАК ОтклонениеСуммаМесяц,
	|	ВЫРАЗИТЬ(СУММА(ОтклонениеСуммаКвартал) КАК ЧИСЛО(15, 2)) КАК ОтклонениеСуммаКвартал
	|ПО
	|	ОБЩИЕ,
	|	Подразделение,
	|	БизнесРегион,
	...
	Если ВыборкаКС.Следующий() Тогда
		...
		Если ВыборкаКС.ОтклонениеСуммаКвартал<0 Тогда
			ОбластьСтрокаКС.Область(1,6).ЦветТекста = WebЦвета.Красный;
		КонецЕсли;
		Если ВыборкаКС.ОтклонениеСуммаКвартал<0 Тогда
			ОбластьСтрокаКС.Область(1,10).ЦветТекста = WebЦвета.Красный;
		КонецЕсли;
		...
		Пока ВыборкаПодразделение.Следующий() Цикл
			...
			Если ВыборкаПодразделение.ОтклонениеСуммаКвартал<0 Тогда
				ОбластьСтрокаПодразделение.Область(1,6).ЦветТекста = WebЦвета.Красный;
			КонецЕсли;
			Если ВыборкаПодразделение.ОтклонениеСуммаКвартал<0 Тогда
				ОбластьСтрокаПодразделение.Область(1,10).ЦветТекста = WebЦвета.Красный;
			КонецЕсли;
			...
			Пока ВыборкаБизнесРегион.Следующий() Цикл
				...
				Если ВыборкаБизнесРегион.ОтклонениеСуммаКвартал<0 Тогда
					ОбластьСтрокаБизнесРегион.Область(1,6).ЦветТекста = WebЦвета.Красный;
				КонецЕсли;
				Если ВыборкаБизнесРегион.ОтклонениеСуммаКвартал<0 Тогда
					ОбластьСтрокаБизнесРегион.Область(1,10).ЦветТекста = WebЦвета.Красный;
				КонецЕсли;
				...
				ВыборкаКанал = ВыборкаБизнесРегион.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
				
				Пока ВыборкаКанал.Следующий() Цикл
					
					ОбластьСтрокаКанал = Макет.ПолучитьОбласть("СтрокаКанал");
					
					ЗаполнитьЗначенияСвойств(ОбластьСтрокаКанал.Параметры, ВыборкаКанал);
					Если ВыборкаКанал.ОтклонениеСуммаКвартал<0 Тогда
						ОбластьСтрокаКанал.Область(1,6).ЦветТекста = WebЦвета.Красный;
					КонецЕсли;
					Если ВыборкаКанал.ОтклонениеСуммаКвартал<0 Тогда
						ОбластьСтрокаКанал.Область(1,10).ЦветТекста = WebЦвета.Красный;
					КонецЕсли;
					ЗаполнитьПроценты(ОбластьСтрокаКанал,ВыборкаКанал, ПроцентРабочихДнейМесяцЧисло, ПроцентРабочихДнейКварталЧисло, ПроцентРабочихДнейГодЧисло);
					
					Результат.Вывести(ОбластьСтрокаКанал, 2);
					
					ВыборкаТерритория = ВыборкаКанал.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
					...
					Пока ВыборкаТерритория.Следующий() Цикл
						...
						Если ЗначениеЗаполнено(ВыборкаТерритория.Территория) Тогда
							...
							Если ВыборкаТерритория.ОтклонениеСуммаКвартал<0 Тогда
								ОбластьСтрокаТеритория.Область(1,6).ЦветТекста = WebЦвета.Красный;
							КонецЕсли;
							Если ВыборкаТерритория.ОтклонениеСуммаКвартал<0 Тогда
								ОбластьСтрокаТеритория.Область(1,10).ЦветТекста = WebЦвета.Красный;
							КонецЕсли;
						...
						Пока ВыборкаГород.Следующий() Цикл
							...
							Если ЗначениеЗаполнено(ВыборкаГород.Город) Тогда
								...
								Если ВыборкаГород.ОтклонениеСуммаКвартал<0 Тогда
									ОбластьСтрокаГород.Область(1,6).ЦветТекста = WebЦвета.Красный;
								КонецЕсли;
								Если ВыборкаГород.ОтклонениеСуммаКвартал<0 Тогда
									ОбластьСтрокаГород.Область(1,10).ЦветТекста = WebЦвета.Красный;
								КонецЕсли;
							...
							Пока ВыборкаПорядок.Следующий() Цикл
								...
								Пока ВыборкаКонтрагент.Следующий() Цикл
									...
									Если ВыборкаКонтрагент.ОтклонениеСуммаКвартал<0 Тогда
										ОбластьСтрокаКонтрагент.Область(1,6).ЦветТекста = WebЦвета.Красный;
									КонецЕсли;
									Если ВыборкаКонтрагент.ОтклонениеСуммаКвартал<0 Тогда
										ОбластьСтрокаКонтрагент.Область(1,10).ЦветТекста = WebЦвета.Красный;
									КонецЕсли;
									...
									Если ВыводитьНоменклатуру Тогда
										...
										Пока ВыборкаПорядокНоменклатуры.Следующий() Цикл
											...
											Пока ВыборкаНоменклатура.Следующий() Цикл
												...
												Если ВыборкаНоменклатура.ОтклонениеСуммаКвартал<0 Тогда
													ОбластьСтрокаНоменклатура.Область(1,6).ЦветТекста = WebЦвета.Красный;
												КонецЕсли;
												Если ВыборкаНоменклатура.ОтклонениеСуммаКвартал<0 Тогда
													ОбластьСтрокаНоменклатура.Область(1,10).ЦветТекста = WebЦвета.Красный;
												КонецЕсли;
...
Процедура ЗаполнитьПроценты(ОбластьМакета, ВыборкаЗапроса, ПроцентРабочихДнейМесяцЧисло, ПроцентРабочихДнейКварталЧисло, ПроцентРабочихДнейГодЧисло)
	...
	ИначеЕсли ПроцентМесяцПрошлогоГода<10 Тогда
		ОбластьМакета.Область(1,18).ЦветТекста = WebЦвета.Синий;
	КонецЕсли;
	...
	Если ПроцентПрошлыйГод<5 Тогда
		ОбластьМакета.Область(1,22).ЦветТекста = WebЦвета.Красный;
	ИначеЕсли ПроцентПрошлыйГод<10 Тогда
		ОбластьМакета.Область(1,22).ЦветТекста = WebЦвета.Синий;
	КонецЕсли;
...
Процедура ЗаполнитьСтруктуруПрочих(СтруктураЗначений)
	...
	СтруктураЗначений.Вставить("ОтклонениеСуммаМесяц",0);
	СтруктураЗначений.Вставить("ОтклонениеСуммаКвартал",0);
