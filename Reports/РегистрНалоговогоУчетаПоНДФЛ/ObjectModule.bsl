...
Процедура ПолучитьДанныеДляПостроенияОтчета(СписокФизЛицОрганизация)
	...
	|	ЕСТЬNULL(СтатусФизЛица.Статус, ЗНАЧЕНИЕ(Перечисление.СтатусыНалогоплательщиковПоНДФЛ.Резидент)) КАК Статус
...
Процедура СформироватьМакет(ДокументРезультат, ФизЛицо, Год, ГоловнаяОрганизация, Уровень) Экспорт
	...
	Если Год < ДатаПереходаНаКодыОКТМО Тогда
		...
		Для Каждого НалогиИтоги Из ТаблицаПоНалогамИтоги Цикл
			...
			Если СтрокиОКАТО.Количество() > 0 Тогда
				НалогиИтоги.ОКТМО_КПП = СоответствиеОКАТОКПП[0].ОКТМО_КПП;
			КонецЕсли;
		КонецЦикла;
	Иначе
	...
	Для Каждого НалогиИтоги Из ТаблицаПоНалогамИтоги Цикл
		...
		СальдоПоНалогам = СуммаНалогаИсчисленная - СуммаНалогаУдержанная - СуммаНалогаВзыскание;
...
Процедура ЗаполнитьКодыТерриторийВПараметрах(Выборка, Параметры, Год)
	...
	ИначеЕсли Год = ДобавитьМесяц(ДатаПереходаНаКодыОКТМО, -12) Тогда
		...
		Если СтрокиОКАТО.Количество() > 0 Тогда
			ОКТМО_КПП = СоответствиеОКАТОКПП[0].ОКТМО_КПП;
		КонецЕсли;
