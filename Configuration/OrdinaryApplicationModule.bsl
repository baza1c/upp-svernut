...
Процедура ПередНачаломРаботыСистемы(Отказ)
	...
	Если НЕ (РольДоступна("Пользователь") И РольДоступна("абс_Пользователь")) //АБС
...
Процедура ПриНачалеРаботыСистемы()
	...
	Если НЕ ВключенИнтерфейсКассира Тогда
		...
		Если УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(глЗначениеПеременной("глТекущийПользователь"), "АвтооткрытиеФормыРабочегоМестаМенеджераПоПродажамПриЗапускеПрограммы") = Истина Тогда
			Если ПравоДоступа("Использование", Метаданные.Обработки.РабочееМестоМенеджераПоПродажам) Тогда
				Если РольДоступна("ЛЕО_Медицина") Тогда
					Обработки.РабочееМестоМенеджераПоПродажам.ПолучитьФорму("ФормаCRM_Медицина").Открыть();
				Иначе
	...
	//Если ТекПользователь = Справочники.Пользователи.НайтиПоНаименованию("Зароднюк Владимир Васильевич") Тогда
	//	ИЛИ ТекПользователь = Справочники.Пользователи.НайтиПоНаименованию("Бессмертных Елена Викторовна") 
	//	ИЛИ ТекПользователь = Справочники.Пользователи.НайтиПоНаименованию("Глухова Светлана Леонидовна") 
	//	ИЛИ ТекПользователь = Справочники.Пользователи.НайтиПоНаименованию("Щербинина Анастасия Андреевна") 
	//	ИЛИ ТекПользователь = Справочники.Пользователи.НайтиПоНаименованию("Убоженко Юлия Меркурьевна") Тогда
	//	ПроверкаОбменаДаннымиАктСборкиМолком();
	//	ПодключитьОбработчикОжидания("ПроверкаОбменаДаннымиАктСборкиМолком", 4800);
	//КонецЕсли;
...
Процедура ПроверкаОбменаДаннымиАктСборкиМолком() Экспорт
	лео_РегЗадания.Лео_ЗагрузитьАктыСборки_Молком();	
КонецПроцедуры