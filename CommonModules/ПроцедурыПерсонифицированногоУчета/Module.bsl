...
Процедура ВывестиОписьАДВ6(ТабличныйДокумент,Знач ИнформацияОПачке,Знач ОтветственныеЛица, ЗаменятьБуквуЕ = Ложь) Экспорт
...
Функция ДоляСтавкиУказанаНеверно(ДоляСтавки)
...
Функция СЗВ4_СформироватьЗаголовокОшибкиЗаписиОСтаже(Запись, ОсновнаяЗапись)
	...
	ЗаголовокСообщения = ОбщегоНазначенияЗК.ПреобразоватьСтрокуИнтерфейса("Организация: ")+СокрЛП(Запись.ОбособленноеПодразделение);
	ЗаголовокСообщения = ЗаголовокСообщения + ", Период: "+Формат(ОсновнаяЗапись.ДатаНачалаПериода,"ДФ='дд.ММ.гггг'")+"-"+Формат(ОсновнаяЗапись.ДатаОкончанияПериода,"ДФ='дд.ММ.гггг'");
...
Функция ОписаниеОтчетногоПериодаДляПоказа(ОтчетныйПериод) Экспорт
	...
	Иначе
		Описание = СтрЗаменить(ПредставлениеПериода(ОтчетныйПериод, ПроцедурыПерсонифицированногоУчета.ОкончаниеОтчетногоПериодаПерсучета(ОтчетныйПериод), "ФП = Истина"),"квартал","кв.");
	КонецЕсли;
...
Процедура ПоместитьВМенеджерСоответствиеВидаЗастрахованныхКатегории(МенеджерВременныхТаблиц) Экспорт
...
Функция ПереходВРабочееМестоПФРвОтветНаВопрос(ДокументОбъект, ФормаДокумента, ТекстВопросаСохранения, ТекстВопросаДействия, ТекстКнопкиДействия, ЗаголовокОкнаВопроса, Организация, ОтчетныйПериод, ОтчетныйГод, ТипСведений) Экспорт
...
Функция ДанныеФизЛицаДляДокументаПерсучета(Физлицо, ДатаАктуальности, Организация, КатегорияЗастрахованныхЛиц, ОтчетныйПериод, ПолучатьАдрес = Ложь, ТипДоговора = Неопределено) Экспорт 
	...
	ОФизлице = ОбщегоНазначенияЗК.ПолучитьЗначенияРеквизитов(Физлицо,"СтраховойНомерПФР,Наименование");
...
Функция ДоначисленныеВзносыДляКорректирующегоДокумента(Организация, Дата, КорректируемыйПериод, КатегорияЗастрахованныхЛиц, ТаблицаФизлиц, ТипДоговора = Неопределено) Экспорт 
	...
	|	СуммыВзносов.НомерСтроки КАК НомерСтроки,
	|	СуммыВзносов.ФизЛицо,
	|	СуммыВзносов.НачисленоНакопительная - ЕСТЬNULL(ПачкаДокументовСЗВ_6_4РаботникиОрганизации.НачисленоНакопительная, 0) КАК ДоначисленоНакопительная,
	|	СуммыВзносов.НачисленоСтраховая - ЕСТЬNULL(ПачкаДокументовСЗВ_6_4РаботникиОрганизации.НачисленоСтраховая, 0) КАК ДоначисленоСтраховая,
...
Функция ВыполнитьНумерациюЗаписейОСтаже(ЗаписиОСтаже) Экспорт
