...
Процедура ФормаЭлементаОбновитьПредставлениеЭлемента(ИмяОбновляемогоЭлемента, ЭтаФорма, Параметр = Неопределено) Экспорт
	...
	Если ИмяОбновляемогоЭлемента = "Гражданство" Тогда
		...
		Если мНаборЗаписейГражданство.Количество() > 0 Тогда
			...
			Если мНаборЗаписейГражданство[0].НеИмеетПравоНаПенсию Тогда
			...
			Иначе
				Гражданство = Гражданство + " имеет право на пенсионное, медицинское и социальное страхование";
			КонецЕсли;
	...
	ИначеЕсли ИмяОбновляемогоЭлемента = "СтандартныеВычеты" Тогда	
		...
		Если Выборка.Следующий() Тогда
			...
			|	ЕСТЬNULL(МАКСИМУМ(ВЫБОР
			|				КОГДА НДФЛСтандартныеВычетыНаДетейСрезПоследних.КодВычета = ЗНАЧЕНИЕ(Справочник.ВычетыНДФЛ.Код116)
			|						И ВЫБОР
			|							КОГДА НДФЛСтандартныеВычетыНаДетейСрезПоследних.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
			|									И НДФЛСтандартныеВычетыНаДетейСрезПоследних.ПериодЗавершения < &парамДатаСреза
			|								ТОГДА НДФЛСтандартныеВычетыНаДетейСрезПоследних.КоличествоДетейЗавершения
			...
			|					ТОГДА ИСТИНА
			|				ИНАЧЕ ЛОЖЬ
			|			КОНЕЦ), ЛОЖЬ) КАК ЕстьВычетНаТретьегоРебенка,
			|	ЕСТЬNULL(МАКСИМУМ(ВЫБОР
			|				КОГДА НДФЛСтандартныеВычетыНаДетейСрезПоследних.КодВычета = ЗНАЧЕНИЕ(Справочник.ВычетыНДФЛ.Код116)
			|					ТОГДА ВЫБОР
			|							КОГДА НДФЛСтандартныеВычетыНаДетейСрезПоследних.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
			...
			|						КОНЕЦ
			|				ИНАЧЕ 0
			|			КОНЕЦ), ЛОЖЬ) КАК КоличествоТретьихДетей,
			|	ЕСТЬNULL(МАКСИМУМ(ВЫБОР
			|				КОГДА НДФЛСтандартныеВычетыНаДетейСрезПоследних.КодВычета = ЗНАЧЕНИЕ(Справочник.ВычетыНДФЛ.Код120)
			|						И ВЫБОР
			|							КОГДА НДФЛСтандартныеВычетыНаДетейСрезПоследних.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
			|									И НДФЛСтандартныеВычетыНаДетейСрезПоследних.ПериодЗавершения < &парамДатаСреза
			|								ТОГДА НДФЛСтандартныеВычетыНаДетейСрезПоследних.КоличествоДетейЗавершения
			...
			|					ТОГДА ИСТИНА
			|				ИНАЧЕ ЛОЖЬ
			|			КОНЕЦ), ЛОЖЬ) КАК ЕстьДвойнойВычетНаТретьегоРебенка,
			|	ЕСТЬNULL(МАКСИМУМ(ВЫБОР
			|				КОГДА НДФЛСтандартныеВычетыНаДетейСрезПоследних.КодВычета = ЗНАЧЕНИЕ(Справочник.ВычетыНДФЛ.Код120)
			|					ТОГДА ВЫБОР
			|							КОГДА НДФЛСтандартныеВычетыНаДетейСрезПоследних.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
			...
			|						КОНЕЦ
			|				ИНАЧЕ 0
			|			КОНЕЦ), ЛОЖЬ) КАК КоличествоДвойныхТретьихДетей,
			|	ЕСТЬNULL(МАКСИМУМ(ВЫБОР
			|				КОГДА НДФЛСтандартныеВычетыНаДетейСрезПоследних.КодВычета = ЗНАЧЕНИЕ(Справочник.ВычетыНДФЛ.Код124)
			|						И ВЫБОР
			|							КОГДА НДФЛСтандартныеВычетыНаДетейСрезПоследних.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
			|									И НДФЛСтандартныеВычетыНаДетейСрезПоследних.ПериодЗавершения < &парамДатаСреза
			|								ТОГДА НДФЛСтандартныеВычетыНаДетейСрезПоследних.КоличествоДетейЗавершения
			...
			|					ТОГДА ИСТИНА
			|				ИНАЧЕ ЛОЖЬ
			|			КОНЕЦ), ЛОЖЬ) КАК ЕстьВторойДвойнойВычетНаТретьегоРебенка,
			|	ЕСТЬNULL(МАКСИМУМ(ВЫБОР
			|				КОГДА НДФЛСтандартныеВычетыНаДетейСрезПоследних.КодВычета = ЗНАЧЕНИЕ(Справочник.ВычетыНДФЛ.Код124)
			|					ТОГДА ВЫБОР
			|							КОГДА НДФЛСтандартныеВычетыНаДетейСрезПоследних.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
			...
			|						КОНЕЦ
			|				ИНАЧЕ 0
			|			КОНЕЦ), ЛОЖЬ) КАК ВтороеКоличествоДвойныхТретьихДетей,
			|	ЕСТЬNULL(МАКСИМУМ(ВЫБОР
			...
			|					ТОГДА ИСТИНА
			|				ИНАЧЕ ЛОЖЬ
			|			КОНЕЦ), ЛОЖЬ) КАК ЕстьВычетНаВторогоРебенка,
			|	ЕСТЬNULL(МАКСИМУМ(ВЫБОР
			|				КОГДА НДФЛСтандартныеВычетыНаДетейСрезПоследних.КодВычета = ЗНАЧЕНИЕ(Справочник.ВычетыНДФЛ.Код115)
			|					ТОГДА ВЫБОР
			|							КОГДА НДФЛСтандартныеВычетыНаДетейСрезПоследних.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
			...
			|						КОНЕЦ
			|				ИНАЧЕ 0
			|			КОНЕЦ), ЛОЖЬ) КАК КоличествоВторыхДетей,
			|	ЕСТЬNULL(МАКСИМУМ(ВЫБОР
			|				КОГДА НДФЛСтандартныеВычетыНаДетейСрезПоследних.КодВычета = ЗНАЧЕНИЕ(Справочник.ВычетыНДФЛ.Код119)
			|						И ВЫБОР
			|							КОГДА НДФЛСтандартныеВычетыНаДетейСрезПоследних.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
			|									И НДФЛСтандартныеВычетыНаДетейСрезПоследних.ПериодЗавершения < &парамДатаСреза
			|								ТОГДА НДФЛСтандартныеВычетыНаДетейСрезПоследних.КоличествоДетейЗавершения
			...
			|					ТОГДА ИСТИНА
			|				ИНАЧЕ ЛОЖЬ
			|			КОНЕЦ), ЛОЖЬ) КАК ЕстьДвойнойВычетНаВторогоРебенка,
			|	ЕСТЬNULL(МАКСИМУМ(ВЫБОР
			|				КОГДА НДФЛСтандартныеВычетыНаДетейСрезПоследних.КодВычета = ЗНАЧЕНИЕ(Справочник.ВычетыНДФЛ.Код119)
			|					ТОГДА ВЫБОР
			|							КОГДА НДФЛСтандартныеВычетыНаДетейСрезПоследних.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
			...
			|						КОНЕЦ
			|				ИНАЧЕ 0
			|			КОНЕЦ), ЛОЖЬ) КАК КоличествоДвойныхВторыхДетей,
			|	ЕСТЬNULL(МАКСИМУМ(ВЫБОР
			|				КОГДА НДФЛСтандартныеВычетыНаДетейСрезПоследних.КодВычета = ЗНАЧЕНИЕ(Справочник.ВычетыНДФЛ.Код123)
			|						И ВЫБОР
			|							КОГДА НДФЛСтандартныеВычетыНаДетейСрезПоследних.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
			|									И НДФЛСтандартныеВычетыНаДетейСрезПоследних.ПериодЗавершения < &парамДатаСреза
			|								ТОГДА НДФЛСтандартныеВычетыНаДетейСрезПоследних.КоличествоДетейЗавершения
			...
			|					ТОГДА ИСТИНА
			|				ИНАЧЕ ЛОЖЬ
			|			КОНЕЦ), ЛОЖЬ) КАК ЕстьВторойДвойнойВычетНаВторогоРебенка,
			|	ЕСТЬNULL(МАКСИМУМ(ВЫБОР
			|				КОГДА НДФЛСтандартныеВычетыНаДетейСрезПоследних.КодВычета = ЗНАЧЕНИЕ(Справочник.ВычетыНДФЛ.Код123)
			|					ТОГДА ВЫБОР
			|							КОГДА НДФЛСтандартныеВычетыНаДетейСрезПоследних.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
			...
			|						КОНЕЦ
			|				ИНАЧЕ 0
			|			КОНЕЦ), ЛОЖЬ) КАК ВтороеКоличествоДвойныхВторыхДетей,
			|	ЕСТЬNULL(МАКСИМУМ(ВЫБОР
			...
			Выборка = Запрос.Выполнить().Выбрать();
			Выборка.Следующий();
			Если Выборка.ЕстьВычетНаДетей Или Выборка.ЕстьДвойнойВычетНаДетей Или Выборка.ЕстьВторойДвойнойВычетНаДетей Или 
				Выборка.ЕстьВычетНаТретьегоРебенка Или Выборка.ЕстьДвойнойВычетНаТретьегоРебенка Или Выборка.ЕстьВторойДвойнойВычетНаТретьегоРебенка Или 
				Выборка.ЕстьВычетНаВторогоРебенка Или Выборка.ЕстьДвойнойВычетНаВторогоРебенка Или Выборка.ЕстьВторойДвойнойВычетНаВторогоРебенка Или 
				Выборка.ЕстьВычетНаДетейИнвалидов Или Выборка.ЕстьВторойДвойнойВычетНаДетейИнвалидов Или Выборка.ЕстьДвойнойВычетНаДетейИнвалидов Тогда
				СтандартныеВычеты = СтандартныеВычеты + "Вычет на детей:";
				Если Выборка.ЕстьВычетНаДетей Тогда
					СтандартныеВычеты = СтандартныеВычеты + " с кодом 114/108 на " + Выборка.КоличествоДетей + ОбщегоНазначенияЗК.ФормаМножественногоЧисла(" ребенка,"," детей,"," детей,",Выборка.КоличествоДетей)
				КонецЕсли;
				Если Выборка.ЕстьДвойнойВычетНаДетей Тогда
					СтандартныеВычеты = СтандартныеВычеты + " с кодом 118/110 на " + Выборка.КоличествоДвойныхДетей + ОбщегоНазначенияЗК.ФормаМножественногоЧисла(" ребенка,"," детей,"," детей,",Выборка.КоличествоДвойныхДетей)
				КонецЕсли;
				Если Выборка.ЕстьВторойДвойнойВычетНаДетей Тогда
					СтандартныеВычеты = СтандартныеВычеты + " с кодом 122/111 на " + Выборка.ВтороеКоличествоДвойныхДетей + ОбщегоНазначенияЗК.ФормаМножественногоЧисла(" ребенка,"," детей,"," детей,",Выборка.ВтороеКоличествоДвойныхДетей)
				КонецЕсли;
				Если Выборка.ЕстьВычетНаВторогоРебенка Тогда
					СтандартныеВычеты = СтандартныеВычеты + " с кодом 115 на " + Выборка.КоличествоВторыхДетей + ОбщегоНазначенияЗК.ФормаМножественногоЧисла(" ребенка,"," детей,"," детей,",Выборка.КоличествоВторыхДетей)
				КонецЕсли;
				Если Выборка.ЕстьДвойнойВычетНаВторогоРебенка Тогда
					СтандартныеВычеты = СтандартныеВычеты + " с кодом 119 на " + Выборка.КоличествоДвойныхВторыхДетей + ОбщегоНазначенияЗК.ФормаМножественногоЧисла(" ребенка,"," детей,"," детей,",Выборка.КоличествоДвойныхВторыхДетей)
				КонецЕсли;
				Если Выборка.ЕстьВторойДвойнойВычетНаВторогоРебенка Тогда
					СтандартныеВычеты = СтандартныеВычеты + " с кодом 123 на " + Выборка.ВтороеКоличествоДвойныхВторыхДетей + ОбщегоНазначенияЗК.ФормаМножественногоЧисла(" ребенка,"," детей,"," детей,",Выборка.ВтороеКоличествоДвойныхВторыхДетей)
				КонецЕсли;
				Если Выборка.ЕстьВычетНаТретьегоРебенка Тогда
					СтандартныеВычеты = СтандартныеВычеты + " с кодом 116 на " + Выборка.КоличествоТретьихДетей + ОбщегоНазначенияЗК.ФормаМножественногоЧисла(" ребенка,"," детей,"," детей,",Выборка.КоличествоТретьихДетей)
				КонецЕсли;
				Если Выборка.ЕстьДвойнойВычетНаТретьегоРебенка Тогда
					СтандартныеВычеты = СтандартныеВычеты + " с кодом 120 на " + Выборка.КоличествоДвойныхТретьихДетей + ОбщегоНазначенияЗК.ФормаМножественногоЧисла(" ребенка,"," детей,"," детей,",Выборка.КоличествоДвойныхТретьихДетей)
				КонецЕсли;
				Если Выборка.ЕстьВторойДвойнойВычетНаТретьегоРебенка Тогда
					СтандартныеВычеты = СтандартныеВычеты + " с кодом 124 на " + Выборка.ВтороеКоличествоДвойныхТретьихДетей + ОбщегоНазначенияЗК.ФормаМножественногоЧисла(" ребенка,"," детей,"," детей,",Выборка.ВтороеКоличествоДвойныхТретьихДетей)
				КонецЕсли;
				Если Выборка.ЕстьВычетНаДетейИнвалидов Тогда
					СтандартныеВычеты = СтандартныеВычеты + " с кодом 109 на " + Выборка.КоличествоДетейИнвалидов + ОбщегоНазначенияЗК.ФормаМножественногоЧисла(" ребенка,"," детей,"," детей,",Выборка.КоличествоДетейИнвалидов)
				КонецЕсли;
				Если Выборка.ЕстьДвойнойВычетНаДетейИнвалидов Тогда
					СтандартныеВычеты = СтандартныеВычеты + " с кодом 112 на " + Выборка.КоличествоДвойныхДетейИнвалидов + ОбщегоНазначенияЗК.ФормаМножественногоЧисла(" ребенка,"," детей,"," детей,",Выборка.КоличествоДвойныхДетейИнвалидов)
				КонецЕсли;
				Если Выборка.ЕстьВторойДвойнойВычетНаДетейИнвалидов Тогда
					СтандартныеВычеты = СтандартныеВычеты + " с кодом 113 на " + Выборка.ВтороеКоличествоДвойныхДетейИнвалидов + ОбщегоНазначенияЗК.ФормаМножественногоЧисла(" ребенка,"," детей,"," детей,",Выборка.ВтороеКоличествоДвойныхДетейИнвалидов)
				КонецЕсли;
