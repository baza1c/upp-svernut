...
Процедура УстановитьЗаголовокФормыДокумента(СтрокаВидаОперации = "", ДокументОбъект, ФормаДокумента) Экспорт
	...
	//АБС ВСТАВКА
	Если ДокументОбъект.Метаданные().Реквизиты.Найти("абс_Статус") <> Неопределено Тогда
		Заголовок = Заголовок + Строка(ДокументОбъект.абс_Статус) + ". ";
	КонецЕсли;
	//АБС КОНЕЦ
...
//{Лео Катанович
Процедура СформироватьНадписьГосконтракт(ДоговорКонтрагента, ЭлементФормыНадписьГосконтракт, 
	                                 СсылкаДокумент = Неопределено) Экспорт
	Если ТипЗнч(ДоговорКонтрагента) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
		ЭлементФормыНадписьГосконтракт.Заголовок = ?(ЗначениеЗаполнено(ДоговорКонтрагента.Лео_ИГК),"Госконтракт: " + ДоговорКонтрагента.Лео_ИГК,"");
	Иначе
		ЭлементФормыНадписьГосконтракт.Заголовок = "";
	КонецЕсли; 
...
Процедура ВвестиСчетФактуру(ДокументОбъект, ФормаДокумента, ВидСчетаФактуры = "СчетФактураВыданный", Отбор = Неопределено) Экспорт
	...
	Если (Не ЗначениеЗаполнено(СчетФактура))
	   И (Не ДокументОбъект.ПометкаУдаления) Тогда
		...
		//{Лео Катанович
		Если ТипЗнч(ОбъектСчетаФактуры.Ссылка) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда
		     ОбъектСчетаФактуры.ИдентификаторГосКонтракта =  ДокументОбъект.ДоговорКонтрагента.Лео_ИГК.Код;
		КонецЕсли;
		//Лео Катанович}
...
Функция ПолучитьТекстДолга(ДоговорКонтрагента, Знач Долг)
	...
	Иначе
		...
		Иначе
			ТекстПодписи = ТекстПодписи + " долг "+СокрЛП(ДоговорКонтрагента.Организация.Наименование);
			Долг = (-1) * Долг;
		КонецЕсли;
...
//АБС ВСТАВКА 38466  10.02.2014 11:07:33  Манжела
// Заполняет список пунктов подменю выбора статусов значениями переданной коллекции объектов метаданных,
// соответствующей перечислению видов операций для данного документа.
// Всем кнопкам назначается одно переданное действие.
// Обычно используется для заполнения пунктов подменю "ПодменюВидаОперации" командной панели
// формы документа значениями перечисления "Вид операции" для данного вида документа
...
//  ОбъектОбработкиВыбораОперации - действие, которое надо выполнить при выборе любого пункта подменю.
//
Процедура абс_УстановитьПодменюВыбораСтатусов(ЭлементМеню, ОбъектЗаполнения, ОбъектОбработкиВыбораОперации) Экспорт
	...
	Для Каждого ЭлементЗаполнения Из ОбъектЗаполнения Цикл
		...
		ПунктМеню = ЭлементМеню.Кнопки.Добавить(
			ОбщегоНазначенияЗК.ПолучитьИмяЭлементаПеречисленияПоЗначению(ЭлементЗаполнения),
			ТипКнопкиКоманднойПанели.Действие,
			ЭлементЗаполнения,
			ОбъектОбработкиВыбораОперации);
