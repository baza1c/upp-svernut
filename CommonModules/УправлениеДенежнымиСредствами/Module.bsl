...
Функция ПолучитьТаблицуПлатежейУпр(ДатаДокумента,ВалютаДокумента,Документ, ВидПлатежногоДокумента) Экспорт
	...
	|	РасшифровкаПлатежаДок.Лео_СтатьяДвиженияДенежныхСредств КАК Лео_СтатьяДвиженияДенежныхСредств,
	|	РасшифровкаПлатежаДок.Лео_СОБР КАК Лео_СОБР,
...
Процедура ПровестиСписаниеДенежныхСредствУпр(СтруктураШапкиДокумента, СтруктураПараметров, ТаблицаПлатежейУпр, Движения, Отказ, Заголовок) Экспорт
	...
	// лео никитин движение по регистру Заявки лео
	ДвиженияПоЗаявкам_лео      = ТаблицаПлатежейУпр.Скопировать();
	////
	ДвиженияПоРезерву      = ТаблицаПлатежейУпр.Скопировать();
	ДвиженияПоКонтрагентам = ТаблицаПлатежейУпр.Скопировать();
	
	ДвиженияПоЗаявкам.Свернуть("ДокументПланированияПлатежа,ВключатьВПлатежныйКалендарь,ДоговорКонтрагента,ВедениеВзаиморасчетов,ВестиПоДокументамРасчетовСКонтрагентом,Сделка,ДокументРасчетовСКонтрагентом,СтатьяДвиженияДенежныхСредств,лео_СтатьяДвиженияДенежныхСредств,Лео_СОБР,Проект","СуммаПлатежа,СуммаВзаиморасчетов,СуммаПлатежаПлан,СуммаУпр");
	
	//лео никитин движение по регистру заявки лео
	ДвиженияПоЗаявкам_лео.Свернуть("ДокументПланированияПлатежа,ВключатьВПлатежныйКалендарь,ДоговорКонтрагента,ВедениеВзаиморасчетов,ВестиПоДокументамРасчетовСКонтрагентом,Сделка,ДокументРасчетовСКонтрагентом,лео_СтатьяДвиженияДенежныхСредств,Лео_СОБР,Проект","СуммаПлатежа,СуммаВзаиморасчетов,СуммаПлатежаПлан,СуммаУпр");
	...
	Если ОтраженоВОперУчете И НЕ ПоРасчетномуДокументу И НЕ ПоОбъявлениюНаВзносНаличными Тогда
		...
		/// лео никтин проводки по регистру заявки _лео
		ТаблицаЗаявкиНаРасходованиеСредств_лео = Движения.ЛЕО_ЗаявкиНаРасходованиеСредств.ВыгрузитьКолонки();
        //                                      
		...
		Для Каждого СтрокаПлатеж ИЗ ДвиженияПоЗаявкам Цикл
			...
			//АБС ИЗМЕНЕНИЕ Бюджетирование  10.10.2014 16:57:46  Тест
			//Если НЕ СтрокаПлатеж.ДокументПланированияПлатежа.Пустая() Тогда
			//АБС ИЗМЕНЕНИЕ Бюджетирование КОНЕЦ 
				...
				//АБС ИЗМЕНЕНИЕ Бюджетирование  10.10.2014 17:40:21  Тест
				//СтрокаДвиженийЗаявки.Сумма							= СтрокаПлатеж.СуммаПлатежаПлан;
				СтрокаДвиженийЗаявки.Сумма							= СтрокаПлатеж.СуммаПлатежа;
				//АБС ИЗМЕНЕНИЕ Бюджетирование КОНЕЦ 
				...
				СтрокаДвиженийЗаявки.СтатьяДвиженияДенежныхСредств	= СтрокаПлатеж.Лео_СтатьяДвиженияДенежныхСредств;
				...
				//АБС ИЗМЕНЕНИЕ Бюджетирование  10.10.2014 17:32:20  Тест
				Если НЕ СтрокаПлатеж.ДокументПланированияПлатежа.Пустая() Тогда
					...
					КонецЕсли;
				КонецЕсли;
				 ////////лео никитин проводки по регистру заявки лео
				СуммаПлатежа = СтрокаПлатеж.СуммаПлатежаПлан;
				
				СтрокаДвиженийЗаявки_лео = ТаблицаЗаявкиНаРасходованиеСредств_лео.Добавить();     
				
				СтрокаДвиженийЗаявки_лео.СуммаУпр						= СтрокаПлатеж.СуммаУпр;
					СтрокаДвиженийЗаявки_лео.Сумма							= СтрокаПлатеж.СуммаПлатежа;
		
				СтрокаДвиженийЗаявки_лео.СуммаВзаиморасчетов			= СтрокаПлатеж.СуммаВзаиморасчетов;
				СтрокаДвиженийЗаявки_лео.ЗаявкаНаРасходование			= СтрокаПлатеж.ДокументПланированияПлатежа;
				СтрокаДвиженийЗаявки_лео.ДоговорКонтрагента				= СтрокаПлатеж.ДоговорКонтрагента;
			
				СтрокаДвиженийЗаявки_лео.СтатьяДвиженияДенежныхСредств	= СтрокаПлатеж.Лео_СтатьяДвиженияДенежныхСредств;
				СтрокаДвиженийЗаявки_лео.Лео_СтатьяОборотов            =  СтрокаПлатеж.Лео_СОБР;
                                         
				СтрокаДвиженийЗаявки_лео.Проект							= СтрокаПлатеж.Проект;
				...
				КонецЕсли;
				СтрокаДвиженийЗаявки_лео.Сделка = СтрокаПлатеж.Сделка;
				Если СтрокаПлатеж.ВестиПоДокументамРасчетовСКонтрагентом Тогда
					СтрокаДвиженийЗаявки_лео.ДокументРасчетовСКонтрагентом = СтрокаПлатеж.ДокументРасчетовСКонтрагентом;
				КонецЕсли;
		...
		Если ТаблицаЗаявкиНаРасходованиеСредств.Количество() > 0 Тогда
			...
			Движения.ЛЕО_ЗаявкиНаРасходованиеСредств.мПериод          = Дата;
			Движения.ЛЕО_ЗаявкиНаРасходованиеСредств.мТаблицаДвижений = ТаблицаЗаявкиНаРасходованиеСредств_лео;
          	Движения.ЛЕО_ЗаявкиНаРасходованиеСредств.ВыполнитьРасход();
