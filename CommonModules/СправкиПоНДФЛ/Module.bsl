...
Процедура ПрочитатьДанныеСправки(ОписаниеСправки, СведенияОДоходах, СведенияОВычетах, СуммыНалогов, НалоговыйПериод, Организация, ГоловнаяОрганизация, ОКАТО_КПП, Дата) Экспорт
...
Функция КодНалоговогоОрганаПоОКАТО_КППиОрганизации(Организация, ОрганизацияЭтоЮрЛицо, ОКАТО_КПП) Экспорт
	...
	Если ОрганизацияЭтоЮрЛицо И ЗначениеЗаполнено(ОКАТО_КПП) Тогда
		ДанныеОрганизации = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(Организация,"КодИМНС,КПП");
		...
		КонецЕсли;
	Иначе 	
		КодНО = ОбщегоНазначения.ПолучитьЗначениеРеквизита(Организация,"КодИМНС");
	КонецЕсли;
...
Функция ОписаниеДоходовПоКодам(Организация, ГоловнаяОрганизация, ОрганизацияЭтоЮрЛицо, НалоговыйПериод, ДатаСведений, ФизЛицо = Неопределено, Ставка = Неопределено) Экспорт
	...
	Возврат СправкиПоНДФЛПолныеПрава.ОписаниеДоходовОрганизации(Организация, ГоловнаяОрганизация, ОрганизацияЭтоЮрЛицо, НалоговыйПериод, ДатаСведений, ФизЛицо, Ставка)
...
Процедура ПроверитьОбщиеДанныеСправки(СтруктураДанных, Отказ, ОбработкаКомментариев) Экспорт
...
Функция ДанныеНалоговогоАгентаДля2НДФЛ(Организация, НалоговыйПериод, ОКАТО_КПП, КодИФНС, Дата, Телефон, ДолжностьПодписавшегоЛица, СправкуПодписал) Экспорт
	...
	Иначе
		...
		Иначе
			...
			Имя = "";
			Отчество = "";
			ОбщегоНазначенияЗК.ФамилияИнициалыФизЛица(ОбщегоНазначения.ПолучитьЗначениеРеквизита(ДанныеОрганизации.ИндивидуальныйПредприниматель,"Наименование"),Фамилия,Имя,Отчество);
		КонецЕсли;
...
Функция ПечатьСправок2НДФЛ(Ссылка, СтруктураДанныхНА, ТаблицаДанныхСотрудников, ТаблицаДанныхОДоходах, ТаблицаДанныхОВычетах, ТаблицаИтоговыхДанных) Экспорт
	...
	Если ДанныеДокумента.НалоговыйПериод = 2010 Или (ДанныеДокумента.НалоговыйПериод = 2011 И ДанныеДокумента.СпециальныйДокумент2011года) Тогда
	...
	Иначе
		ТабДокумент = Печать2011(Отказ, ДанныеДокумента.НалоговыйПериод, СтруктураДанныхНА, ТаблицаДанныхСотрудников, ТаблицаДанныхОДоходах, ТаблицаДанныхОВычетах, ТаблицаИтоговыхДанных, СоответствиеКодовСтран, СоответствиеИменСтран, СоответствиеДокументовУдостоверяющиеЛичность, СоответствиеКодовРегионов, СоответствиеКодовДоходов, СоответствиеКодовВычетов, СоответствиеСтавокДоходов)
	КонецЕсли;
