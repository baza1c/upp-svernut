	...
	Иначе	
		...
		Если ЕстьРеквизитПроцентСкидкиНаценки Тогда
			...
			Иначе
				...
				Иначе
					ПроцентАвтоматическихСкидок = 0;
					//+Лео Всеи скидки учтены в заказе
					//Если ЕстьРеквизитПроцентАвтоматическихСкидок Тогда
					//	ПроцентАвтоматическихСкидок = СтрокаТабличнойЧасти.ПроцентАвтоматическихСкидок;
					//КонецЕсли;
					...
					Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.РеализацияТоваровУслуг")Тогда
						Если Не ТипЗнч(ДокументОбъект.Сделка)= Тип("ДокументСсылка.ЗаказПокупателя") Тогда
							Если ЕстьРеквизитПроцентАвтоматическихСкидок Тогда
								ПроцентАвтоматическихСкидок = СтрокаТабличнойЧасти.ПроцентАвтоматическихСкидок;
							КонецЕсли;
							СуммаБезСкидок  = СтрокаТабличнойЧасти.Сумма * 100/
	...
	Если ПроверятьРеквизит Тогда
		...
		Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("ЕдиницаИзмеренияМест", МетаданныеДокумента, ИмяТабличнойЧасти)  Тогда
			Если НЕ ЗначениеЗаполнено(СтрокаТабличнойЧасти.ЕдиницаИзмеренияМест) Тогда
				//АБС ИЗМЕНЕНИЕ   24.03.2014 8:45:47  Балашенко
				Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ЗаказПокупателя") Или ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.РеализацияТоваровУслуг") Тогда
					абс_Дистрибуция.ЗаполнитьЕдиницуМестТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект);
				Иначе
					// берем из самого реквизита Номенклатура
			...
			Иначе
				...
				Иначе
					...
					Если Цел(КоличествоМест)<>КоличествоМест Тогда
						...
						КонецЕсли;
						//АБС ИЗМЕНЕНИЕ   08.05.2014 9:21:12  Балашенко
						//для Заказа покупателя оставляем дробное количество мест, если в единице измерения установлен реквизит абс_РазрешатьДробноеКоличествоМест
						КоличествоМестОригинал = КоличествоМест;
						Если НЕ (ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ЗаказПокупателя") И ЕдиницаИзмеренияМест.абс_РазрешатьДробноеКоличествоМест) И 
							 НЕ (ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.РеализацияТоваровУслуг") И ЕдиницаИзмеренияМест.абс_РазрешатьДробноеКоличествоМест) Тогда
						...
						//АБС ИЗМЕНЕНИЕ  КОНЕЦ
						//+ Лаврухин 30.03.2015
						Если  (ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ПеремещениеТоваров") И ЕдиницаИзмеренияМест.абс_РазрешатьДробноеКоличествоМест) Тогда
							Если ЕдиницаИзмеренияМест.ПорогОкругления = 0 Тогда
								//Округление в сторону уменьшения
								КоличествоМест = КоличествоМестОригинал;								
							Иначе
								//расчет количества в базовых единицах, не поместившегося в целое количество мест - ОстатокВБазовыхЕИ
								КоличествоВБазовыхЕИ = СтрокаТабличнойЧасти.Количество * СтрокаТабличнойЧасти.Коэффициент;
								ОстатокВБазовыхЕИ = КоличествоВБазовыхЕИ - Цел(КоличествоМест)*ЕдиницаИзмеренияМест.Коэффициент;
