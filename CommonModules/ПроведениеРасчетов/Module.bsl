...
Функция ПолучитьДанныеНДФЛПоРегистратору(Знач ПервыйМесяцНалоговогоПериода,
	...
	|		И НЕ ДополнительныеВычеты.КодВычета В (ЗНАЧЕНИЕ(Справочник.ВычетыНДФЛ.Код311), ЗНАЧЕНИЕ(Справочник.ВычетыНДФЛ.Код312), ЗНАЧЕНИЕ(Справочник.ВычетыНДФЛ.Код318))) КАК Вычеты
	|
	|СГРУППИРОВАТЬ ПО
	|	Вычеты.ФизЛицо,
	|	Вычеты.МесяцНалоговогоПериода,
	...
	|					ВТСписокФизЛиц КАК СписокСотрудников)
	|		И ДополнительныеВычеты.МесяцНалоговогоПериода МЕЖДУ &НачалоГодаПериодаРасчета И &КонецПериодаРасчета
	|		И ДополнительныеВычеты.КодВычета В (ЗНАЧЕНИЕ(Справочник.ВычетыНДФЛ.Код311), ЗНАЧЕНИЕ(Справочник.ВычетыНДФЛ.Код312), ЗНАЧЕНИЕ(Справочник.ВычетыНДФЛ.Код318))) КАК Вычеты
	|
	|СГРУППИРОВАТЬ ПО
	|	Вычеты.ФизЛицо,
	|	Вычеты.МесяцНалоговогоПериода,
	...
	|	ИЗ
	|		ВТПериодыФизлиц КАК ПериодыФизлиц
	|			ЛЕВОЕ СОЕДИНЕНИЕ ВТДополнительныеДоходы КАК КорректировкаДанныхРегистратора
	...
	|				И (КорректировкаДанныхРегистратора.КодДохода.СтавкаНалогообложенияРезидента = ЗНАЧЕНИЕ(Перечисление.НДФЛСтавкиНалогообложенияРезидента.Ставка13))
	|				И (НЕ КорректировкаДанныхРегистратора.КодДохода.НеОблагаетсяУНалоговогоАгента)) КАК Доходы
	|
	|СГРУППИРОВАТЬ ПО
	|	Доходы.ФизЛицо,
	|	Доходы.МесяцНалоговогоПериода,
	...
	|		НАЧАЛОПЕРИОДА(ДополнительныеНалоги.МесяцНалоговогоПериода, МЕСЯЦ),
...
Процедура СформироватьРасчетыПоНДФЛПриВыплате(ВыборкаПоШапкеДокумента, ТекстТаблицыВыплаты = Неопределено, НаборЗаписейНДФЛ = Неопределено, НаборЗаписейРасчетыНА = Неопределено, СтруктураПараметровОтбора = Неопределено) Экспорт
	...
	|		ПО ЗарплатаКВыплатеОрганизацииЗарплата.ФизЛицо = НДФЛРасчетыСБюджетомПоМесяцам.ФизЛицо
	|ГДЕ
	|	ЕСТЬNULL(НДФЛРасчетыСБюджетомВсего.СальдоПоНалогу, 0) > 0
	...
	Пока Выборка.СледующийПоЗначениюПоля("Физлицо") Цикл 
		...
		Пока Выборка.СледующийПоЗначениюПоля("СтавкаНалогообложенияРезидента") Цикл
			...
			Пока Выборка.СледующийПоЗначениюПоля("МесяцНалоговогоПериода") И СуммаКВыплате > 0 Цикл
				...
				Если Выборка.СальдоПоНалогуЗаМесяц > 0 Тогда
					...
					Если СальдоПоНалогуЗаМесяц > 0 Тогда
						ПогашаемаяСумма = ПогашаемаяСумма + Мин(СуммаКВыплате, СальдоПоНалогуЗаМесяц);
						СуммаКВыплате = Макс(0,СуммаКВыплате - СальдоПоНалогуЗаМесяц);
					КонецЕсли;
					
					Если ПогашаемаяСумма > 0 Тогда
						// движения по удержанию в пределах выплаченных сумм
						ДвиженияПоУдержанию(НаборЗаписейНДФЛ, ПогашаемаяСумма, Выборка, ВыборкаПоШапкеДокумента);
					КонецЕсли;
				...
				Иначе
					...
					ДвиженияПоУдержанию(НаборЗаписейНДФЛ, Выборка.СальдоПоНалогуЗаМесяц, Выборка, ВыборкаПоШапкеДокумента);
...
Процедура СвернутьДвиженияНДФЛСведенияОДоходах(ДвиженияНДФЛСведенияОДоходах) Экспорт
	...
	МетаданныеРегистра = Метаданные.РегистрыНакопления.НДФЛСведенияОДоходах;
	СтрокаСуммирования = "";
...
Процедура ВписатьПромежуточныйНДФЛвДокумент(ТЧпоНДФЛ, ДанныеНДФЛ, ДатаПолученияДохода, ВсеМесяцыПолученияДохода) Экспорт
	...
	ТекущийНДФЛ = ДанныеНДФЛ.Скопировать(ДанныеНДФЛ.НайтиСтроки(Новый Структура("МесяцНалоговогоПериода",ТекущийПериод)));
	...
	Для каждого СтрокаДанных Из ДанныеНДФЛ Цикл  // налоги предыдущих периодов добавляем в пределах положительного налога текущего периода, не допуская возврата налога
		Если ВсеМесяцыПолученияДохода.Найти(СтрокаДанных.МесяцНалоговогоПериода) <> Неопределено Тогда
...
Процедура УточнитьСторнируемыеДокументы(ТаблицаСторнируемыхДокументов, ТаблицаПромежуточныхИсправлений)
	...
	Для каждого СтрокаТЗ Из ТаблицаСторнируемыхДокументов Цикл
		...
		Иначе
			...
			СледующаяСсылка = ОбщегоНазначения.ПолучитьЗначениеРеквизита(ПредыдущаяСсылка, "ПерерассчитываемыйДокумент");
