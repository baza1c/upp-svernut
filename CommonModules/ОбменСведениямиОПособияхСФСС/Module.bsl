...
Процедура ДобавитьСтрокуВРеестр(Узел, ФорматСтроки, СтруктураДанных)
	...
	СтруктураАдреса = УправлениеКонтактнойИнформацией.ПолучитьСтруктуруАдресаИзСтроки(СтруктураДанных.АдресРегистрации);	
	Если СтруктураДанных.ВидВыплаты = "1" Или СтруктураДанных.ВидВыплаты = "6" Тогда
	...
	Если СтруктураДанных.ВидВыплаты = "4" Или СтруктураДанных.ВидВыплаты = "5" Тогда
		НаборЗаписей.CERT_NO.Значение = ?(Не ЗначениеЗаполнено(СтруктураДанных.ДокументОРожденииСерияНомер),"00000000",СтруктураДанных.ДокументОРожденииСерияНомер);
		НаборЗаписей.CERT_DT.Значение = ?(Не ЗначениеЗаполнено(СтруктураДанных.ДокументОРожденииДатаВыдачи),СтруктураДанных.ДатаРожденияРебенка,СтруктураДанных.ДокументОРожденииДатаВыдачи);
...
Функция ИмяФайла(ДокументОбъект) Экспорт
	...
	Возврат "E_" + ДокументОбъект.РегистрационныйНомерФСС + "_" + Формат(ДокументОбъект.Дата,"ДФ=yyyy_MM_dd") + "_" + Прав("0" + ОбщегоНазначенияЗК.ПолучитьНомерНаПечать(ДокументОбъект),2);
...
Функция СформироватьФайлРеестра(ДокументОбъект, Отказ, ПроверятьДанные = Истина, ВыборкаПоШапкеДокумента = Неопределено, ВыборкаПоРаботникиОрганизации = Неопределено) Экспорт
	...
	Атрибуты.Вставить("version", "1.7.1");
	Атрибуты.Вставить("software", Лев("1С:" + Метаданные.Синоним, 255));
...
Функция ПечатьЗаявлениеВФССОВыплатеПособия(СтруктураДанных, ИмяМакета = "ЗаявлениеВФССОВыплатеПособия_2011") Экспорт
	...
	ФормированиеПечатныхФормЗК.ВывестиДанныеПоБуквенно(НаименованиеРаботодателяИВидЗанятости, Макет, "РаботодательИВидЗанятости_", 78);
	...
	Если Макет.Параметры.НеполноеРабочееВремя Тогда
		ФормированиеПечатныхФормЗК.ВывестиДанныеПоБуквенно(Формат(СтруктураДанных.ДоляВремени, "ЧЦ=3; ЧДЦ=0; ЧС=-2; ЧН=' '; ЧВН="), Макет, "Ставка_", 3);
	КонецЕсли;
...
Процедура ПриНачалеРаботыСистемы() Экспорт
	...
	Если Не РезультатПроверки.ПроверкаПрошлаУспешно Тогда
		...
		Форма.ЭлементыФормы.НадписьДействияПользователя.Заголовок = 
			...
			+"где в настоящее время проводится Пилотный проект ФСС по непосредственной выплате пособий."
