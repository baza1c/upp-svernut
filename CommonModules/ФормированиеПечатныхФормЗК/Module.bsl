...
Функция РасчетСреднегоЗаработкаПособий2011(ДокументОбъект, ВыводитьСамостоятельныйОтчет = Истина, СтруктураПоказателей = Неопределено, ОсобоеОписаниеОплачиваемогоПериода = "") Экспорт
	...
	Если ВыводитьИсключаемыеДни Тогда
		...
		Величина1 = РегистрыСведений.ПредельнаяВеличинаБазыСтраховыхВзносов.ПолучитьПоследнее(ДобавитьМесяц(НачалоГода(ДатаСобытия),-12) - 1).Размер;
		Величина2 = РегистрыСведений.ПредельнаяВеличинаБазыСтраховыхВзносов.ПолучитьПоследнее(НачалоГода(ДатаСобытия) - 1).Размер;
