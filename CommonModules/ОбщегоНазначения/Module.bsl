...
Процедура ДобавитьПрефиксОрганизации(ДокументОбъект, Префикс) Экспорт
	...
	Если ДокументОбъект.Метаданные().Реквизиты.Найти("Организация") <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ПрефиксОрганизации) Тогда
			...
			//ЛЕО никитин
			Если ТипЗнч(ДокументОбъект.Ссылка) = Тип("ДокументСсылка.ПеремещениеТоваров") и (ДокументОбъект.Дата >='20230101' )  Тогда 	
...
//АБС ВСТАВКА   27.02.2014 15:09:53  Шамов
...
Функция ПолучитьНомерНаПечать(Документ, МассивПрефиксов = Неопределено) Экспорт
	...
	//АБС Изменение Балашенко 04.06.2014
	...
	//+Лео Сафонов ЕА
	// С 2016г префиксы и лидирующие нцли не обрезаем
	Попытка
	     ДатаДо2016 = Документ.Дата < Дата(2016,01,01,00,00,00) 
	Исключение
		Попытка
			ДатаДо2016 =  Документ.ДатаДокумента < Дата(2016,01,01,00,00,00)
		Исключение
			//Форма М4 ПоступлениеТоваровУслуг
		    ДатаДо2016 =  Документ.ДатаСоставления < Дата(2016,01,01,00,00,00)
		КонецПопытки;
	КонецПопытки;	 
	
	Если ДатаДо2016 Тогда
	//-Лео Сафонов ЕА
		...
		Если ЕстьСимвол Тогда
			Если НомерСимвола = 1 Тогда
				Возврат Номер;
			Иначе
				ЧисловойНомер =  Число(Сред(Номер,СтрДлина(Номер)-НомерСимвола+2,НомерСимвола-1));
				Номер = СтрЗаменить(СокрЛП(ЧисловойНомер),Символы.НПП,"");
			КонецЕсли;
		Иначе
...
//перенесено для совместимости для помска ИНН
...
Процедура УдалитьДанныеИзБезопасногоХранилища(Владелец, Ключи = Неопределено) Экспорт
	...
	Если ТипЗнч(БезопасноеХранилищеДанных.Данные) = Тип("ХранилищеЗначения") Тогда
		...
		Если Ключи <> Неопределено И ТипЗнч(ДанныеДляСохранения) = Тип("Структура") Тогда
			СписокКлючей = СТ_СтрРазделить(Ключи, ",", Ложь);
...
//замещение 8.3.6
Функция СТ_СтрРазделить(Знач ИсходнаяСтрока, Знач Разделитель, ВключатьПустые = Истина) 
