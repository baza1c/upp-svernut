...
Процедура ИзменитьЦенуПоУсловиюПродаж(Дата, Номенклатура, УсловиеПродаж, ПолученнаяЦена) Экспорт
	...
	Если ПолученнаяЦена <> 0 И ЗначениеЗаполнено(УсловиеПродаж) Тогда
		...
		//АБС ИЗМЕНЕНИЕ 39049  18.02.2014 18:14:47  Шамов
		Запрос.Текст = "ВЫБРАТЬ
		               |	НаценкиПоУсловиямПродаж.абс_СуммаНаценки КАК СуммаНаценки,
		               |	НаценкиПоУсловиямПродаж.ПроцентНаценки КАК ПроцентНаценки
		               |ИЗ
		               |	РегистрСведений.НаценкиПоУсловиямПродаж.СрезПоследних(
		               |			&Дата,
		               |			УсловиеПродаж = &УсловиеПродаж
		               |				И НоменклатурнаяЦеноваяГруппа В (&Группы)) КАК НаценкиПоУсловиямПродаж";
 	 	//АБС ИЗМЕНЕНИЕ 39049 КОНЕЦ

		Выборка = Запрос.Выполнить().Выбрать();
		Если Выборка.Следующий() Тогда
			//АБС ИЗМЕНЕНИЕ 39049  18.02.2014 18:14:47  Шамов
			//ПолученнаяЦена = ПолученнаяЦена * (1 + Выборка.ПроцентНаценки / 100);
			Если Выборка.СуммаНаценки <> 0 Тогда
				ПолученнаяЦена = ПолученнаяЦена + Выборка.СуммаНаценки;
			Иначе
				ПолученнаяЦена = ПолученнаяЦена * (1 + Выборка.ПроцентНаценки / 100);
			КонецЕсли;
 	 		//АБС ИЗМЕНЕНИЕ 39049 КОНЕЦ
...
Функция ПолучитьМассивГруппНоменклатуры(Номенклатура)
	...
	//АБС ВСТАВКА 39049  18.02.2014 18:14:47  Шамов
	Группы.Добавить(Номенклатура);
	Группы.Добавить(Неопределено);
	//АБС ВСТАВКА 39049 КОНЕЦ
	Возврат Группы;
...
Функция ПолучитьЦенуНоменклатуры(Номенклатура, ХарактеристикаНоменклатуры = Неопределено, Знач ТипЦен, ДатаПараметр = Неопределено, ЕдиницаИзмерения = Неопределено,
	...
	Если ДоговорКонтрагента <> Неопределено Тогда
		...
		//АБС ИЗМЕНЕНИЕ Балашенко 23.06.2014
		Если ПолученнаяЦена = 0 И Не ЗначениеЗаполнено(ТипЦен) Тогда
			Запрос = Новый Запрос;
			Запрос.УстановитьПараметр("Дата", Дата);
			...
			Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
			               ...
			               |				И НоменклатурнаяЦеноваяГруппа В (&Группы)
			               |				И абс_ДоговорКонтрагента = &ДоговорКонтрагента) КАК ТипыЦенПоГруппам";
...
Функция СформироватьЗапросПоЦенам(СтруктураЗначений, СпособЗаполненияЦен, МассивНоменклатуры,
	...
	Если СпособЗаполненияЦен = Перечисления.СпособыЗаполненияЦен.ПоЦенамНоменклатуры Тогда
		...
		//АБС ВСТАВКА 39049  18.02.2014 18:19:23  Шамов
		|	ЕСТЬNULL(НаценкиПоУсловиямПродаж.абс_СуммаНаценки, 0.00)     КАК СуммаНаценкиПоУсловиямПродаж,
		//АБС ВСТАВКА 39049 КОНЕЦ
		...
		//АБС ВСТАВКА 39049  18.02.2014 18:19:23  Шамов
		|	ИЛИ БазовыеЦены.Номенклатура = НаценкиПоУсловиямПродаж.НоменклатурнаяЦеноваяГруппа
		|	ИЛИ Неопределено = НаценкиПоУсловиямПродаж.НоменклатурнаяЦеноваяГруппа
		//АБС ВСТАВКА КОНЕЦ
		|ГДЕ
	...
	ИначеЕсли СпособЗаполненияЦен = Перечисления.СпособыЗаполненияЦен.ПоЦенамНоменклатурыКонтрагентов Тогда
		...
		//АБС ВСТАВКА 39049  18.02.2014 18:19:23  Шамов
		|	ЕСТЬNULL(НаценкиПоУсловиямПродаж.абс_СуммаНаценки, 0)     КАК СуммаНаценкиПоУсловиямПродаж,
		//АБС ВСТАВКА 39049 КОНЕЦ
		|	КурсыВалютСрезПоследних.Кратность КАК КратностьВалютыЦены,
		|	КурсыВалютСрезПоследних.Курс КАК КурсВалютыЦены
		|ИЗ
		...
		//АБС ВСТАВКА 39049  18.02.2014 18:19:23  Шамов
		|	ИЛИ ЦеныКонтрагентаСрезПоследних.Номенклатура = НаценкиПоУсловиямПродаж.НоменклатурнаяЦеноваяГруппа
		|	ИЛИ Неопределено = НаценкиПоУсловиямПродаж.НоменклатурнаяЦеноваяГруппа
		//АБС ВСТАВКА КОНЕЦ
		|ГДЕ
	...
	ИначеЕсли СпособЗаполненияЦен = Перечисления.СпособыЗаполненияЦен.ПоПродажнымЦенам Тогда
		...
		//АБС ВСТАВКА 39049  18.02.2014 18:19:23  Шамов
		|	ИЛИ ЦеныПродажные.Номенклатура = НаценкиПоУсловиямПродаж.НоменклатурнаяЦеноваяГруппа
		|	ИЛИ Неопределено = НаценкиПоУсловиямПродаж.НоменклатурнаяЦеноваяГруппа
		//АБС ВСТАВКА КОНЕЦ
		|";
	КонецЕсли;
...
Процедура ЗаполнитьЦенуВСтрокеТабличнойЧасти(ДокументОбъект, СтрокаТабличнойЧасти, СтруктураПоиска, ВыборкаИзРезультатаЗапроса, 
	...
	Если НашлиВВыборке Тогда // Нашли запись
		...
		Иначе // присваиваем значение цены и единицы цены
			...
			КонецЕсли;
			//АБС ИЗМЕНЕНИЕ 39049  18.02.2014 18:23:05  Шамов
			//Цена = Цена * (1 + ВыборкаИзРезультатаЗапроса.ПроцентНаценкиПоУсловиямПродаж / 100);
			Если ВыборкаИзРезультатаЗапроса.СуммаНаценкиПоУсловиямПродаж <> 0 ТОгда
				Цена = Цена + ВыборкаИзРезультатаЗапроса.СуммаНаценкиПоУсловиямПродаж;
			Иначе
				Цена = Цена * (1 + ВыборкаИзРезультатаЗапроса.ПроцентНаценкиПоУсловиямПродаж / 100);
			КонецЕсли;
			//АБС ИЗМЕНЕНИЕ 39049 КОНЕЦ
