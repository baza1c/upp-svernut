...
Процедура ПолучениеОтправкаПисем(СоответствиеТекстовЭлектронныхПисем, ТекущийПользователь, МассивУчетныхЗаписей = Неопределено,
	...
	#КонецЕсли
			...
			Попытка
				...
				ТекстСтроки = "Попытка подключиться вроде удалась " + Формат(ТекущаяДата())+ Символы.ПС;
				ТекстЛога.Записать(ТекстСтроки);	
			...
			Исключение
				...
				ТекстСтроки = Символы.ПС + ТекстОшибок + (Строка(УчетнаяЗапись) + ". " + ОписаниеОшибки()) + Формат(ТекущаяДата())+ Символы.ПС;
				ТекстЛога.Записать(ТекстСтроки);	
	...
	#КонецЕсли
							...
							ТекстСтроки = " письмо должно было уйти" + Символы.ПС ;
							ТекстЛога.Записать(ТекстСтроки);
							...
							Исключение
								...
								ТекстСтроки = Символы.ПС + ТекстОшибок + (Строка(УчетнаяЗапись) + ". " + ОписаниеОшибки()) + Символы.ПС ;
					  			ТекстЛога.Записать(ТекстСтроки);
...
// Процедура осуществляет транспорт писем БЕЗ ЛОГИРОВАНИЯ
//
// Параметры
//  СоответствиеТекстовЭлектронныхПисем - Соответствие, текстов писем и ДокументСсылка.ЭлектронноеПисьмо
//  ТекущийПользователь - СправочникСсылка.Пользователи, Текущий пользователь системы
...
//   НЕТ
//
Процедура ПолучениеОтправкаПисемБезЛогирования(СоответствиеТекстовЭлектронныхПисем, ТекущийПользователь, МассивУчетныхЗаписей = Неопределено,
	Письма = Неопределено, Отправить = Ложь, Получить = Ложь, ПоказыватьОкноВыполнения = Истина, ТекстОшибок = "") Экспорт
...
Функция ЛогированиеФайлЛога ();
	...
	ИмяФайла = "ЛогПочтовикаРассылкиОтчетаПДЗ_" + Формат(ТекущаяДата(),"ДФ=гг_ММ_д") +".txt";
