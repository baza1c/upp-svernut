...
Процедура ЗаполнитьДанныеПоАктуНаПередачуПрав(СсылкаНаОбъект, СтруктураЭД, ДеревоДанных) Экспорт
	...
	ДокументыОснования.Колонки.Добавить("Представление");
	ДокОснование = ДокументыОснования.Добавить();
	ДокОснование.Представление = Строка(Данные.Основание);
	ОбщегоНазначенияЭД.ЗагрузитьТаблицуВДерево(ДеревоДанных, ДокументыОснования, "ДокументыОснования");
	...
	Иначе
		ОтветственныеЛица = РегламентированнаяОтчетность.ОтветственныеЛицаОрганизаций(СтруктураЭД.Организация, СтруктураЭД.ДатаЭД);
		ФИОРуководителя = ОтветственныеЛица.Руководитель;
		ДолжностьРуководителя = ОтветственныеЛица.РуководительДолжность;
		ОбщегоНазначенияЭД.ЗаполнитьЗначениеРеквизитаВДереве(ДеревоДанных, "Подписант.ЮЛ.Должность", ДолжностьРуководителя);
		ОбщегоНазначенияЭД.ЗаполнитьЗначениеРеквизитаВДереве(ДеревоДанных, "Подписант.ЮЛ.ИНН",       ДанныеЮрФизЛица.ИНН);
		ЗаполнитьФИОПодписантаВДереве(ДеревоДанных, "ЮЛ", ФИОРуководителя);
	КонецЕсли;
...
//  СтруктураПараметров - структура, параметры для заполнения.
//
Функция ЗаполнитьДанныеПоКорректировочномуДокументуПолучатель(СсылкаНаОбъект, СтруктураЭД, ДеревоДанных) Экспорт
...
Функция ЗначениеПеречисленияСтавкаНДС(СтавкаЧислом) Экспорт
	...
	ИначеЕсли Найти("5#0.05#0,05#5%", ПредставлениеСтавкиНДС) > 0 Тогда
		ЗначениеНДС = Перечисления.СтавкиНДС.НДС5;
		
	ИначеЕсли Найти("7#0.07#0,07#7%", ПредставлениеСтавкиНДС) > 0 Тогда
		...
		Иначе
			...
			Иначе 
				...
				Иначе
					...
					// Добавим серийные номера
					НоваяСтрока.КлючСвязи = КлючСвязи;
					Если ДанныеДляЗаполненияСтрокиТЧ.Свойство("СерийныйНомер") И 
						ЗначениеЗаполнено(ДанныеДляЗаполненияСтрокиТЧ.Номенклатура) Тогда
...
Процедура ЗаполнитьФИОПодписантаВДереве(ДеревоДанных, ТипПодписанта = "", ИсточникДанных = "")
	...
	ОбщегоНазначенияЭД.ЗаполнитьЗначениеРеквизитаВДереве(ДеревоДанных,
														 "Подписант." + ?(ТипПодписанта = "", "", ТипПодписанта + ".") + "Фамилия",
														 Фамилия);
	ОбщегоНазначенияЭД.ЗаполнитьЗначениеРеквизитаВДереве(ДеревоДанных,
														 "Подписант." + ?(ТипПодписанта = "", "", ТипПодписанта + ".") + "Имя",
														 Имя);
	ОбщегоНазначенияЭД.ЗаполнитьЗначениеРеквизитаВДереве(ДеревоДанных,
														 "Подписант." + ?(ТипПодписанта = "", "", ТипПодписанта + ".") + "Отчество",
														 Отчество);
