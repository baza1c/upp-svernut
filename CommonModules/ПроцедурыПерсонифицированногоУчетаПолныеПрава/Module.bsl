...
Функция ДанныеОДоходахЗаОтчетныеПериоды(ПервыйОтчетныйПериод, ПоследнийОтчетныйПериод, Организация, ДатаСозданияДокументов, МассивФизлиц = Неопределено, ОтборПоКатегорииЗастрахованныхЛиц = Неопределено, ОтборПоТипуДоговора = Неопределено) Экспорт
	...
	Иначе
		...
		|			&Окончание,
		|			МЕСЯЦ,
		|			(НЕ ВидДохода В (&НеучитываемыеВидыДохода))
		|				И (НЕ ВидДохода В (&НестрахуемыеВидыДохода))
	...
	|		КОГДА ДоходыВзносов.ВидТарифаСтраховыхВзносов В (ЗНАЧЕНИЕ(Перечисление.ТарифыСтраховыхВзносов.ОбщийНалоговыйРежим), ЗНАЧЕНИЕ(Перечисление.ТарифыСтраховыхВзносов.УпрощенныйНалоговыйРежим), ЗНАЧЕНИЕ(Перечисление.ТарифыСтраховыхВзносов.ЕНВД))
	|			ТОГДА 0
