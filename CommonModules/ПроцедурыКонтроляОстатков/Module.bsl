...
Процедура ТоварыНаСкладахКонтрольОстатков(ИмяТабличнойЧасти, СтруктураШапкиДокумента, ОтказПроведения, ЗаголовокСообщения, РежимПроведения) Экспорт
	...
	//АБС Глебов
	Если (СтруктураШапкиДокумента.Свойство("Склад") И СтруктураШапкиДокумента.Склад.абс_МожноУходитьВМинус)
		Или (СтруктураШапкиДокумента.Свойство("СкладГруппа") И ТипЗнч(СтруктураШапкиДокумента.СкладГруппа) = Тип("СправочникСсылка.Склады") И СтруктураШапкиДокумента.СкладГруппа.абс_МожноУходитьВМинус) Тогда
		Возврат;
	КонецЕсли;
	//\\АБС Глебов
...
Процедура ТоварыВРезервеНаСкладахКонтрольОстатков(ИмяТабличнойЧасти, СтруктураШапкиДокумента, ОтказПроведения, ЗаголовокСообщения, РежимПроведения) Экспорт
	...
	//АБС ВСТАВКА   30.05.2014 18:26:58  Балашенко
	Если (СтруктураШапкиДокумента.Свойство("Склад") И СтруктураШапкиДокумента.Склад.абс_МожноУходитьВМинус)
		Или (СтруктураШапкиДокумента.Свойство("СкладГруппа") И ТипЗнч(СтруктураШапкиДокумента.СкладГруппа) = Тип("СправочникСсылка.Склады") И СтруктураШапкиДокумента.СкладГруппа.абс_МожноУходитьВМинус) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ТоварыВРезервеНаСкладахКонтрольСвободныхОстатков_Заказ_КорректировкаЗаказа_Резервирование(СтруктураШапкиДокумента, СтруктураПараметров, Заголовок, Отказ)
	...
	//АБС ИЗМЕНЕНИЕ   22.07.2014 19:18:11  Балашенко
	////АБС ВСТАВКА 37613
	//Если СтруктураШапкиДокумента.ВидДокумента = "ЗаказПокупателя" 
	//	И СтруктураШапкиДокумента.РежимПроведения = РежимПроведенияДокумента.Неоперативный Тогда
	//	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,".Остатки(",".Остатки(&ДатаОстатков");
	//	Запрос.УстановитьПараметр("ДатаОстатков",Новый Граница(СтруктураШапкиДокумента.Дата,ВидГраницы.Исключая));
	//КонецЕсли;
	////АБС КОНЕЦ
	//АБС ИЗМЕНЕНИЕ  КОНЕЦ	
...
Процедура ТоварыПереданныеКонтрольОстатков_ЗаказВШапке(СтруктураШапкиДокумента, СтруктураПараметров, МетаданныеТабЧасти, Заголовок, Отказ)
	...
	|		Номенклатура В (" + ТекстЗапросаСписокНоменклатуры + ")
	|	И ДоговорКонтрагента = &ДоговорКонтрагента"
	+ ?(ПараметрЗапросаСделка <> Неопределено, "И Сделка                = &Сделка","") + " 
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ОстатокСумма < Выборка.ДокументСумма Тогда
            	Если СтруктураПараметров.ИмяДокумента = "ОтчетКомиссионераОПродажах" Тогда
			     ЗначениеПрава = УправлениеДопПравамиПользователей.ПравоРазрешитьПроведениеБезКонтроляВзаиморасчетов();
...
Процедура СвободныеОстаткиКонтрольОстатков(ИмяТабличнойЧасти, СтруктураШапкиДокумента, ОтказПроведения, ЗаголовокСообщения)
	...
	//АБС ИЗМЕНЕНИЕ   22.07.2014 19:18:40  Балашенко
	////АБС ВСТАВКА 37613
	//Если СтруктураШапкиДокумента.ВидДокумента = "ЗаказПокупателя" 
	//	И СтруктураШапкиДокумента.РежимПроведения = РежимПроведенияДокумента.Неоперативный Тогда
	//	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,".Остатки(",".Остатки(&ДатаОстатков");
