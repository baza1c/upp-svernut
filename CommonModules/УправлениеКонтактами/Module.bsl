...
Функция ПолучитьКартинкуНапоминания(Напоминание,ТипСобытия = Неопределено) Экспорт
	...
	ИначеЕсли ИндексВозврата = 22 Тогда

	Возврат БиблиотекаКартинок.ЗагрузитьИзИнтернета32;
	ИначеЕсли ИндексВозврата = 23 Тогда
		
		Возврат  БиблиотекаКартинок.ПодсистемаНормативноСправочнаяИнформация;

	 ИначеЕсли ИндексВозврата = 24 Тогда
		 
		 Возврат  БиблиотекаКартинок.ПодсистемаПродажи;
       ИначеЕсли ИндексВозврата = 25 Тогда
...
Функция ОпределитьИндексКартинкиВидаСобытия(Объект,ТипСобытия = Неопределено) Экспорт
	...
	Если ТипЗнч(ВидСобытия) = Тип("ПеречислениеСсылка.ВидыСобытий") Тогда
		...
		ИначеЕсли ВидСобытия = Перечисления.ВидыСобытий.Командировка Тогда

				Возврат 22;
		ИначеЕсли ВидСобытия = Перечисления.ВидыСобытий.ВизитВУчреждение Тогда
				Возврат 24;
    ИначеЕсли ВидСобытия = Перечисления.ВидыСобытий.ПередачаКонтакта Тогда
			    Возврат 25;
         ИначеЕсли ВидСобытия = Перечисления.ВидыСобытий.Выставка Тогда
                Возврат 23;
		Иначе
			Если ТипСобытия = Перечисления.ВходящееИсходящееСобытие.Исходящее Тогда
				Возврат 13;
