...
Процедура ПолучитьДанныеВыписки(Знач ЭД, СсылкаНаХранилище, МассивСчетов = Неопределено, Организация = Неопределено, СоглашениеЭД = Неопределено) Экспорт
	...
	ЭлектронныеДокументы.ПолучитьДанныеВыпискиБанка(ЭД, СсылкаНаХранилище, МассивСчетов);
...
Функция ПолучитьДвоичныеДанныеФайла(ЭД, СертификатПодписи) Экспорт
	...
	Если ВидЭД = Перечисления.ВидыЭД.АктИсполнитель
		...
		ИЛИ ВидЭД = Перечисления.ВидыЭД.ИзвещениеОПолучении
		ИЛИ ВидЭД = Перечисления.ВидыЭД.УведомлениеОбУточнении Тогда
		...
		Если ЗначениеЗаполнено(СертификатПодписи) Тогда
			...
			Если ВидЭД = Перечисления.ВидыЭД.ИзвещениеОПолучении
				...
				ЗаполнитьУзелПодписантаСлужебныхЭД(ИмяФайла, Организация, СертификатПодписи);
			Иначе
				ЗаполнитьУзелПодписантаЭД(ИмяФайла, Организация, СертификатПодписи);
			КонецЕсли;
...
Функция ВыполнитьДействияПоЭД(Знач МассивСсылокНаОбъект,
	...
	Если ЭлектронныеДокументыКлиентСервер.ЕстьДействие(Действия, "Подписать") Тогда
		...
		Если ТЗ_Сертификатов.Количество() = 0 И ДоступныеСертификаты.Количество() > 0 И ЗначениеЗаполнено(ЭД) Тогда
			...
			Если СпособОбменаЭД = Перечисления.СпособыОбменаЭД.ЧерезОператораЭДОТакском Тогда
				ТекстСообщения = Нстр("ru = 'В соглашении об использовании электронных документов %1
											|не найден ни один из доступных сертификатов ЭЦП'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
																ТекстСообщения,
																"""" + СоглашениеЭД + """");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		...
		Если Выборка.Количество() > 0 Тогда
			Пока Выборка.Следующий() Цикл
				...
				Если Выборка.ЭтоИнтеркампани Тогда
					...
					КонецЕсли;
				КонецЕсли;
				Если Выборка.СпособОбменаЭД = Перечисления.СпособыОбменаЭД.ЧерезВебРесурсБанка Тогда
					ТЗ_Отпечатков = ТЗ_УстановленныхПодписей.Скопировать(Отбор);
					Если ТЗ_Отпечатков.Количество() > 0 Тогда
						МассивОтпечатковИсключения = ТЗ_Отпечатков.ВыгрузитьКолонку("УстановленныеПодписи");
					КонецЕсли;
		...
		Если ФайлИнформации <> Неопределено Тогда
			...
			Пока ЧтениеХМЛ.Прочитать() Цикл
				...
				Если ЧтениеХМЛ.ТипУзла = ТипУзлаXML.КонецЭлемента И ЧтениеХМЛ.ЛокальноеИмя = "ДокументШифрования" Тогда
					...
					СтруктураШифрования.Вставить("Зашифрован", Зашифрован);
					СтруктураШифрования.Вставить("Документ",   ВыборкаПакеты.СсылкаНаДокумент);
					...
					МассивВозврата.Добавить(СтруктураШифрования);
					Документ = Неопределено;
					Зашифрован = Ложь;
					Сертификат = "";
				КонецЕсли;
					...
					Если ЭД.Description.AdditionalData <> Неопределено
						...
						Для Каждого Свойство Из ЭД.Description.AdditionalData.AdditionalParameter Цикл
							Если Свойство.Name = "Зашифрован" Тогда
								...
								СтруктураШифрования = Новый Структура;
								СтруктураШифрования.Вставить("Зашифрован", Булево(Свойство.Value));
								СтруктураШифрования.Вставить("Документ",   ВыборкаПакеты.СсылкаНаДокумент);
...
Процедура ЗаполнитьДокументИБПоЭД(СсылкаНаВладельца, СсылкаНаЭД, ДокЗаполнен =Ложь)
	...
	Если МенеджерЗаписи.ЭлектронныйДокумент <> СсылкаНаЭД Тогда
...
Процедура ЗаполнитьУзелПодписантаЭД(ИмяФайла, Организация, СертификатПодписи)
...
Процедура ЗаполнитьУзелПодписантаСлужебныхЭД(ИмяФайла, Организация, СертификатПодписи)
