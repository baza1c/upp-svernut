...
Процедура ЗаполнитьМатериалыПоСпецификациям(ТабличнаяЧастьМатериалы, ТаблицаПродукция, СтруктураДопКолонок, Отбор, ДатаСпецификации, КоличествоУровнейРазузлования = 1, ПараметрыВыпускаПродукции = Неопределено) Экспорт
	...
	// {Лео Катанович
	ТабличнаяЧастьПродукция.Колонки.Добавить("Лео_Количествобезпотерь", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 3)));
	// Лео Катанович}
	ДопКолонки = "";
	...
	ТабличнаяЧастьПродукция.Свернуть("Номенклатура, ХарактеристикаНоменклатуры, ЕдиницаИзмерения, Спецификация, Коэффициент, КлючСвязи" + ДопКолонки, "Количество, Лео_Количествобезпотерь");
	...
	Для каждого СтрокаТабличнойЧасти из ТабличнаяЧастьПродукция Цикл
		...
		СтруктураИсточник = Новый Структура("Номенклатура, ХарактеристикаНоменклатуры, ЕдиницаИзмерения, Коэффициент, Количество, Лео_Количествобезпотерь, Спецификация");
		ЗаполнитьЗначенияСвойств(СтруктураИсточник, СтрокаТабличнойЧасти);
		// {Лео Катанович
		СтруктураИсточник.Лео_Количествобезпотерь = СтрокаТабличнойЧасти.Количество;
		// Лео Катанович}
...
Процедура ЗаполнитьРаспределениеМатериаловНаПродукцию(СтруктураШапкиДокумента, Материалы, ТаблицаПродукция, РаспределениеМатериалов, СтруктураДопКолонок, Отбор, ПараметрыВыпускаПродукции = Неопределено, ЭтоВозвратныеОтходы = Ложь) Экспорт
	...
	//{Лео Катанович
	ТабличнаяЧастьПродукция.Колонки.Добавить("Лео_Количествобезпотерь", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(15, 3));
	//Лео Катанович}
	...
	ОбщегоНазначения.ЗагрузитьВТаблицуЗначений(ТаблицаПродукция, ТабличнаяЧастьПродукция);
	ТабличнаяЧастьПродукция.Свернуть("Номенклатура, ХарактеристикаНоменклатуры, СерияНоменклатуры, ЕдиницаИзмерения, Спецификация, Коэффициент, КлючСвязи" + ДопКолонки, "Количество, Лео_Количествобезпотерь, КоличествоДоделка");
	...
	//{Лео Катанович
	ТаблицаМатериалыПродукция.Колонки.Добавить("Лео_Количествобезпотерь");
    //Лео Катанович}
    ...
    //{Лео Катанович
	ТаблицаПотребностей.Колонки.Добавить("Лео_Количествобезпотерь", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(15, 3));
    //Лео Катанович}
	...
	Для каждого СтрокаТабличнойЧасти из ТаблицаИсточник Цикл
		...
		Для каждого НайденнаяСтрокаПотребности из НайденныеСтрокиПотребности Цикл
			...
			// Чернов Распределение сырья меньше 0.001. Операторы всегда ставят 0.001 сырья в материалы, если значение сырья > 0.001
			Если НайденнаяСтрокаПотребности.КоличествоМатериала = 0 тогда  НайденнаяСтрокаПотребности.КоличествоМатериала = 0.001 КонецЕсли;
