...
Процедура ПриИзмененииЗначенияДоговора(ДокументОбъект, СтараяВалютаВзаиморасчетов, ВалютаРегламентированногоУчета,
	...
	Иначе
		...
		Иначе
			//АБС ИЗМЕНЕНИЕ   27.05.2014 12:44:38  Балашенко
...
#КонецЕсли
	...
	Если НадоПересчитывать = Ложь Тогда
		...
		Если МетаданныеДокумента.Реквизиты.Найти("ТипЦен") <> Неопределено Тогда
			...
			//АБС+++ Гончаров 31.01.2014
			Если ЗначениеЗаполнено(Договор.ТипЦен) ИЛИ ЗначениеЗаполнено(абс_Дистрибуция.ПолучитьТипЦеныПоДоговору(ДокументОбъект.Дата, Договор.Владелец, Договор)) Тогда
			//\\АБС--- Гончаров 31.01.2014
			
				ДокументОбъект.ТипЦен = Договор.ТипЦен;
				
				//АБС+++ Гончаров 31.01.2014
				ДокументОбъект.ТипЦен = абс_Дистрибуция.ПолучитьТипЦеныПоДоговору(ДокументОбъект.Дата, Договор.Владелец, Договор);
				//\\АБС--- Гончаров 31.01.2014
				
				//абс изменение Балашенко 05.03.2014
				Если МетаданныеДокумента.Реквизиты.Найти("СуммаВключаетНДС") <> Неопределено 
				   И МетаданныеДокумента.Реквизиты.Найти("УчитыватьНДС") <> Неопределено
				   И ДокументОбъект.УчитыватьНДС И Договор.ТипЦен<>Неопределено Тогда
					ДокументОбъект.СуммаВключаетНДС= Договор.ТипЦен.ЦенаВключаетНДС;
				КонецЕсли;
				//абс изменение Балашенко 05.03.2014 конец
			КонецЕсли;
		КонецЕсли;
	...
	Иначе // ДокументОбъект.[ИмяТабличнойЧасти].Количество() > 0
		...
		Если МетаданныеДокумента.Реквизиты.Найти("ТипЦен") <> Неопределено Тогда
			...
			//АБС+++ Гончаров 31.01.2014
			НовыйТипЦен = абс_Дистрибуция.ПолучитьТипЦеныПоДоговору(ДокументОбъект.Дата, Договор.Владелец, Договор);
			//\\АБС--- Гончаров 31.01.2014
...
Процедура ИзменитьЦеныВалюту(ДокументОбъект, СпособЗаполненияЦен, СтруктураРеквизитовДокумента, 
	...
	//Лео+ ZAA Цен
	Если СтруктураЗначений.НовыйВалютаДокумента <> Справочники.Валюты.НайтиПоКоду("643") и // только валютные
		 СтруктураЗначений.НовыйКурсВзаиморасчетов > 0 и СтруктураЗначений.НовыйКурсДокумента > 0 тогда
	Если СтруктураЗначений.НовыйКурсВзаиморасчетов <> СтруктураЗначений.НовыйКурсДокумента тогда
		 СтруктураЗначений.НовыйКурсДокумента = СтруктураЗначений.НовыйКурсВзаиморасчетов;
		 СтруктураРеквизитовДокумента.КурсВзаиморасчетов = СтруктураЗначений.НовыйКурсВзаиморасчетов;
    КонецЕсли;
	КонецЕсли;
	//Лео- ZAA
