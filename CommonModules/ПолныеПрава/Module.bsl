...
Процедура ПодготовитьСтруктуруДляЗаполненияШапкиДокумента(СтруктураПараметров, ТипОперации, ОбъектКопирования) Экспорт
	...
	//АБС ВСТАВКА Цены всегда без НДС  04.06.2014 23:19:46  Фролов
	флСуммаВключаетНДС = Ложь;
	//АБС ВСТАВКА Цены всегда без НДС КОНЕЦ 
...
//АБС ВСТАВКА   27.02.2014 14:15:39  Шамов
...
Процедура ПолучитьСоответствиеУзлов(СоответствиеУзловИОрганизаций, СоответствиеУзловИСкладов, СоответствиеУзловИПодразделений, МассивУзловБезФильтраПоБухУчету, МассивУзловДлярегистрацииИзмененийДокументов, ИмяОбменаБП = "")
	...
	|	ИЗ
	|		ПланОбмена.ОбменУправлениеТорговлейБухгалтерияКОРП" + ИмяОбменаБП + ".Организации КАК СписокОрганизаций
	|	ГДЕ
	...
	|		ОбменУправлениеТорговлейБухгалтерияКОРП.Ссылка КАК Ссылка
	|	ИЗ
	|		ПланОбмена.ОбменУправлениеТорговлейБухгалтерияКОРП" + ИмяОбменаБП + " КАК ОбменУправлениеТорговлейБухгалтерияКОРП
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		ВременнаяВсеОрганизации КАК ВременнаяВсеОрганизации
	|	ПО
	|		ИСТИНА
	...
	|		ПланОбмена.ОбменУправлениеТорговлейБухгалтерияКОРП" + ИмяОбменаБП + ".Организации КАК СписокОрганизаций
	|	ПО
	|		СписокОрганизаций.Ссылка = ОбменУправлениеТорговлейБухгалтерияКОРП.Ссылка
	|	ГДЕ
	|		ОбменУправлениеТорговлейБухгалтерияКОРП.Ссылка <> &ЭтотУзелПоОрганизации" + ИмяОбменаБП + "
	...
	|	ИЗ
	|		ПланОбмена.ОбменУправлениеТорговлейБухгалтерияКОРП" + ИмяОбменаБП + ".Подразделения КАК СписокПодразделений
	|	ГДЕ
	...
	|		ПланОбмена.ОбменУправлениеТорговлейБухгалтерияКОРП" + ИмяОбменаБП + ".Подразделения КАК СписокПодразделений
	|	ПО
	|		СписокПодразделений.Ссылка = ОбменУправлениеТорговлейБухгалтерияКОРП.Ссылка
	|	ГДЕ
	|		ОбменУправлениеТорговлейБухгалтерияКОРП.Ссылка <> &ЭтотУзелПоОрганизации" + ИмяОбменаБП + "
	...
	|	ИЗ
	|		ПланОбмена.ОбменУправлениеТорговлейБухгалтерияКОРП" + ИмяОбменаБП + ".Склады КАК СписокСкладов
	|	ГДЕ
	...
	|		ПланОбмена.ОбменУправлениеТорговлейБухгалтерияКОРП" + ИмяОбменаБП + ".Склады КАК СписокСкладов
	|	ПО
	|		СписокСкладов.Ссылка = ОбменУправлениеТорговлейБухгалтерияКОРП.Ссылка
	|	ГДЕ
	|		ОбменУправлениеТорговлейБухгалтерияКОРП.Ссылка <> &ЭтотУзелПоОрганизации" + ИмяОбменаБП + "
	...
	|	ПланОбмена.ОбменУправлениеТорговлейБухгалтерияКОРП" + ИмяОбменаБП + " КАК ОбменУправлениеТорговлейБухгалтерияКОРП
	|ГДЕ
	|	ОбменУправлениеТорговлейБухгалтерияКОРП.ПередаватьТолькоДокументыОтраженныеВБухгалтерскомУчете = Ложь
	|	И ОбменУправлениеТорговлейБухгалтерияКОРП.Ссылка <> &ЭтотУзелПоОрганизации" + ИмяОбменаБП + "
	|;
	...
	|	ПланОбмена.ОбменУправлениеТорговлейБухгалтерияКОРП" + ИмяОбменаБП + " КАК ОбменУправлениеТорговлейБухгалтерияКОРП
	|ГДЕ
	|	ОбменУправлениеТорговлейБухгалтерияКОРП.ВариантПереносаСебестоимостиСписанияВБухгалтериюПредприятия <> Значение(Перечисление.ВариантыПереносаСебестоимостиСписанияВБухгалтериюПредприятия.НеПереносить)
	|	И ОбменУправлениеТорговлейБухгалтерияКОРП.Ссылка <> &ЭтотУзелПоОрганизации" + ИмяОбменаБП + "
	|;
	|";
