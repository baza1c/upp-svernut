...
Функция РазузловатьНоменклатуру(Источник, Результат = Неопределено, Параметры = Неопределено) Экспорт
	...
	Если Источник.Свойство("Номенклатура") И ТипЗнч(Источник.Номенклатура) = Тип("СправочникСсылка.Номенклатура") И НЕ Источник.Номенклатура.Пустая() Тогда
		...
		//{ Лео Катанович
		НоваяСтрока.Лео_Количествобезпотерь = ?(Источник.Свойство("Лео_Количествобезпотерь"), Источник.Лео_Количествобезпотерь, 1);
		// Лео Катанович}
	...
	ИначеЕсли Источник.Свойство("Спецификация") И ТипЗнч(Источник.Спецификация) = Тип("СправочникСсылка.СпецификацииНоменклатуры") 
				И НЕ Источник.Спецификация.Пустая() И НЕ Источник.Спецификация.ПолучитьОбъект() = Неопределено Тогда
		...
		Сообщить(""+Источник+"  -  "+Источник.Количество+" - ");
			...
			Если СтрокаТекущегоУровня.Спецификация <> Неопределено И НЕ СтрокаТекущегоУровня.Спецификация.Пустая() Тогда
				...
				Пока ИсходноеКомплектующее.Следующий() Цикл
					...
					Если Результат.Свойство("СопутствующиеИзделия") Тогда
						...
						//{Лео Катанович
						Результат.ПолноеРазузлование.Колонки.Лео_Количествобезпотерь.Имя = "Лео_Количествобезпотерь";
						//Лео Катанович}
					КонецЕсли;
...
Функция ДобавитьВПолноеРазулование(СтрокаСпецификации, ИсходнаяСтрока, КоличествоВыпусков, КоэффициентОсновногоСырья, Параметры, Уровень, Результат, МассивОшибок, ПоследовательностьУзлов = Неопределено)
	...
	Если СтрокаСпецификации.ВидНорматива = Перечисления.ВидыНормативовНоменклатуры.Номенклатура Тогда
		...
		//{Лео Катанович
		НоваяСтрока.Лео_Количествобезпотерь = СтрокаСпецификации.Лео_количествобезпотерь;	
		//Лео Катанович}	
		...
		Если СтрокаСпецификации.УказаниеНорматива = Перечисления.ВидыУказанияНорматива.НаКоличествоОсновногоСырья Тогда
			...
			//{Лео Катанович
			НоваяСтрока.Лео_Количествобезпотерь = НоваяСтрока.Лео_Количествобезпотерь * КоэффициентОсновногоСырья;
			//Лео Катанович}	
		Иначе
			НоваяСтрока.Количество = НоваяСтрока.Количество * КоличествоВыпусков.Числитель / КоличествоВыпусков.Знаменатель;
			//{Лео Катанович
			НоваяСтрока.Лео_Количествобезпотерь = НоваяСтрока.Лео_Количествобезпотерь * КоличествоВыпусков.Числитель / КоличествоВыпусков.Знаменатель;
			//Лео Катанович}	
		КонецЕсли;
...
Процедура ДобавитьВСопутствующиеИзделия(Источник, Результат)
	...
	Пока ВыходныеИзделия.Следующий() Цикл
		...
		НоваяСтрока.Лео_Количествобезпотерь = ?(Результат.КоличествоВыпусков.Знаменатель = 0, 0, НоваяСтрока.Лео_Количествобезпотерь * Результат.КоличествоВыпусков.Числитель / Результат.КоличествоВыпусков.Знаменатель);
...
Функция ПолучитьВыходныеИзделия(Источник)
	...
	|	СпецификацииНоменклатурыВыходныеИзделия.Количество КАК Лео_Количествобезпотерь
	|ИЗ
...
Функция ПолучитьИсходныеКомплектующие(Спецификация, Источник, НаследуемыеСвойства = Неопределено)
	...
	// {Лео Катанович
	|	ИсходныеКомплектующие.Лео_количествобезпотерь КАК Лео_количествобезпотерь,
    // Лео Катанович}
	|	ВЫБОР
...
Процедура УстановитьРазузлован(НоваяСтрока, Параметры) Экспорт
...
Процедура ИнициализацияРезультатаРазузлования(Результат, Параметры)
	...
	Если Результат.Свойство("ПолноеРазузлование") И Результат.ПолноеРазузлование = Неопределено Тогда
		...
		//{ zhiga
		Результат.ПолноеРазузлование.Колонки.Добавить("Количество", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(18, 6)));
		//zhiga }
		
		//{Лео Катанович
		Результат.ПолноеРазузлование.Колонки.Добавить("Лео_Количествобезпотерь", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(18,6)));
        //Лео Катанович}
	...
	Если Результат.Свойство("ВыходныеИзделия") И Результат.ВыходныеИзделия = Неопределено Тогда
		...
		//{Лео Катанович
		Результат.ВыходныеИзделия.Колонки.Добавить("Лео_Количествобезпотерь", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 3)));
        //Лео Катанович}
	...
	Если Результат.Свойство("ИсходныеКомплектующие") И Результат.ИсходныеКомплектующие = Неопределено Тогда
		...
		//{ zhiga
		Результат.ИсходныеКомплектующие.Колонки.Добавить("Количество", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(18, 6)));
		//zhiga }
		
		//{Лео Катанович
		Результат.ИсходныеКомплектующие.Колонки.Добавить("Лео_Количествобезпотерь", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(18, 6)));
        //Лео Катанович}
	...
	Если (Параметры.РазмещатьВСопутствующихИзделиях И (НЕ Результат.Свойство("СопутствующиеИзделия") ИЛИ Результат.Свойство("СопутствующиеИзделия") И Результат.СопутствующиеИзделия = Неопределено)) ИЛИ (Результат.Свойство("СопутствующиеИзделия") И Результат.СопутствующиеИзделия = Неопределено) Тогда
        ...
        //{Лео Катанович
        Результат.СопутствующиеИзделия.Колонки.Добавить("Лео_Количествобезпотерь", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 3)));
        //Лео Катанович}
