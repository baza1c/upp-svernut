...
Функция ОКТМО(ИсточникСведений)
	...
	ИначеЕсли МетаданныеРеквизит.Тип.СодержитТип(Тип("Строка")) Тогда
		Значение = СокрЛП(ИсточникСведений.КодПоОКТМО);
	Иначе
		Значение = СокрЛП(ИсточникСведений.КодПоОКТМО.Код);
	КонецЕсли;
		...
		Иначе
			...
			Если ИДКонфигурации = "УПП" ИЛИ ИДКонфигурации = "КА" Тогда
				...
				ЗапросПоЛицам.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
				                      ...
				                      |				ИНАЧЕ """"
				                      |			КОНЕЦ
				                      |	КОНЕЦ + ОтветственныеЛицаОрганизацийСрезПоследних.Документ КАК ФИОПолное
				                      |ИЗ
				                      |	РегистрСведений.ОтветственныеЛицаОрганизаций.СрезПоследних(&ДатаСреза, СтруктурнаяЕдиница = &Организация) КАК ОтветственныеЛицаОрганизацийСрезПоследних
				                      |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизЛиц.СрезПоследних(&ДатаСреза, ФизЛицо ССЫЛКА Справочник.ФизическиеЛица) КАК ФИОФизЛицСрезПоследних
				                      |		ПО ОтветственныеЛицаОрганизацийСрезПоследних.ФизическоеЛицо = ФИОФизЛицСрезПоследних.ФизЛицо";
...
//Рекурсивная процедура  для возврата объекта типа Структура
...
//В случае использования массивов в JSON-объекте используем третью процедуру
...
//Функция раскладывает строку с данными об адресе (в формате 20 запятых) на элементы структуры
Функция РазложитьАдресJSON(Знач СтрокаАдрес) Экспорт
	
	Чтение = Новый ЧтениеJSON;
	Чтение.УстановитьСтроку(СтрокаАдрес);
	...
	Если Структура.Свойство("country")	тогда
		Страна	=	СокрЛП(Структура.country);
	КонецЕсли;
	Если Структура.Свойство("ZIPcode")	тогда
		Индекс	=	СокрЛП(Структура.ZIPcode);
	КонецЕсли;
	Если Структура.Свойство("area")	тогда
		Регион	=	СокрЛП(Структура.area);
	КонецЕсли;
	Если Структура.Свойство("district")	тогда
		Район	=	СокрЛП(Структура.district);
	КонецЕсли;
	Если Структура.Свойство("city")	тогда
		Город	=	СокрЛП(Структура.city);
	КонецЕсли;
	Если Структура.Свойство("М")	тогда
		НаселенныйПункт	=	СокрЛП(Структура.М);
	КонецЕсли;
	Если Структура.Свойство("street")	тогда
		Улица	=	СокрЛП(Структура.street);
	КонецЕсли;
	Если Структура.Свойство("houseNumber")	тогда
		Дом	=	СокрЛП(Структура.houseNumber);
	КонецЕсли;
	Если Структура.Свойство("М")	тогда
		Корпус	=	СокрЛП(Структура.М);
	КонецЕсли;
	Если Структура.Свойство("М")	тогда
		Квартира	=	СокрЛП(Структура.М);
	КонецЕсли;
	
    //Расширенное представление
	Если Структура.Свойство("streetType")	тогда
		ТипДома	    = СокрЛП(Структура.houseType);
	КонецЕсли;
	Если Структура.Свойство("М")	тогда
		ТипКорпуса	= СокрЛП(Структура.М);
	КонецЕсли;
	Если Структура.Свойство("М")	тогда
		ТипКвартиры = СокрЛП(Структура.М);
	КонецЕсли;
