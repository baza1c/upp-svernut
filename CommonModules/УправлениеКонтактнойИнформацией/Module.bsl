...
Функция ПолучитьСтруктуруАдресаИзСтроки(Знач СтроковыйАдрес) Экспорт
    ...
    ИначеЕсли Сред(СтроковыйАдрес,1,1) = "{" Тогда
		
		СтруктураАдреса = РегламентированнаяОтчетность.РазложитьАдресJSON(СтроковыйАдрес);                  
...
Процедура ПрочитатьКонтактнуюИнформацию(НаборЗаписей, Ссылка) Экспорт
	...
	//АБС ИЗМЕНЕНИЕ   24.03.2014 14:20:13  Балашенко
	//добавлены поля абс_ДатаАктуальностиКИ, абс_ФайлДопДанных
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	ВложенныйЗапрос.абс_ДатаАктуальностиКИ,
	               |	ВложенныйЗапрос.абс_ФайлДопДанных
	               |ИЗ
	               |	(ВЫБРАТЬ
	               |		&Объект КАК Объект,
	               ...
	               |		ДАТАВРЕМЯ(1, 1, 1) КАК абс_ДатаАктуальностиКИ,
	               |		ЗНАЧЕНИЕ(Справочник.ХранилищеДополнительнойИнформации.ПустаяСсылка) КАК абс_ФайлДопДанных
	               |	ИЗ
	               |		Справочник.ВидыКонтактнойИнформации КАК ВидыКонтактнойИнформации
	               |	ГДЕ
	               ...
	               |		РегистрСведенийКонтактнаяИнформация.абс_ДатаАктуальностиКИ,
	               |		РегистрСведенийКонтактнаяИнформация.абс_ФайлДопДанных
	               |	ИЗ
...
Функция СведенияОЮрФизЛице(ЮрФизЛицо, ДатаПериода, ДляФизЛицаТолькоИнициалы = Истина, БанковскийСчет = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(ЮрФизЛицо) = Тип("СправочникСсылка.Организации") Тогда
		...
		|
		|	, МАКСИМУМ (ВЫБОР КОГДА КонтактнаяИнформация.Вид = &ВидАдресДоставки 
		|						ТОГДА ВЫРАЗИТЬ(КонтактнаяИнформация.Представление КАК Строка(1000))
		|	  					ИНАЧЕ """"
		...
		|	И КонтактнаяИнформация.Вид В (&ВидЮридическийАдресФизЛицо, &ВидФактическийАдресФизЛицо, &ВидЮридическийАдрес, &ВидФактическийАдрес, &ВидТелефона, &ВидАдресДоставки)
		...
		Запрос.УстановитьПараметр("ВидАдресДоставки",	 	Справочники.ВидыКонтактнойИнформации.АдресДоставкиКонтрагента);
	...
	ИначеЕсли ТипЗнч(ЮрФизЛицо) = Тип("СправочникСсылка.Контрагенты") Тогда
		...
		|	, МАКСИМУМ (ВЫБОР КОГДА КонтактнаяИнформация.Вид = &ВидАдресДоставки ТОГДА ВЫРАЗИТЬ(КонтактнаяИнформация.Представление КАК Строка(1000))
		|	  	ИНАЧЕ """" КОНЕЦ) КАК АдресДоставки ";
		
		ТекстУсловияОбъектКонтактнойИнформации = 
		"КонтактнаяИнформация.Объект = Организации.Ссылка
		|	И КонтактнаяИнформация.Вид В (&ВидЮридическийАдрес, &ВидФактическийАдрес, &ВидАдресДоставки, &ВидТелефона)
		|	И КонтактнаяИнформация.Тип В (&ТипАдрес, &ТипТелефон)";
	...
	|	КонтактнаяИнформация.абс_ФайлДопДанных
	|ПОМЕСТИТЬ ВТ_РегистрКонтактнаяИнформация
	|ИЗ
	|	РегистрСведений.КонтактнаяИнформация КАК КонтактнаяИнформация
	|ГДЕ
	|	КонтактнаяИнформация.Объект = &ПарЮрФизЛицо
	|	И КонтактнаяИнформация.абс_ДатаАктуальностиКИ <= &ДатаПериода
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	МАКСИМУМ(ВТ_РегистрКонтактнаяИнформация.абс_ДатаАктуальностиКИ) КАК абс_ДатаАктуальностиКИ,
	|	ВТ_РегистрКонтактнаяИнформация.Вид
	|ПОМЕСТИТЬ ВТ_МаксимумДат
	|ИЗ
	|	ВТ_РегистрКонтактнаяИнформация КАК ВТ_РегистрКонтактнаяИнформация
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТ_РегистрКонтактнаяИнформация.Вид
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_РегистрКонтактнаяИнформация.Объект,
	|	ВТ_РегистрКонтактнаяИнформация.Тип,
	|	ВТ_РегистрКонтактнаяИнформация.Вид,
	|	ВТ_РегистрКонтактнаяИнформация.абс_ДатаАктуальностиКИ,
	|	ВТ_РегистрКонтактнаяИнформация.Представление,
	|	ВТ_РегистрКонтактнаяИнформация.Поле1,
	|	ВТ_РегистрКонтактнаяИнформация.Поле2,
	|	ВТ_РегистрКонтактнаяИнформация.Поле3,
	|	ВТ_РегистрКонтактнаяИнформация.Поле4,
	|	ВТ_РегистрКонтактнаяИнформация.Поле5,
	|	ВТ_РегистрКонтактнаяИнформация.Поле6,
	|	ВТ_РегистрКонтактнаяИнформация.Поле7,
	|	ВТ_РегистрКонтактнаяИнформация.Поле8,
	|	ВТ_РегистрКонтактнаяИнформация.Поле9,
	|	ВТ_РегистрКонтактнаяИнформация.Поле10,
	|	ВТ_РегистрКонтактнаяИнформация.Комментарий,
	|	ВТ_РегистрКонтактнаяИнформация.ЗначениеПоУмолчанию,
	|	ВТ_РегистрКонтактнаяИнформация.ТипДома,
	|	ВТ_РегистрКонтактнаяИнформация.ТипКорпуса,
	|	ВТ_РегистрКонтактнаяИнформация.ТипКвартиры,
	|	ВТ_РегистрКонтактнаяИнформация.абс_ФайлДопДанных
	|ПОМЕСТИТЬ ВТ_СрезПоследнихКИ
	|ИЗ
	|	ВТ_МаксимумДат КАК ВТ_МаксимумДат
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_РегистрКонтактнаяИнформация КАК ВТ_РегистрКонтактнаяИнформация
	|		ПО ВТ_МаксимумДат.абс_ДатаАктуальностиКИ = ВТ_РегистрКонтактнаяИнформация.абс_ДатаАктуальностиКИ
	|			И ВТ_МаксимумДат.Вид = ВТ_РегистрКонтактнаяИнформация.Вид
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|		КОНЕЦ = Банки.Ссылка
	|
	|   ЛЕВОЕ СОЕДИНЕНИЕ ВТ_СрезПоследнихКИ КАК КонтактнаяИнформация
	...
	Если ЮрФизЛицо.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо Тогда
		///Никитин ДатаПериода
		Если (ИмяСправочника = "Организации") и (Данные.Наименование = "ООО ""ЛЕОВИТ нутрио""") Тогда
			 Если (ДатаПериода<'20160315000000')  Тогда
		           Сведения.Вставить("КПП",       "771301001");
             ИначеЕсли (ДатаПериода<'20160315000000')  Тогда
				    Сведения.Вставить("КПП",       "771301001");
             ИначеЕсли (ДатаПериода < '20180606000000')  Тогда
				     Сведения.Вставить("КПП",       "770701001");
			 Иначе 
					Сведения.Вставить("КПП",       Данные.КПП);
				КонецЕсли;
		иначе 	Сведения.Вставить("КПП",       Данные.КПП);
	...
	Если (ИмяСправочника = "Организации") и (Данные.Наименование = "ООО ""ЛЕОВИТ нутрио""") Тогда
		Если (ДатаПериода<'20160315000000')  Тогда
	         Сведения.Вставить("ЮридическийАдрес", "127412, Москва г, Лобненская ул, дом № 21");
	    ИначеЕсли (ДатаПериода < '20180606000000')  Тогда
	         Сведения.Вставить("ЮридическийАдрес", "127055, Москва г, Новослободская ул, дом № 14/19, строение 1");
		 Иначе
			 Сведения.Вставить("ЮридическийАдрес", СокрЛП(Данные.ЮридическийАдрес));
		КонецЕсли;	 
    ИначеЕсли (ИмяСправочника = "Организации") и (Данные.Наименование = "ООО ""Леотон Трейдинг""") и (ДатаПериода >= Дата('20180625'))   Тогда
	          Сведения.Вставить("ЮридическийАдрес", СокрЛП(Данные.ЮридическийАдрес)); // + ", комната 69, пом.1, цокол.  этаж");	Добавлен в юр.адрес Организации 
	ИначеЕсли (ИмяСправочника = "Организации") и (Данные.Наименование = "ООО ""Леотон Трейдинг""") и (ДатаПериода >= Дата('20180115'))  Тогда
	          Сведения.Вставить("ЮридическийАдрес", СокрЛП(Данные.ЮридическийАдрес)); //+ ", комната 69, пом.I, цокол.  этаж");	Добавлен в юр.адрес Организации 
    ИначеЕсли (ИмяСправочника = "Организации") и (Данные.Наименование = "ЭЙКОСЪ УК") и (ДатаПериода >= Дата('20180125'))  Тогда
	          Сведения.Вставить("ЮридическийАдрес", СокрЛП(Данные.ЮридическийАдрес)); //+ ", антресоль 1 этажа, комната 13");	    Добавлен в юр.адрес Организации
    //Лео Катанович}
    Иначе
         Сведения.Вставить("ЮридическийАдрес", СокрЛП(Данные.ЮридическийАдрес));
	КонецЕсли; //-Никитин
	//Если (ИмяСправочника = "Организации") и (Данные.Наименование = "ООО УК ""ЭЙКОСЪ""") и (ДатаПериода >= Дата('20180125'))  Тогда
	//		  Сведения.Вставить("ФактическийАдрес", СокрЛП(Данные.ФактическийАдрес) + ", антресоль 1 этажа, комната 13");	
	//Иначе			  
			  Сведения.Вставить("ФактическийАдрес", СокрЛП(Данные.ФактическийАдрес));
	//КонецЕсли;		  
	Сведения.Вставить("АдресДоставки",	  СокрЛП(Данные.АдресДоставки));
