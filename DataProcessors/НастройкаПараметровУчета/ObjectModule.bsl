...
Функция ЗаписатьКонстанты(ФормаОбработки, НадоОбновитьУчетнаяПолитикаПоПерсоналуОрганизации) Экспорт
	...
	Если ЗначениеЗаполнено(ФормаОбработки.мСтрокаКонстант) Тогда
		...
		// запишем изменения
		КэшируемыеКонстанты = "ИспользуетсяТрудЧленовЭкипажейМорскихСудов, ИспользуетсяТрудРаботающихВСтудотряде, ИспользуетсяТрудФармацевтов, ИспользуетсяТрудЧленовЛетныхЭкипажей, ИспользуетсяТрудШахтеров, ИспользуютсяРаботыСДосрочнойПенсией";
