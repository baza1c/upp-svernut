#Если Клиент Тогда
	...
	Процедура ЗаполнитьПостроительОтчетаПоЦенамНоменклатуры() Экспорт
		...
		ПостроительОтчета.Параметры.Вставить("ВысотаИзображения", ВысотаИзображения);
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ЭтоНижнийУровень Тогда
				...
				Для Тмп = 1 По мКоличествоДопПолейДо Цикл
					...
					Если Найти(мСоответствиеДопПолейДо[Тмп], "Изображение") > 0 Тогда
						
						Если  НЕ Выборка.Номенклатура.ОсновноеИзображение.Пустая() Тогда
							мТабДок.Область(мТабДок.ВысотаТаблицы,1).ВысотаСтроки = ?(ВысотаИзображения >0,ВысотаИзображения,100);
							ВысотаИзображения = мТабДок.Область(мТабДок.ВысотаТаблицы,1).ВысотаСтроки;
							СекцияДопПоле.ТекущаяОбласть.ШиринаКолонки =  мТабДок.Область(мТабДок.ВысотаТаблицы,1).ВысотаСтроки/5;
							Рисунок = СекцияДопПоле.Рисунки.Добавить(ТипРисункаТабличногоДокумента.Картинка); 
							СекцияДопПоле.Рисунки[Рисунок.Имя].Картинка =(Выборка.Номенклатура.ОсновноеИзображение.Хранилище.Получить());
							Рисунок.РазмерКартинки = РазмерКартинки.Пропорционально;
							//Рисунок.ГраницаСлева = Ложь;
							Рисунок.Расположить(СекцияДопПоле.ТекущаяОбласть);
							Рисунок.ГиперСсылка = Истина;
							СекцияДопПоле.Параметры.ЗначениеДопПоля = "";						
						Иначе
							СекцияДопПоле.ТекущаяОбласть.ШиринаКолонки = ?(ВысотаИзображения >0,ВысотаИзображения,100)/5;
							Попытка
								СекцияДопПоле.Параметры.ЗначениеДопПоля = "";
								СекцияДопПоле.Рисунки.Удалить(Рисунок.Имя);
					...
					Иначе 
						//БЫЛО
						//ЗначениеДопПоля = СекцияДопПоле.Параметры.Добавить("ЗначениеДопПоля");
						СекцияДопПоле.Параметры.ЗначениеДопПоля = Выборка[мСоответствиеДопПолейдо[Тмп]];
						СекцияДопПоле.ТекущаяОбласть.ШиринаКолонки =  12.75;//Стандарт
						Попытка
							СекцияДопПоле.Рисунки.Удалить(Рисунок.Имя);
						Исключение
						Конецпопытки;
				...
				Для Тмп = 1 По мКоличествоДопПолейПосле Цикл
					...
					Если Найти(мСоответствиеДопПолейПосле[Тмп], "Изображение") > 0 Тогда
	...
	Процедура ПечатьПрайсЛиста(ДокументРезультат) Экспорт
		...
		Пока Выборка.Следующий() Цикл
			...
			//АБС ВСТАВКА   03.04.2014 14:11:03  Балашенко
			Если мЕстьЕдиница Тогда
				Секция.Параметры.СтрокаЦена = ?(Выборка.ТипЦен.ЦенаВключаетНДС, "Цена с НДС", "Цена без НДС");
			КонецЕсли;
			//АБС ВСТАВКА  КОНЕЦ
