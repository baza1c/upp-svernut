...
Функция УстановитьПараметр(ПутьКПараметру, Значение) Экспорт
	...
	Путь 	= EDI_РазложитьСтрокуВМассивСлов(ПутьКПараметру, ".");
...
&НаКлиенте
Функция ВремяНачалаЗамера(ВремяНачала = Неопределено) Экспорт

	Если ВремяНачала = Неопределено Тогда
		ВремяНачала = Метрика_ОтметкаВремени();	
	КонецЕсли;	
	
	Замеры = Метрика_ИзвлечьИзВХ("Замеры");
	
	Если Замеры = Неопределено Тогда
		Замеры = Новый Соответствие;
	КонецЕсли;
	
	Замеры.Вставить(СтруктураСобытияМетрики["TraceID"], ВремяНачала);
	
	Метрика_ПоместитьВоВХ("Замеры", Замеры);
	
	Возврат ЭтаФорма;

КонецФункции // ВремяНачалаЗамера()

&НаКлиенте
Функция ДлительностьЗамера(ВремяОкончания = Неопределено) Экспорт
	
	Длительность = Неопределено;

	Если ВремяОкончания = Неопределено Тогда
		ВремяОкончания = Метрика_ОтметкаВремени();	
	КонецЕсли;	
	...
	ВремяНачала = Замеры[СтруктураСобытияМетрики["TraceID"]];
	
	Если ТипЗнч(ВремяНачала) = ТипЗнч(ВремяОкончания) Тогда
		Замеры.Удалить(СтруктураСобытияМетрики["TraceID"]);
		Длительность = Метрика_Длительность(ВремяНачала, ВремяОкончания);
	КонецЕсли;
	
	Метрика_ПоместитьВоВХ("Замеры", Замеры);
		
	Возврат ДобавитьПеременную("Длительность", Длительность);

КонецФункции // ДлительностьЗамера()
...
&НаКлиенте
// Добавить готовый набор переменных в качестве дополнительной информации
//
// Параметры:
...
//   Форма
//
Функция ДобавитьПеременные(Переменные) Экспорт

	Если СтруктураСобытияМетрики["Variables"] = Неопределено Тогда
		СтруктураСобытияМетрики["Variables"] = Переменные;
	КонецЕсли;
	
	Возврат ЭтаФорма
	
КонецФункции // ДобавитьПеременные()
...
&НаКлиенте
// Если необходимо передать весь дополнительный контекст работы модуля, например, при начале работы или в момент ошибки
// Контекст формируется в функции Метрика_СформироватьКонтекстМодуля()
//
// Возвращаемое значение:
//   Форма
//
Функция КонтекстМодуля() Экспорт
	...
	Возврат Статистика().
			Событие("moduleContext").
			ДобавитьПеременные(Метрика_ИзвлечьИзВХ("КонтекстМодуля"));
...
&НаКлиенте
Функция Элемента(Элемент) Экспорт
...
&НаКлиенте
Функция Кнопки(Команда) Экспорт
	
	Возврат Метка(Команда.Имя)
			.ДобавитьПеременную("Команда.Действие", Команда.Действие);
...
&НаКлиенте
//Отправляет событие по нажатию гиперссылки в поле HTML документа
//
// Параметры:
//  ПредставлениеСсылки  	- Строка - представление ссылки для пользователя
//  ТекстСсылки  			- Строка - url 
//
// Возвращаемое значение:
//   Форма
//
Функция НажатаСсылка(ПредставлениеСсылки, ТекстСсылки = Неопределено) Экспорт
	
	Возврат Поведение()
			.Метка(ПредставлениеСсылки)
			.ДобавитьПеременную("url", ТекстСсылки)
			.Событие("linkClick");
...
//
// Параметры:
//  Метка  - имя выбранного объекта
//
// Возвращаемое значение:
//   Форма
//
Функция Выбор(Метка) Экспорт
...
&НаКлиенте
// Отправляет данные о предупреждении
//
// Параметры:
//  Место					- Строка - событие, для фиксации информации
//  ОписаниеПредупреждения	- Строка - информация о событии
//
// Возвращаемое значение:
//   Форма
//
Функция Предупреждение_(Место, ОписаниеПредупреждения) Экспорт
	
	Возврат Журнал()
			.Событие("warning")
			.Категория(Место)
			.ССистемнойИнформацией()
			.Метка(ОписаниеПредупреждения);
			
КонецФункции // Предупреждение_()
...
&НаКлиенте
// В переменные устанавливает подробное описание ошибки
// 
Функция ПодробноеОписаниеОшибки(ИнформацияОбОшибке = Неопределено) Экспорт

	ПодробноеОписаниеОшибки = Метрика_СформироватьПодробноеОписаниеОшибки(ИнформацияОбОшибке);
	
	Возврат ДобавитьПеременную("ИнформацияОбОшибке", ПодробноеОписаниеОшибки);

КонецФункции // ПодробноеОписаниеОшибки()
...
&НаКлиенте
Функция Метрика_Длительность(ВремяНачала, ВремяОкончания)
	
	Знаменатель = ?(ТипЗнч(ВремяНачала) = Тип("Число"), 1000, 1);
	Возврат (ВремяОкончания - ВремяНачала) / Знаменатель; 
	
КонецФункции

&НаКлиенте
Функция Метрика_НовыйУИД() Экспорт
...
&НаКлиенте
// На вход подается объект ИнформацияОбОшибке()
// на выходе Структура с полями: ИмяМодуля, ИсходнаяСтрока, НомерСтроки, Описание, ПричинаОписание
// так должно быть удобнее отслеживать ошибки в метрике
//
Функция Метрика_СформироватьПодробноеОписаниеОшибки(ИнформацияОбОшибке = Неопределено)
	...
	ПодробноеОписаниеОшибки.Вставить("ИмяМодуля",			ИнформацияОбОшибке.ИмяМодуля);
	ПодробноеОписаниеОшибки.Вставить("ИсходнаяСтрока",		ИнформацияОбОшибке.ИсходнаяСтрока);
	ПодробноеОписаниеОшибки.Вставить("НомерСтроки",			ИнформацияОбОшибке.НомерСтроки);
	ПодробноеОписаниеОшибки.Вставить("Описание",			ИнформацияОбОшибке.Описание);
	ПричинаОписание = Неопределено;
	Если НЕ ИнформацияОбОшибке.Причина = Неопределено Тогда  
		ПричинаОписание = ИнформацияОбОшибке.Причина.Описание;
    КонецЕсли;
	ПодробноеОписаниеОшибки.Вставить("ПричинаОписание",		ПричинаОписание);
	
	Возврат ПодробноеОписаниеОшибки;

КонецФункции // Метрика_СформироватьПодробноеОписаниеОшибки()
...
Функция Метрика_ТиповаяСтруктураМетрики() Экспорт
	...
	СтруктураСобытия.Вставить("Session", 				Метрика_НовыйСессия(Объект.ПараметрыКлиентСерверМетрика.ИдентификаторСессии,
																			,,,,
																			Объект.ПараметрыКлиентСерверМетрика.СессияEDI));
...
&НаКлиенте
Функция Метрика_НовыйСессия(CustomSessionId, 
							СессияКонтурЭкстерн = Неопределено, 
							СессияНорматив = Неопределено, 
							СессияПортал = Неопределено, 
							СессияПаспорт = Неопределено, 
							СессияEDI = Неопределено) Экспорт
	
	Сессия = Новый Соответствие;	
	Сессия.Вставить("CustomSessionId",				CustomSessionId);	
	
	Если ЗначениеЗаполнено(СессияКонтурЭкстерн) Тогда
		Сессия.Вставить("KeSession",				СессияКонтурЭкстерн);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СессияНорматив) Тогда
		Сессия.Вставить("NormativeSession",			СессияНорматив);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СессияПортал) Тогда
		Сессия.Вставить("PortalSession",			СессияПортал);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СессияПаспорт) Тогда
		Сессия.Вставить("KonturPassportSession",	СессияПаспорт);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СессияEDI) Тогда
		Сессия.Вставить("EDISession",				СессияEDI);
	КонецЕсли;
