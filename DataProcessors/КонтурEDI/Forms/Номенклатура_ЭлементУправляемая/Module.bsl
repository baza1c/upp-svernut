...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТипЕдиницы 			= Параметры.ТипЕдиницы;
	КодТовараВнутренний = Параметры.КодТовараВнутренний;
	//
	//+
	...
	мИмяСправочникаНоменклатура = МодульОбъекта().ПолучитьТипЗначенияОбъекта("Номенклатура",,Истина);   
...
Процедура ДобавитьНовуюНоменклатуруСервер(ВыбраннаяНоменклатура)
	...
	ТипНоменклатуры = МодульОбъекта().ПолучитьТипЗначенияОбъекта("Номенклатура");
		...
		Если ЗначениеЗаполнено(ВыбраннаяНоменклатура) Тогда
			...
			Если ИмяКонфигурации1С = "УФ_УТ" или ИмяКонфигурации1С = "УФ_УНФ" или ИмяКонфигурации1С = "УФ_Розница" или ИмяКонфигурации1С = "УФ_БП" Тогда
			...
			ИначеЕсли НЕ ИмяКонфигурации1С = "БП" Тогда
				...
				ИмяРеквизитаЕдиницаИзмерения = МодульОбъекта().ПолучитьИмяРеквизитаВладельцаПоИмениТипа("Номенклатура", "ЕдиницаИзмерения",СтандартнаяОбработка);
				
				Если СтандартнаяОбработка = Истина И МодульОбъекта().ЕстьНеобходимыеМетаданные("Справочники." + мИмяСправочникаНоменклатура + ".Реквизиты.ЕдиницаХраненияОстатков") Тогда
					
					НоваяСтрока.ЕдиницаИзмерения = ВыбраннаяНоменклатура.ЕдиницаХраненияОстатков;
					
				Иначе
															    
					НоваяСтрока.ЕдиницаИзмерения = ВыбраннаяНоменклатура[ИмяРеквизитаЕдиницаИзмерения];
			...
			ТипХарактеристикаНоменклатуры     = МодульОбъекта().ПолучитьТипЗначенияОбъекта("ХарактеристикаНоменклатуры",,Истина);
			ЕстьТипХарактеристикаНоменклатуры = НЕ ТипХарактеристикаНоменклатуры = Неопределено;
			
			Если ЕстьТипХарактеристикаНоменклатуры = Истина Тогда 
				
				НоваяСтрока.ХарактеристикаНоменклатуры = Справочники[ТипХарактеристикаНоменклатуры].ПустаяСсылка();
...
Процедура ОформитьПодсказкиЭлементовУправления()
	...
	Иначе
		Если ТипЕдиницы = "Services" Тогда
...
Процедура ПриОткрытии(Отказ)
	...
	ОсновнаяФорма().ПриОткрытииФормы(ЭтаФорма, "Номенклатура_ЭлементУправляемая");
...
Функция ПолучитьТоварыПоКодуШтрихкоду()
	...
	Если ЗначениеЗаполнено(GTIN) Тогда
		...
		НайденныеТоварыПоШК = МодульОбъекта().НайтиТоварыПоШтрихкоду(СокрЛП(GTIN), ТорговаяСеть, КодТовараПартнера);
	...
	Если ЗначениеЗаполнено(КодТовараВнутренний) 
		И ЗначениеЗаполнено(Справочники.Номенклатура.НайтиПоКоду(КодТовараВнутренний)) Тогда
		...
		ТипХарактеристики = МодульОбъекта().ПолучитьТипЗначенияОбъекта("ХарактеристикаНоменклатуры", , Истина);
		Если НЕ ТипХарактеристики = Неопределено Тогда
			НоваяСтрока.ХарактеристикаНоменклатуры = Справочники[ТипХарактеристики].ПустаяСсылка();
		КонецЕсли;
		
		НоваяСтрока.ЕдиницаИзмерения = МодульОбъекта().УФ_ПолучитьБазовуюЕдиницуИзмерения(НоваяСтрока.Номенклатура);
...
Процедура СписокНоменклатурыНоменклатураПриИзмененииСервер(ИдентификаторТекСтроки)
	...
	Если ТекСтрока<>Неопределено Тогда 
		...
		ИначеЕсли ИмяКонфигурации1С = "УФ_Розница" Тогда
			ТекСтрока.ЕдиницаИзмерения = Справочники.УпаковкиНоменклатуры.ПустаяСсылка();
		Иначе
...
Процедура СписокНоменклатурыХарактеристикаНоменклатурыНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	МодульОбъектаКлиент().ОткрытьФорму_КонтурEDI("Справочник.ХарактеристикиНоменклатуры.ФормаВыбора", "", ПараметрыФормыВыбора, ЭтаФорма, "ДобавитьХарактеристикуЗавершение", ЭтаФорма, ДополнительныеПараметры);
...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	ОсновнаяФорма().ПриЗакрытииФормы(ЭтаФорма, "Номенклатура_ЭлементУправляемая");	
