...
Функция ПеренестиВЗаказСервер()
	...
	Попытка
		...
		ЗаполнитьТоварыДокумента_Реализация_Заказ(Док, ТабСоответствий, "Заказ");
		РежимЗаписи = ?(Док.Проведен, РежимЗаписиДокумента.Проведение, РежимЗаписиДокумента.Запись);
		Док.Записать(РежимЗаписи);
		
	Исключение
		
		ТекстОшибки = "Не удалось перенести расхождения в документ заказа - " + ОписаниеОшибки();
...
Функция ПеренестиВДокументРеализацияСервер()
	...
	Результат = МодульОбъекта().ОбработкаСобытияПодключаемогоМодуля("ОбработатьРасхожденияРезультатовПриемки",
	...
	Если СтандартнаяОбработкаEDI = Истина Тогда
		...
		ЗаполнитьТоварыДокумента_Реализация_Заказ(Док, ТабСоответствий, "Реализация");
	    ...
	    //Для УФ_УТ изменения серий
		//Програмно нажимаем кнопку Заполнить серии по FEFO
		Если МодульОбъекта().ИмяКонфигурации1С = "УФ_УТ" Тогда
			
			Попытка
				Выполнить("ЕстьСтрокиСЗаполняемымиПоFEFOСериями=НоменклатураСервер.ЕстьСтрокиСЗаполняемымиПоFEFOСериями(Док.Товары);");
			Исключение
				ЕстьСтрокиСЗаполняемымиПоFEFOСериями=Ложь;
			КонецПопытки;
			
			Если ЕстьСтрокиСЗаполняемымиПоFEFOСериями Тогда
				
				Попытка
					Выполнить("НоменклатураСервер.ЗаполнитьСерииПоFEFO(Док, Новый ФиксированнаяСтруктура(НоменклатураСервер.ПараметрыУказанияСерий(Док, Документы.РеализацияТоваровУслуг)));");	
				Исключение
					МодульОбъекта().ЗаписьЖурналаРегистрации_КонтурEDI(УровеньЖурналаРегистрации.Ошибка, "Не удалось заполнить серии при создании реализации - " + ОписаниеОшибки());	
				Конецпопытки;
...
Процедура СоздатьВозвратОтПокупателя(Команда)
	...
	Если СсылкаДокумента <> Неопределено Тогда
		...
		МодульОбъектаКлиент().ОткрытьФорму_КонтурEDI(ИмяФормыДокумента, "", ПараметрыФормы, ЭтаФорма, "ОбработчикЗакрытияСозданногоДокумента", ЭтаФорма);
...
&НаКлиенте
Процедура ОбработчикЗакрытияСозданногоДокумента(Параметр=Неопределено, ДопПараметр=Неопределено) Экспорт
	
	ЭтаФорма.Закрыть();

КонецПроцедуры // ОбработчикЗакрытияСозданногоДокумента()
	...
	Для Каждого СтрокаСоответствий Из ТабСоответствий Цикл
		...
		Если НЕ НайденнаяСтрока = Неопределено Тогда
			...
			ИначеЕсли СтрокаСоответствий.КоличествоПринято < СтрокаСоответствий.КоличествоВНакладной Тогда
				...
				Если МодульОбъекта().ИмяКонфигурации1С="УФ_УТ" ИЛИ  МодульОбъекта().ИмяКонфигурации1С="УФ_Розница" Тогда 
	...
	Если СтандартнаяОбработкаEDI = Истина Тогда
		Если МодульОбъекта().ИмяКонфигурации1С = "УФ_УТ" Тогда 
			...
			Для Каждого СтрокаRECADV из _Таб Цикл
				Если 	 НЕ СтрокаRECADV.КоличествоВНакладной  = СтрокаRECADV.КоличествоПринято
					 ИЛИ НЕ СтрокаRECADV.СуммаБезНДСВНакладной = СтрокаRECADV.СуммаБезНДСПринято
					 ИЛИ НЕ СтрокаRECADV.СуммаСНДСВНакладной   = СтрокаRECADV.СуммаСНДСПринято Тогда

					НайденнаяСтрока = МодульОбъекта().НайтиСоответствиеСтрокиВДокументе(СтрокаRECADV, ДокументКорректировки, "Товары");  					 
					
					Если НайденнаяСтрока<>Неопределено Тогда 
						
						НайденнаяСтрока.КоличествоУпаковок=СтрокаRECADV.КоличествоПринято;
						_Цена = ?(Документ.ЦенаВключаетНДС,СтрокаRECADV.ЦенаСНДСПринято,СтрокаRECADV.ЦенаБезНДСПринято);
						_Сумма = ?(Документ.ЦенаВключаетНДС,СтрокаRECADV.СуммаСНДСПринято,СтрокаRECADV.СуммаБезНДСПринято);
			...
			Для Каждого СтрокаRECADV из _Таб Цикл
				...
				Если 	 НЕ СтрокаRECADV.КоличествоВНакладной  = СтрокаRECADV.КоличествоПринято
					...
					Если НайденнаяСтрока<>Неопределено Тогда 
						...
						_Цена = ?(ДокументКорректировки.СуммаВключаетНДС,СтрокаRECADV.ЦенаСНДСПринято,СтрокаRECADV.ЦенаБезНДСПринято);
		...
		ИначеЕсли МодульОбъекта().ИмяКонфигурации1С = "УФ_УНФ" Тогда
			...
			ТЧДокумента=ДокументКорректировки.Запасы;
			Для Каждого СтрокаRECADV из _Таб Цикл
				...
				Если 	 НЕ СтрокаRECADV.КоличествоВНакладной  = СтрокаRECADV.КоличествоПринято
					...
					Если НайденнаяСтрока<>Неопределено Тогда 
						...
						НайденнаяСтрока.СуммаНДС = ?(ДокументКорректировки.СуммаВключаетНДС, 
													НайденнаяСтрока.Сумма - (НайденнаяСтрока.Сумма) / ((СтавкаНДС + 100) / 100),
													НайденнаяСтрока.Сумма * СтавкаНДС / 100);
						
						НайденнаяСтрока.Всего = НайденнаяСтрока.Сумма + ?(ДокументКорректировки.СуммаВключаетНДС, 0, НайденнаяСтрока.СуммаНДС);
...
Процедура ПриОткрытии(Отказ)
	...
	ОсновнаяФорма().ПриОткрытииФормы(ЭтаФорма, "Сервис_ОбработкаРасхожденийПриемкиУправляемая");
...
Процедура ЗаполнитьТоварыДокумента_Реализация_Заказ(Док,ТабСоответствий,ВариантЗаполнения)
	...
	МассивНеобработанныхСтрокКорректировки	= Новый Массив;
	МассивСтрокДляСвоихЦен					= Новый Массив; 
	...
	Для Каждого СтрокаСоответствий Из ТабСоответствий Цикл
		...
		Если НЕ НайденнаяСтрока = Неопределено Тогда
			...
			Иначе //КоличествоПринято<>0, нам надо изменить Количество в строке документа, а также при необходимости перезаполнить/пересчитать цены
				...
				Если СтрокаСоответствий.КоличествоПринято <> НайденнаяСтрока.Количество 
					ИЛИ ((МодульОбъекта().ИмяКонфигурации1С="УФ_УТ" ИЛИ МодульОбъекта().ИмяКонфигурации1С="УФ_Розница") И СтрокаСоответствий.КоличествоПринято<>НайденнаяСтрока.КоличествоУпаковок) 
					Тогда
		...
		ИначеЕсли НЕ СтрокаСоответствий.КоличествоПринято = 0 Тогда
			...
			Если МодульОбъекта().ПеременнаяСодержитСвойство(НоваяСтрока, "КоличествоУпаковок") Тогда
			...
			МодульОбъекта().ПриИзмененииНоменклатурыВСтрокеДокумента_КонтурEDI(НоваяСтрока, Док);
			...
			Если НоваяСтрока.Цена = 0 Тогда
				МассивСтрокДляСвоихЦен.Добавить(НоваяСтрока);
			КонецЕсли;  
			
		КонецЕсли;
	КонецЦикла;//по строкам
	
	Если МодульОбъекта().ПеременнаяСодержитСвойство(Док, "ТипЦен") Тогда
		ТипЦен = Док.ТипЦен;
	ИначеЕсли МодульОбъекта().ПеременнаяСодержитСвойство(Док, "ВидЦен") Тогда
		ТипЦен = Док.ВидЦен;	
	Иначе
		ТипЦен = Неопределено;
	КонецЕсли;  	
	
	МодульОбъекта().ЗаполнитьСтрокиТаблицыСвоимиЦенами(МассивСтрокДляСвоихЦен, ТипЦен, Док.Дата, Ложь, Истина, Док);
...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	ОсновнаяФорма().ПриЗакрытииФормы(ЭтаФорма, "Сервис_ОбработкаРасхожденийПриемкиУправляемая");	
