...
//		Элементы.ТаблицаТочекДоставкиОбновитьСписокТочекДоставок.Доступность = ЗначениеЗаполнено(КодСети);
	...
	Иначе
		...
		Если МыПоставщик И МыТорговаяСеть Тогда
			// нужно выбрать руками, мы не знаем кто это
		ИначеЕсли МыПоставщик Тогда
...
Процедура УправлениеВидимостью()
	...
	Если МодульОбъекта().ПараметрыПользователяEDI.ФильтрОбменаПоТочкамДоставки = Истина Тогда
		Элементы.ТаблицаТочекДоставкиЗагружатьСообщения.Видимость = Истина;
	Иначе
		Элементы.ТаблицаТочекДоставкиЗагружатьСообщения.Видимость = Ложь;
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если МодульОбъектаКлиент().ПеременнаяСодержитСвойство(Элементы.ПолеПодсказки, "АвтоМаксимальнаяШирина") Тогда	
		Элементы.ПолеПодсказки.АвтоМаксимальнаяШирина = Ложь;
	КонецЕсли;
	
	ОсновнаяФорма().ПриОткрытииФормы(ЭтаФорма, "Партнеры_ЭлементУправляемая");
...
Процедура ЗаписатьНастройкиАвтотестирования()
	...
	Если Не ЗаписатьИзменения_Клиент() Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого СтрТоваров Из СтруктураПараметров.СоответствияТоваров Цикл
		...
		МодульОбъектаКлиент().ОткрытьФорму_КонтурEDI("Номенклатура_ЭлементУправляемая", , ПараметрыФормы, ЭтаФорма);
...
Процедура ОбновитьСписокПартнеров(Команда)
	...
	МодульОбъектаКлиент().ОткрытьФорму_КонтурEDI("Сервис_ОбновлениеЮрЛицСетиУправляемая", , ПараметрыФормы, ЭтаФорма);
...
Процедура ОбновитьСписокТочекДоставок(Команда)
	...
	МодульОбъектаКлиент().ОткрытьФорму_КонтурEDI("Сервис_ОбновлениеДанныхПартнераУправляемая", , ПараметрыФормы, ЭтаФорма, "ОбработчикПослеПроверкиНовыхТД", ЭтаФорма);
...
Процедура КоманднаяПанельНоменклатурыДобавить(Команда)
	...
	Если Не ЗначениеЗаполнено(Параметры.СсылкаНаПартнера) Тогда 
		...
		Если ЗаписатьИзменения_Клиент() Тогда
			//все ок
	...
	МодульОбъектаКлиент().ОткрытьФорму_КонтурEDI("Номенклатура_ЭлементУправляемая", , ПараметрыФормы, ЭтаФорма, "ОбработчикПослередактированияНоменклатуры", ЭтаФорма);
...
Процедура ДеревоНастроекЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ТекСтрока.Список) Тогда
		...
		Список = ЗначениеИзСтрокиВнутрВызовСервера(ТекСтрока.Список);
		
		Если Параметры.МодальностьЗапрещена Тогда
			Выполнить("ПоказатьВыборИзСписка(Новый ОписаниеОповещения(""ОбработчикВыбораЗначения"", ЭтаФорма), Список,Элемент,)");
		Иначе
			Выбранный = Список.ВыбратьЭлемент();
			ОбработчикВыбораЗначения(Выбранный); 
		КонецЕсли;
...
&НаКлиенте
Процедура ОбработчикВыбораЗначения(ВыбранноеЗначение, Параметр = Неопределено) Экспорт
	
	Если НЕ ВыбранноеЗначение = Неопределено Тогда
		
		Элементы.ДеревоНастроек.ТекущиеДанные.Значение = ВыбранноеЗначение.Значение;
...
&НаКлиенте
Процедура ВывестиПредупреждения_КонтурEDI(МассивПредупреждений) Экспорт
	
	Если ТипЗнч(МассивПредупреждений) = Тип("Массив") И МассивПредупреждений.Количество() > 0 Тогда
		
		ТекстПредупреждения = МассивПредупреждений[0];
		
		МассивПредупреждений.Удалить(0);
		
		Если Параметры.МодальностьЗапрещена Тогда
			Выполнить("ПоказатьПредупреждение(Новый ОписаниеОповещения(""ВывестиПредупреждения_КонтурEDI"", ЭтаФорма, МассивПредупреждений),ТекстПредупреждения, 5,""Контур.EDI"")");
		Иначе
			Предупреждение(ТекстПредупреждения, 5,"Контур.EDI");
			ВывестиПредупреждения_КонтурEDI(МассивПредупреждений)
		КонецЕсли;
...
&НаКлиенте
Функция ЗаписатьИзменения_Клиент()

	РезультатЗаписиНастройки = ЗаписатьИзменения();
	
	Если Не РезультатЗаписиНастройки.Успешно Тогда
			
		РезультатЗаписиНастройки.РезультатПроверки.Добавить("Не удалось записать настройки");
		
		ВывестиПредупреждения_КонтурEDI(РезультатЗаписиНастройки.РезультатПроверки);
		
	КонецЕсли;
	
	Возврат РезультатЗаписиНастройки.Успешно; 
...
Функция ЗаписатьИзменения()
	...
	РезультатЗаписиНастройки = Новый Структура("Успешно,РезультатПроверки", Истина, Новый Массив);
	
	Если НЕ ПроверитьЗначенияФормы() Тогда
		РезультатЗаписиНастройки.Успешно = Ложь;
		Возврат РезультатЗаписиНастройки;
	КонецЕсли;
	...
	МодульОбъекта().ПодготовитьНастройкиПартнера_КонтурEDI(ДеревоНастроек,СтруктураПолейПартнера);
	
	РезультатПроверки =  МодульОбъекта().ПроверитьКорректностьНастроекПартнераПередЗаписью(СтруктураПолейПартнера);
		
	Если РезультатПроверки.Отказ Тогда
		
		РезультатПроверки.РезультатПроверки.Добавить("Внимание! Если вы не до конца понимаете значение этих настроек - обратитесь в техподдержку."); 		
		РезультатЗаписиНастройки.РезультатПроверки	= РезультатПроверки.РезультатПроверки;		
		РезультатЗаписиНастройки.Успешно			= Ложь;
    ...
    Исключение
		...
		ТекстОшибки = "Не удалось записать партнера - " + ОписаниеОшибки();
...
Процедура ТаблицаТочекДоставкиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если НЕ ТекСтрока = Неопределено Тогда
		...
		МодульОбъектаКлиент().ОткрытьФорму_КонтурEDI("ТочкиДоставкиСторонние_ЭлементУправляемая", , ПараметрыФормы, ЭтаФорма, "ОбработчикМодификацииТочкиДоставки", ЭтаФорма);
	КонецЕсли;
...
Процедура КРасширениям(Команда)
	...
	Если ЗначениеЗаполнено(Параметры.СсылкаНаПартнера) Тогда 
		...
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("Партнер",Параметры.СсылкаНаПартнера);
		МодульОбъектаКлиент().ОткрытьФорму_КонтурEDI("ФормаРасширения_СписокУправляемая", , ПараметрыФормы, ЭтаФорма);
...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	ОсновнаяФорма().ПриЗакрытииФормы(ЭтаФорма, "Партнеры_ЭлементУправляемая");	
