...
Функция СтруктураОбщихРеквизитовДокумента()
	...
	СтруктураРеквизитов = Новый Структура("Организация, НалоговыйПериод, КодИФНС, ОснованиеДляПредставленияСправок, Ответственный,ГлавныйБухгалтер, Телефон, СправкуПодписал, ДолжностьПодписавшегоЛица, Дата, ОрганизацияЭтоЮрЛицо");
...
Процедура ЗаполнитьДанныеСправок(НовыйДок, НомерПервойСправки)
	...
	КонецЦикла;
	СтруктураДанных = СправкиПоНДФЛ.ДанныеОДоходах(НовыйДок.СотрудникиОрганизации.Выгрузить(), НовыйДок.НалоговыйПериод, НовыйДок.Организация, ОбщегоНазначенияЗК.ГоловнаяОрганизация(НовыйДок.Организация), НовыйДок.ОКАТО_КПП, НачалоМесяца(НовыйДок.Дата));
...
Функция Автозаполнение() Экспорт
	...
	Пока Выборка.СледующийПоЗначениюПоля("ОКАТО_КПП") Цикл
		...
		ТекущийКодНО = ?(СтруктураРеквизитов.ОрганизацияЭтоЮрЛицо, СправкиПоНДФЛ.КодНалоговогоОрганаПоОКАТО_КППиОрганизации(Организация, СтруктураРеквизитов.ОрганизацияЭтоЮрЛицо, Выборка.ОКАТО_КПП), СтруктураРеквизитов.КодИФНС);
