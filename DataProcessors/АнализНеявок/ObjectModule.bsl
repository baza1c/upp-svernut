...
Процедура ПровестиДокументы(СтрокиНеявок) Экспорт
	...
	Для Каждого РасчетныйДокумент Из МассивУспешныхДокументов Цикл
		ОбщегоНазначенияЗК.КомментарийРасчета("Документ №" + РасчетныйДокумент.Номер + " от " + РасчетныйДокумент.Дата + " обработан успешно. ", Заголовок, "Открыть", СтрокаТаблицы.РасчетныйДокумент);
	КонецЦикла;
