...
Функция ПолучитьЦену(Строка, ТипЦен, ДатаЦены, СтратегияРасчета, ФункцияРасчета)
	...
	|							ТОГДА 10     
	|						КОГДА Цены.Номенклатура.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС5)
	|								ИЛИ Цены.Номенклатура.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС5_105)
	|							ТОГДА 5      
	|						КОГДА Цены.Номенклатура.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС7)
	|								ИЛИ Цены.Номенклатура.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС7_107)
	|							ТОГДА 7
	|						ИНАЧЕ 0
	|					КОНЕЦ
	|			ИНАЧЕ 0
