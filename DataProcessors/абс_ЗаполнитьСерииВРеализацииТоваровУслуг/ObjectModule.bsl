Процедура ЗаполнитьНачальныеЗначенияСвойств() Экспорт
	...
	ПроцентСвежестиПоУмолчанию = ПолучитьЗначениеСвойства(ДокументРеализация.Контрагент,ПланыВидовХарактеристик.СвойстваОбъектов.абс_ПроцентСвежестиТовара);
	...
	ОстаточныйСрокГодностиПоУмолчанию = ПолучитьЗначениеСвойства(ДокументРеализация.Контрагент,ПланыВидовХарактеристик.СвойстваОбъектов.абс_ОстаточныйСрокГодности);
	...
	Запрос.УстановитьПараметр("Объект", ДокументРеализация.Контрагент);
...
Процедура ОбновитьТаблицуТоваров() Экспорт
	...
	Для Каждого СтрокаТаб Из ДокументРеализация.Товары Цикл
...
Процедура ЗаполнитьТоварыВозможнымРазмещением(Параметры, Товары) Экспорт
	...
	СкладПоУмолчанию = ЗаказСсылка.Склад;
	//-Лео Сафонов ЕА
...
Процедура РассчитатьСуммуНДСТабЧасти(СтрокаТабличнойЧасти)
	...
	УчитыватьНДС     = ДокументРеализация.УчитыватьНДС;
	СуммаВключаетНДС = ДокументРеализация.СуммаВключаетНДС;
