...
Процедура ЗаполнитьСтрокиРеквизитовПлДокументовДляИмпорта()
	...
	//никитин 04062020 изменился формат файла выписки
...
Процедура СформироватьСоответствиеЗагружаемых()
	...
	//АБС ВСТАВКА Фролов
	Импорт_Загружаемые.Вставить("КОД", Истина);
...
Процедура РаспознатьДанныеВСтрокеДокумента(СтрокаДокумента)
	...
	Если Не ЗначениеЗаполнено(СтрокаДокумента.ВидОперации) Тогда
		...
		//+Лаврухин
		Если ЗначениеЗаполнено(СтрокаДокумента.Операция) Тогда
			Если СтрокаДокумента.Операция = "Банковский ордер" Тогда
				ВидОперацииДокумента = Перечисления.ВидыОперацийСписаниеБезналичныхДенежныхСредств.ПрочееСписаниеБезналичныхДенежныхСредств;//СтрокаДокумента.ВидОперации;	
				СтрокаДокумента.ВидОперации = ВидОперацииДокумента;
			КонецЕсли;
		КонецЕсли;
		// {Лео Катанович
		...
		//		Если Лев(СтрокаДокумента.НазначениеПлатежа, 25) = "Для зачисления по реестру" Тогда
		//			СтрокаДокумента.Контрагент = Справочники.Контрагенты.НайтиПоКоду("К00000006",Истина);
		//			СтрокаДокумента.ВидОперации = Перечисления.ВидыОперацийСписаниеБезналичныхДенежныхСредств.ПеречислениеЗП;
		//		КонецЕсли;	
		//	КонецЕсли;	
		//КонецЕсли;
		//Если ЗначениеЗаполнено(СтрокаДокумента.Операция) Тогда
		//	Если СтрокаДокумента.Операция = "Платежное поручение" Тогда
		//		Если Лев(СтрокаДокумента.НазначениеПлатежа, 27) = "Для зачисления отпускных за" Тогда					
		//			СтрокаДокумента.ВидОперации = Перечисления.ВидыОперацийСписаниеБезналичныхДенежныхСредств.ПеречислениеЗП;
		//		КонецЕсли;	
		//	КонецЕсли;	
		//КонецЕсли;	
		//Если ЗначениеЗаполнено(СтрокаДокумента.Операция) Тогда
		//	Если СтрокаДокумента.Операция = "Платежное поручение" Тогда
		//		Если Лев(СтрокаДокумента.НазначениеПлатежа, 23) = "Для зачисления зарплаты" Тогда
		//			СтрокаДокумента.Контрагент = Справочники.Контрагенты.НайтиПоКоду("К00000006",Истина);   // По всем сотрудникам
		//			СтрокаДокумента.ВидОперации = Перечисления.ВидыОперацийСписаниеБезналичныхДенежныхСредств.ПеречислениеЗП;
		//			СтрокаДокумента.СтатьяДДС = Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоКоду("000000003",Истина); // Оплата труда
		//		КонецЕсли;	
		...
		Если ЗначениеЗаполнено(СтрокаДокумента.Операция) Тогда
			Если СтрокаДокумента.Операция = "Платежное поручение" Тогда
				Если Лев(СтрокаДокумента.НазначениеПлатежа, 25) = "Для зачисления по реестру" ИЛИ
					Лев(СтрокаДокумента.НазначениеПлатежа, 23) = "Для зачисления зарплаты" ИЛИ
					Лев(СтрокаДокумента.НазначениеПлатежа, 20) = "Для зачисл. зарплаты" ИЛИ     //никитин    
                    Лев(СтрокаДокумента.НазначениеПлатежа, 18) = "Для зачисл. аванса" ИЛИ              
					Лев(СтрокаДокумента.НазначениеПлатежа, 21) = "Для зачисл. Отпускных" ИЛИ
					Лев(СтрокаДокумента.НазначениеПлатежа, 20) = "Отпускные по реестру" ИЛИ
					Лев(СтрокаДокумента.НазначениеПлатежа, 16) = "Заработная плата" ИЛИ    //23.04.2020
					Лев(СтрокаДокумента.НазначениеПлатежа, 20) = "Пособие по временной" ИЛИ     //23.04.2020
					Лев(СтрокаДокумента.НазначениеПлатежа, 21) = "Расчет при увольнении" ИЛИ     //23.04.2020
					Лев(СтрокаДокумента.НазначениеПлатежа, 23) = "Премия (вознаграждение)" ИЛИ     //23.04.2020
					
					                                              					                                   
					Найти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), "ЗАРАБОТНАЯ ПЛАТА В СООТВЕТСТВИИ С ДОГОВОРОМ") > 0 ИЛИ
					Найти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), "ДЛЯ ЗАЧИСЛ. ЗАРПЛАТЫ") > 0  ИЛИ
					Найти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), "ЗАРАБОТНОЙ ПЛАТЕ") > 0  ИЛИ //23.04.2020
                    Найти(СтрокаДокумента.НазначениеПлатежа, "№38051193") > 0  ИЛИ //23.04.2020
					...
					Найти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), "ДЛЯ ЗАЧИСЛЕНИЯ АВАНСА ПО ЗАРАБ. ПЛАТЕ") > 0  ИЛИ
					Найти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), "ДЛЯ ЗАЧИСЛЕНИЯ ОТПУСКНЫХ") > 0  ИЛИ
					Найти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), "ДЛЯ ЗАЧИСЛ.ПОСОБИЯ ПО УХОДУ") > 0  Тогда
					СтрокаДокумента.Контрагент = Справочники.Контрагенты.НайтиПоКоду("К00000006",Истина);
					СтрокаДокумента.ВидОперации = Перечисления.ВидыОперацийППИсходящее.ПеречислениеЗП;
					СтрокаДокумента.СтатьяДДС = Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоКоду("000000003",Истина); // Оплата труда
					Подстрока_ЛС = Найти(нрег(СтрокаДокумента.НазначениеПлатежа), "л/с ");
					Если Подстрока_ЛС > 0 Тогда
						Срока_ЛС = Сред(СтрокаДокумента.НазначениеПлатежа, Подстрока_ЛС, 25);
					    СтрокаДокумента.НазначениеПлатежа = СтрЗаменить(СтрокаДокумента.НазначениеПлатежа,Срока_ЛС,"");
					КонецЕсли;	
        ...
        Если ЗначениеЗаполнено(СтрокаДокумента.Операция) Тогда
			Если СтрокаДокумента.Операция = "Платежное поручение" Тогда
				Если Найти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), "ПО ДОГОВОРУ О ПРЕДОСТАВЛЕНИИ ОВЕРДРАФТНОГО КРЕДИТА") > 0 Тогда
					СтрокаДокумента.Контрагент = Справочники.Контрагенты.НайтиПоКоду("К00001251",Истина);
					СтрокаДокумента.Договор = Справочники.ДоговорыКонтрагентов.НайтиПоКоду("УТ0004221");
 					СтрокаДокумента.ВидОперации = Перечисления.ВидыОперацийПоступлениеБезналичныхДенежныхСредств.РасчетыПоКредитамИЗаймам;
					ВидОперацииДокумента = СтрокаДокумента.ВидОперации;
					СтрокаДокумента.СтатьяДДС = Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоКоду("000000031",Истина); // Кредиты и займы
					СтрокаДокумента.ПлательщикСчет = "30301810500001000014";    // по кредам
				КонецЕсли;	
			КонецЕсли;	
		КонецЕсли;
        // ЛЕО Катанович}
		...
		Если ЗначениеЗаполнено(СтрокаДокумента.Операция) Тогда
			Если СтрокаДокумента.Операция = "Платежное поручение" Тогда
				// {ЛЕО Катанович по просьбе Луханиной Г.А. 28.03.2016
				// Было
				//Если Лев(СтрокаДокумента.НазначениеПлатежа, 24) = "Для зач. опл. Дог.Аренды" Тогда 
				Если Лев(СтрокаДокумента.НазначениеПлатежа, 24) = "Для зач. опл. Дог.Аренды" ИЛИ Найти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), "АРЕНДЫ ТС") <> 0 Тогда
				// ЛЕО Катанович}
                    СтрокаДокумента.Контрагент = Справочники.Контрагенты.НайтиПоКоду("ККК002888",Истина);
					СтрокаДокумента.ВидОперации = Перечисления.ВидыОперацийППИсходящее.ПрочееСписаниеБезналичныхДенежныхСредств;
					ВидОперацииДокумента = СтрокаДокумента.ВидОперации;
					СтрокаДокумента.СтатьяДДС = Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоКоду("000000051",Истина);
				КонецЕсли;	
			КонецЕсли;
			// {ЛЕО Катанович по просьбе Луханиной Г.А. 16.05.2016
			Если СтрокаДокумента.Операция = "Платежное требование" Тогда
				Если СтрокаДокумента.ВидДокумента = "ПлатежноеТребованиеПолученное" Тогда
					Если Найти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), "СПИСАНИЕ СРЕДСТВ ПО СОГЛАШЕНИЮ") <> 0 Тогда
						СтрокаДокумента.Контрагент = Справочники.Контрагенты.НайтиПоКоду("К00001251",Истина); // Сбербанк
						СтрокаДокумента.Договор = Справочники.ДоговорыКонтрагентов.НайтиПоКоду("УТ0004221");  //Договор об овердрафтном кредите №3438 от 16.02.16г. 
						СтрокаДокумента.СчетКонтрагента = Справочники.БанковскиеСчета.НайтиПоРеквизиту("НомерСчета", "30301810500001000014");
						СтрокаДокумента.ВидОперации = Перечисления.ВидыОперацийСписаниеБезналичныхДенежныхСредств.РасчетыПоКредитамИЗаймам;
						ВидОперацииДокумента = СтрокаДокумента.ВидОперации;
						СтрокаДокумента.СтатьяДДС = Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоКоду("000000031",Истина); // Получение кредитов, займов, овердрафта
					КонецЕсли;	
				КонецЕсли;	
			КонецЕсли;	
			// ЛЕО Катанович по просьбе Луханиной Г.А. 16.05.2016}
	...
	Если Не ЗначениеЗаполнено(СтрокаДокумента.СчетКонтрагента) Тогда
		...
		Иначе
			//{Лео Катанович по просьбе Луханиной Г.А. 13/05/2016
			...
			СчетПлательщикаДляПоиска = СтрокаДокумента.ПлательщикСчет;
			Если Лев(СокрЛП(СтрокаДокумента.ПлательщикСчет),2) = "30" Тогда // задан корсчет 
				Попытка
					РсВНаименовании = Сред(СтрокаДокумента.Плательщик,(Найти(СтрокаДокумента.Плательщик,"р/с") + 4),20);
					Если Лев(РсВНаименовании,2) = "40" И СтрДлина(РсВНаименовании) = 20 и ОбщегоНазначения.ТолькоЦифрыВСтроке(РсВНаименовании) Тогда 
						СчетПлательщикаДляПоиска = РсВНаименовании;
					КонецЕсли;
				Исключение
				КонецПопытки	
			КонецЕсли;	
			ЗапросПоискаСчета.УстановитьПараметр("НомерСчета", СчетПлательщикаДляПоиска);
			СчетКонтрагента = СчетПлательщикаДляПоиска;
			//Лео Катанович по просьбе Луханиной Г.А. 13/05/2016}
	    ...
	    МассивИсключений.Добавить("ККК004694");
 	    ЗапросПоискаСчета.УстановитьПараметр("МассивИсключений", МассивИсключений);
		// Лео Катанович}
		...
		|	И БанковскиеСчета.НомерСчета = &НомерСчета И БанковскиеСчета.Владелец.Код НЕ В(&МассивИсключений)";
		...
		ИначеЕсли Лев(СтрокаДокумента.НазначениеПлатежа, 24) <> "Для зач. опл. Дог.Аренды" Тогда   
	...
	Если Не ЗначениеЗаполнено(СтрокаДокумента.Контрагент) Тогда
		...
		ИначеЕсли НЕ ПустаяСтрока(ИННКонтрагента) Тогда
			...
			ЗапросПоискаКонтрагента = Новый Запрос("ВЫБРАТЬ РАЗРЕШЕННЫЕ
													...
													// {Лео Катанович
													|	И Контрагенты.Код НЕ В (&МассивИсключений)
													// Лео Катанович}
													|");
			
			ЗапросПоискаКонтрагента.УстановитьПараметр("КонтрагентИНН", ИННКонтрагента);
			// {Лео Катанович
			ЗапросПоискаКонтрагента.УстановитьПараметр("МассивИсключений", МассивИсключений);
			// Лео Катанович}
	...
	Если Не ЗначениеЗаполнено(СтрокаДокумента.Договор) Тогда
		...
		Если Не ВидОперацииДокумента = Перечисления.ВидыОперацийППИсходящее.ПереводНаДругойСчет 
			...
			ИначеЕсли Лев(СтрокаДокумента.НазначениеПлатежа, 23) = "Для зачисления зарплаты" Тогда
				СтрокаДокумента.Договор = Справочники.ДоговорыКонтрагентов.ПустаяСсылка();
			ИначеЕсли Лев(СтрокаДокумента.НазначениеПлатежа, 27) = "Для зачисления отпускных за" Тогда
				СтрокаДокумента.Договор = Справочники.ДоговорыКонтрагентов.ПустаяСсылка();	
			ИначеЕсли Лев(СтрокаДокумента.НазначениеПлатежа, 24) = "Для зач. опл. Дог.Аренды" Тогда
				СтрокаДокумента.Договор = Справочники.ДоговорыКонтрагентов.ПустаяСсылка();
			ИначеЕсли СтрокаДокумента.Договор="Не найден"Тогда
	...
	Если ВидОперацииДокумента = Перечисления.ВидыОперацийПоступлениеБезналичныхДенежныхСредств.ОплатаПокупателя Тогда
		СтрокаДокумента.СтатьяДДС =	Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоКоду("000000064",Истина);
	КонецЕсли;
	// - Лаврухин
	// {Лео Катанович
	Если ВидОперацииДокумента = Перечисления.ВидыОперацийППИсходящее.ОплатаПоставщику Тогда
		СтрокаДокумента.СтатьяДДС =	Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоКоду("000000001");//Оплата товаров, работ, услуг, сырья и иных оборотных активов
	КонецЕсли;
	// Лео Катанович}
	//_______________________________________________________________________________________________________________
	//10) Определим сумму
...
//Организация, ДАТА, НОМЕР 
	...
	Если ОбъектДокумента.ЭтоНовый() Тогда
		...
		Если Не ОбщегоНазначения.ЕстьНеЦифры(ИсходныеДанные.Номер) Тогда
			...
			ОбъектДокумента.Номер = УстановитьНомерДокумента(ОбъектДокумента, ИсходныеДанные);
			// Лео Катанович}
		КонецЕсли;
	КонецЕсли;
...
//Контрагент,ДоговорКонтрагента,СтатьяДвиженияДенежныхСредств
	...
	//Запрос.Текст = 
	//	"ВЫБРАТЬ
	//	|	СУММА(ЗаявкиНаРасходованиеСредствОстатки.СуммаОстаток) КАК СуммаОстаток,
	...
	//	|	ЗаявкиНаРасходованиеСредствОстатки.Лео_СтатьяОборотов
	//	|ИЗ
	//	|	РегистрНакопления.ЛЕО_ЗаявкиНаРасходованиеСредств.Остатки(
	//	|			&датаВыписки,
	//	|			Контрагент = &контрагент
	//	|				И Организация = &организация
	//	|				И ЗаявкаНаРасходование.ДатаРасхода = &датаплатежа) КАК ЗаявкиНаРасходованиеСредствОстатки
	//	|
	//	|СГРУППИРОВАТЬ ПО
	...
	//Запрос.УстановитьПараметр("датаВыписки", ОбъектДокумента.дата +1);
	//Запрос.УстановитьПараметр("датаплатежа", ОбъектДокумента.Дата);
	...
	//	УстановитьСвойство(СтрокаРасшифровки, "ДокументПланированияПлатежа", ВыборкаДетальныеЗаписи.ЗаявкаНаРасходование);
	//	УстановитьСвойство(СтрокаРасшифровки, "КурсВзаиморасчетовПлан", ВыборкаДетальныеЗаписи.ЗаявкаНаРасходование.КурсДокумента);
	//	УстановитьСвойство(СтрокаРасшифровки, "СуммаПлатежаПлан", ВыборкаДетальныеЗаписи.СуммаОстаток);
	//	УстановитьСвойство(СтрокаРасшифровки, "лео_СтатьяДвиженияДенежныхСредств", ВыборкаДетальныеЗаписи.СтатьяДвиженияДенежныхСредств);
	//	УстановитьСвойство(СтрокаРасшифровки, "Лео_СОБР", ВыборкаДетальныеЗаписи.Лео_СтатьяОборотов);
	//	
	//	
...
//{Лео Катанович
Функция УстановитьНомерДокумента(ОбъектДокумента, ИсходныеДанные)
	
	Если Не ОбщегоНазначения.ЕстьНеЦифры(ИсходныеДанные.Номер) Тогда
			ОкончаниеНомера = Формат(Число(СокрЛП(ИсходныеДанные.Номер)), "ЧН=; ЧГ=0");
			ДлинаОкончания = СтрДлина(ОкончаниеНомера);
			ДлинаНомера = СтрДлина(СокрЛП(ОбъектДокумента.Номер));
			КоличествоНулей = ДлинаНомера - ДлинаОкончания - СтрДлина(Организация.Префикс);
			Подстрока = Организация.Префикс;
			Для Н=1 по КоличествоНулей Цикл
		        Подстрока = Подстрока + "0";
	        КонецЦикла;

			Номер =  Подстрока + Формат(Число(ОкончаниеНомера), "ЧЦ=" + ДлинаОкончания + "; ЧВН=; ЧГ=0");
