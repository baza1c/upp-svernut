...
Процедура СформироватьСписокДолговПоРегистру(ТабРасшифровка)
	...
	|	ТекущиеЗадолженности.ДокументРасчетовСКонтрагентом.абс_ДатаОплаты КАК абс_ДатаОплаты	
	|ИЗ (";
	ТекстОсновнойЗапрос="";
...
// +Лаврухин 270815	
	...
	//|	ТекущиеЗадолженности.ДокументРасчетовСКонтрагентом.абс_ДатаОплаты	
	//|
	//|УПОРЯДОЧИТЬ ПО
	//|	абс_ДатаОплаты "+ ?(СпособЗаполнения="ФИФО", "Возр", "Убыв")+",	
	////|	ДатаВозникновения "+ ?(СпособЗаполнения="ФИФО", "Возр", "Убыв")+",
	//|	ДокументРасчетовСКонтрагентом,
	//|	Сделка";	
	
	Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ПлатежноеПоручениеВходящее") Тогда
		Запрос.Текст=Запрос.Текст+ТекстОсновнойЗапрос+") КАК ТекущиеЗадолженности
		|
		|СГРУППИРОВАТЬ ПО
		|	ТекущиеЗадолженности.ТипЗадолженности,
...
//-Лаврухин 270815
	...
	ТабРасшифровка.Колонки.Добавить("абс_ДатаОплаты");
...
// и формирует таблицу для подбора.
//
Процедура СформироватьСписокПланируемыхДвижений_лео(ДокументПланирования) Экспорт
	...
	|	ПланируемыеДвиженияОстатки.ДокументРасчетовСКонтрагентом,
	|	ПланируемыеДвиженияОстатки.СтатьяДвиженияДенежныхСредств,
	|	ПланируемыеДвиженияОстатки.Лео_СтатьяОборотов,
	|	ПланируемыеДвиженияОстатки.Проект,
	|	ПланируемыеДвиженияОстатки.ДокументПланирования КАК ДокументПланирования,
	|	ПланируемыеДвиженияОстатки.ДокументПланирования.СуммаДокумента КАК СуммаДокумента,
	|ВЫРАЗИТЬ 
	...
	|	ПланируемыеОстатки.ДокументРасчетовСКонтрагентом,
	|	ПланируемыеОстатки.СтатьяДвиженияДенежныхСредств,
	|	ПланируемыеОстатки.Лео_СтатьяОборотов,
	...
	|   ПланируемыеОстатки.СуммаВзаиморасчетовОстаток КАК СуммаВзаиморасчетов
	|ИЗ
	|	РегистрНакопления."+"ЛЕО_ЗаявкиНаРасходованиеСредств"+".Остатки(,("+ТекстУсловия+"))КАК ПланируемыеОстатки";		
	...
	Если НЕ ДокументСсылка = Неопределено Тогда
		...
		|	ПланируемыеДвижения.ДокументРасчетовСКонтрагентом,
		|	ПланируемыеДвижения.СтатьяДвиженияДенежныхСредств,
		|	ПланируемыеДвижения.Лео_СтатьяОборотов,
		...
		|	ПланируемыеДвижения.СуммаВзаиморасчетов КАК СуммаВзаиморасчетов
		|ИЗ
		|	РегистрНакопления."+"ЛЕО_ЗаявкиНаРасходованиеСредств"+" КАК ПланируемыеДвижения
		|ГДЕ
		|	ПланируемыеДвижения.Регистратор = &Регистратор";
...
Процедура ЗаполнитьРасшифровкуПоДолгам(ПодборПоСуммеПлатежа=Истина,КурсПоДоговору=Истина) Экспорт
	...
	Если Не СпособЗаполнения="ТЧ" Тогда
		...
		Если СпособЗаполнения="ЛИФО" Тогда
			...
			РасшифровкаПлатежа.Сортировать("абс_ДатаОплаты Убыв");
...
Процедура ЗаполнитьРасшифровкуПоПланам() Экспорт
	...
	СформироватьСписокПланируемыхДвижений_лео(ИмяДокумента);
	...
	Для Каждого СтрокаПлан Из ПланируемыеДвижения Цикл
		...
		///лео никитин фэо  заполнить расшифровку платежа по заявкам   
		СтрокаПлатеж.Лео_СтатьяДвиженияДенежныхСредств=СтрокаПлан.СтатьяДвиженияДенежныхСредств;
		СтрокаПлатеж.Лео_СОБР=СтрокаПлан.Лео_СтатьяОборотов;
		//СтрокаПлатеж.Лео_СОБР=СтрокаПлан.СтатьяДвиженияДенежныхСредств;

		//СтрокаПлатеж.СтатьяДвиженияДенежныхСредств=ДокументСсылка.ЭлементыФормы.СтатьяДС.Значение;
